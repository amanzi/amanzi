...
Configuration:
  ...
  --skip_rpath            do not add runtime path information during linking. Instead, rely on
                          linking provided by LD_LIBRARY_PATH or /etc/ld.so.conf.
...
if [ ! -n "${mpi_root_dir}" ]; then
  ...
  ${cmake_binary} \
      ...
      -DSKIP_CMAKE_RPATH=${skip_cmake_rpath} \
      ${tools_build_src_dir}
