...
class Amanzi(CMakePackage):
        ...
        depends_on('kokkos +wrapper +debug +cuda +cuda_uvm +cuda_relocatable_device_code ' 
        ...
        depends_on('seacas')
        depends_on('mstk@3.3.2 partitioner=all +exodusii +parallel', when='+mstk')
        ...
        depends_on('trilinos@12.18.1 +pnetcdf +boost +cgns +hdf5 +hypre +metis '
        ...
        def cmake_args(self):
                ...
                if '+gpu_support' in self.spec:
                        options.append('-DAMANZI_ARCH="Summit"')
                else:
                        options.append('-DAMANZI_ARCH="NOTHING"')
