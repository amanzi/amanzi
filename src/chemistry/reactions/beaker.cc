...
void Beaker::ScaleRHSAndJacobian(void) {
  for (int i = 0; i < jacobian_->getSize(); i++) {
...
void Beaker::SolveLinearSystem(void) {
  // LU direct solve
#ifndef GLENN
  double D;
  ludcmp(jacobian_->getValues(), ncomp(), &indices_, &D);
  lubksb(jacobian_->getValues(), ncomp(), indices_, &rhs_);
#else
  solver->Solve(A, b);
#endif
