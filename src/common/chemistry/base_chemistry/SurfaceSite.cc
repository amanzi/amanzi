...
SurfaceSite::SurfaceSite(const std::string& name, int id, const Teuchos::ParameterList& plist)
  : name_(name), identifier_(id), charge_(0.0), molar_surface_density_(0.0)
{
...
void
SurfaceSite::AddMineral(Mineral* mineral)
{
...
void
SurfaceSite::UpdateSiteDensity(double site_density)
{
...
void
SurfaceSite::Display(const Teuchos::Ptr<VerboseObject> vo) const
{
  std::stringstream message;
  message << std::setw(15) << name() << std::setw(15) << std::scientific << molar_density()
          << std::endl;
...
void
SurfaceSite::DisplayResultsHeader(const Teuchos::Ptr<VerboseObject> vo) const
{
  std::stringstream message;
  message << std::setw(15) << "Site Name" << std::setw(15) << "Free Conc." << std::endl;
  message << std::setw(15) << " " << std::setw(15) << "[mol/m^3 bulk]" << std::endl;
  vo->Write(Teuchos::VERB_HIGH, message.str());
}


void
SurfaceSite::DisplayResults(const Teuchos::Ptr<VerboseObject> vo) const
{
  std::stringstream message;
  message << std::setw(15) << name() << std::scientific << std::setprecision(5) << std::setw(15)
          << free_site_concentration() << std::endl;
  vo->Write(Teuchos::VERB_HIGH, message.str());
}
