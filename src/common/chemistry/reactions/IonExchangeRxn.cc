...
IonExchangeRxn::IonExchangeRxn()
  : uniform_z_(false), uniform_z_set_(false), ref_cation_sorbed_conc_(1.e-9)
{
  ionx_site_.clear();
...
void
IonExchangeRxn::AddIonExchangeComplex(const IonExchangeComplex& ionx_complex)
{
...
void
IonExchangeRxn::AddIonExchangeSite(const IonExchangeSite& site)
{
...
void
IonExchangeRxn::Update(const std::vector<Species>& primary_species)
{
...
void
IonExchangeRxn::AddContributionToTotal(std::vector<double>* total)
{
  // pflotran: reaction.F90, function RTotalSorbEqIonX
...
void
IonExchangeRxn::AddContributionToDTotal(const std::vector<Species>& primary_species,
                                        MatrixBlock* dtotal)
{
  // pflotran: reaction.F90, function RTotalSorbEqIonX
  ...
  for (auto ionx = ionx_complexes_.begin(); ionx != ionx_complexes_.end(); ionx++) {
    ...
    for (auto ionx2 = ionx_complexes_.begin(); ionx2 != ionx_complexes_.end(); ionx2++) {
      ...
      if (ionx == ionx2) {
        value =
          ionx->concentration() * (1.0 - temp * ionx2->X()) / primary_species[jcomp].molality();
      } else {
...
void
IonExchangeRxn::CheckUniformZ(const std::vector<Species>& primary_species)
{
  bool flag = true;
...
void
IonExchangeRxn::DisplayResultsHeader() const
{
...
void
IonExchangeRxn::DisplayResults(const Teuchos::Ptr<VerboseObject> vo) const
{
