...
****************************************************************** */
void
MatrixBlock::Resize(int new_size)
{
  if (A_) delete[] A_;
  size_ = new_size;
  cols_ = new_size;
...
void
MatrixBlock::PutScalar(double val)
{
  for (int i = 0; i < size_ * cols_; i++) A_[i] = val;
}
...
double
MatrixBlock::GetRowAbsMax(int irow)
{
  double max = 0.0;
...
void
MatrixBlock::Scale(double scale)
{
...
void
MatrixBlock::AddValue(int i, int j, double value)
{
  A_[j * size_ + i] += value;
}
...
void
MatrixBlock::AddValues(MatrixBlock* b)
{
...
void
MatrixBlock::AddValues(MatrixBlock* b, double scale)
{
  double* B = b->GetValues();
