...
string
ConvertTimeToSeconds(const string& time_string)
{
...
string
MakePPPrefix(const string& s1)
{
  return MangleString(s1);
}

string
MakePPPrefix(const string& s1, const string& s2)
{
  return MangleString(s1) + string(".") + MangleString(s2);
}

string
MakePPPrefix(const string& s1, const string& s2, const string& s3)
{
  return MangleString(s1) + string(".") + MangleString(s2) + string(".") + MangleString(s3);
}

string
MakePPPrefix(const string& s1, const string& s2, const string& s3, const string& s4)
{
  return MangleString(s1) + string(".") + MangleString(s2) + string(".") + MangleString(s3) +
         string(".") + MangleString(s4);
}
...
{
  return MangleString(s1) + string(".") + MangleString(s2) + string(".") + MangleString(s3) +
         string(".") + MangleString(s4) + string(".") + MangleString(s5);
}

// Construct a ParmParse entry from sets of strings/values.
list<string>
MakePPEntry(const string& s1)
{
  list<string> pp;
...
list<string>
MakePPEntry(const vector<string>& ss)
{
  list<string> pp;
  for (size_t i = 0; i < ss.size() ; ++i) pp.push_back(MangleString(ss[i]));
  return pp;
}

list<string>
MakePPEntry(const vector<double>& ds)
{
  list<string> pp;
...
list<string>
MakePPEntry(const vector<int>& is)
{
  list<string> pp;
...
void
AddToTable(list<ParmParse::PP_entry>& table, const string& entry_name, list<string> entry)
{
...
bool
InputConverterS::ParseMechProperty_(DOMElement* mech_prop_node,
  ...
  if (found) {
    if (property_name == "dispersion_tensor") // Weirdo!
    {
      ...
      if (type == "bear") {
        ...
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_l"),
                   MakePPEntry(alpha_l));
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_t"),
                   MakePPEntry(alpha_t));

        non_zero_diffusion = (atof(alpha_l.c_str()) != 0) || (atof(alpha_t.c_str()) != 0);
        ...
        AddToTable(table,
                   ...
                   MakePPEntry(alpha_l));
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_th"),
                   MakePPEntry(alpha_th));
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_tv"),
                   MakePPEntry(alpha_tv));
        non_zero_diffusion =
          (atof(alpha_l.c_str()) != 0) || (atof(alpha_th.c_str()) != 0) || (atof(alpha_tv.c_str()));
        ...
        string alpha_tv = GetAttributeValueS_(property, "alpha_tv");
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_lh"),
                   MakePPEntry(alpha_lh));
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_lv"),
        ...
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "alpha_tv"),
                   MakePPEntry(alpha_tv));
        non_zero_diffusion = (atof(alpha_lh.c_str()) != 0) || (atof(alpha_lv.c_str()) != 0) ||
                             (atof(alpha_th.c_str()) != 0) || (atof(alpha_tv.c_str()));
      } else if (type == "file") {
        string filename = GetAttributeValueS_(property, "filename");
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "filename"),
                   MakePPEntry(filename));
      } else ThrowErrorIllformed_("materials->mechanical_properties", "type", property_name);
      AddToTable(
        table, MakePPPrefix("rock", material_name, property_name, "type"), MakePPEntry(type));
    } else {
      ...
      if (!value.empty() && ((property_name != "porosity") || (type != "gslib"))) {
        AddToTable(
          table, MakePPPrefix("rock", material_name, property_name, "vals"), MakePPEntry(value));
        AddToTable(table,
                   MakePPPrefix("rock", material_name, property_name, "distribution_type"),
                   MakePPEntry("uniform"));
      } else {
        if (!type.empty()) {
          if (type == "file") {
            ...
            AddToTable(table,
                       ...
                       MakePPEntry(filename));
            AddToTable(table,
                       MakePPPrefix("rock", material_name, property_name, "type"),
                       MakePPEntry("file"));
          } else if ((property_name == "porosity") && (type == "gslib")) // special considerations!
          {
            ...
            AddToTable(table,
                       MakePPPrefix("rock", material_name, property_name, "parameter_file"),
            ...
            if (!data_file.empty()) {
              AddToTable(table,
                         MakePPPrefix("rock", material_name, property_name, "data_file"),
  ...
  if (found) {
    ...
    if (found) {
      for (size_t i = 0; i < constants.size(); ++i) {
        ...
        if (type == "time") labeled_times_[name] = value;
        else if (type == "numerical") labeled_numbers_[name] = value;
        else if (type == "area_mass_flux") labeled_area_mass_fluxes_[name] = value;
      }
    }
  }
  ...
  if (found) {
    ...
    if (found) {
      for (size_t i = 0; i < time_macros.size(); ++i) {
        ...
        if (found) {
          ...
          AddToTable(
            table, MakePPPrefix("amr", "time_macro", macro_name, "type"), MakePPEntry("period"));
          AddToTable(
            table, MakePPPrefix("amr", "time_macro", macro_name, "start"), MakePPEntry(start));
          AddToTable(
            table, MakePPPrefix("amr", "time_macro", macro_name, "stop"), MakePPEntry(stop));
          AddToTable(table,
                     MakePPPrefix("amr", "time_macro", macro_name, "period"),
          ...
          AddToTable(
            table, MakePPPrefix("amr", "time_macro", macro_name, "type"), MakePPEntry("times"));
          AddToTable(
            table, MakePPPrefix("amr", "time_macro", macro_name, "times"), MakePPEntry(times));
        }
      }
    }
    // List of all time macro names.
    AddToTable(table, MakePPPrefix("amr", "time_macros"), MakePPEntry(time_macro_names));
    ...
    if (found) {
      for (size_t i = 0; i < cycle_macros.size(); ++i) {
        ...
        // Shove this macro into our table.
        AddToTable(
          table, MakePPPrefix("amr", "cycle_macro", macro_name, "type"), MakePPEntry("period"));
        AddToTable(
          table, MakePPPrefix("amr", "cycle_macro", macro_name, "start"), MakePPEntry(start));
        AddToTable(
          table, MakePPPrefix("amr", "cycle_macro", macro_name, "stop"), MakePPEntry(stop));
        AddToTable(table,
                   MakePPPrefix("amr", "cycle_macro", macro_name, "period"),
                   MakePPEntry(timestep_interval));
      }
    }
    // List of all cycle macro names.
    AddToTable(table, MakePPPrefix("amr", "cycle_macros"), MakePPEntry(cycle_macro_names));
...
void
InputConverterS::ParseExecutionControls_()
{
  list<ParmParse::PP_entry> table;
  bool found;
  ...
  if (found) {
    ...
    default_vals["reduction_factor"] =
      GetAttributeValueS_(def, "reduction_factor", TYPE_NUMERICAL, true);
    default_vals["increase_factor"] =
      GetAttributeValueS_(def, "increase_factor", TYPE_NUMERICAL, true);
    ...
    if (!found) {
      ...
      string control_names[controls_size] = {
        "start",           "end",    "init_dt",   "mode", "max_dt", "reduction_factor",
      ...
      for (int i = 0; i < ncn; ++i) {
        for (map<string, string>::iterator it = exec_control[i].begin();
             it != exec_control[i].end();
             ++it) {
          if (it->first == "start" || it->first == "end" || it->first == "init_dt" ||
              it->first == "max_dt") {
            map<string, string>::const_iterator iter = labeled_times_.find(it->second);
            if (iter != labeled_times_.end() ) it->second = ConvertTimeToSeconds(iter->second);
            else it->second = ConvertTimeToSeconds(it->second);
          }
        }
      }
      ...
      for (int i = 0; i < exec_control.size(); ++i) {
        ...
        for (map<string, string>::const_iterator it = exec_control[i].begin();
             ...
             ++it) {
          AddToTable(
            table, MakePPPrefix("exec_control", ecnames[i], it->first), MakePPEntry(it->second));
        }
      }
...
void
InputConverterS::ParseNumericalControls_(const string& flow_model)
{
  list<ParmParse::PP_entry> table;
  bool found;
...
#undef CONTROLS_ARE_ATTRIBUTES
  ...
  // Structured controls.
  DOMNode* structured_controls =
    GetUniqueElementByTagsString_("numerical_controls, structured_controls", found);
  if (found) {
    bool found;
    MemoryManager mm;
    ...
    if (ts_controls.size() > 0) {
      for (map<string, string>::const_iterator it = ts_controls.begin() ; it != ts_controls.end();
           ++it) {
        string s_parameter_name = "steady_" + it->first;
        AddToTable(
          table, MakePPPrefix("prob", (s_parameter_name).c_str()), MakePPEntry(it->second));
      }
    }
    ...
    if (flow_controls.size() > 0) {
      for (map<string, string>::const_iterator it = flow_controls.begin();
           it != flow_controls.end();
           ++it) {
        string s_parameter_name;
        if ((it->first == "petsc_options_file") || (it->first == "gravity") ||
            (it->first == "gravity_dir") || (it->first == "domain_thickness")) {
    ...
    if (transport_controls.size() > 0) {
      for (map<string, string>::const_iterator it = transport_controls.begin();
           it != transport_controls.end();
           ++it) {
    ...
    if (found) {
      ...
      for (int i = 0; i < children->getLength(); ++i) {
        ...
        if (strcmp(cname, "refinement_indicator") == 0) {
          ...
          for (int j = 0; j < rc_children->getLength(); ++j) {
            ...
            } else if (strcmp(rc_ename, "start_time") == 0 || strcmp(rc_ename, "end_time") == 0) {
              string thisTime = mm.transcode(jnode->getTextContent());
              map<string, string>::const_iterator iter = labeled_times_.find(thisTime);
              if (iter != labeled_times_.end() ) rc_data[rc_name][rc_ename].push_back(iter->second);
              else rc_data[rc_name][rc_ename].push_back(thisTime);
            } else {
              if (strcmp(rc_ename, "value_greater") == 0 || strcmp(rc_ename, "value_less") == 0 ||
      ...
      if (rc_data.size() > 0) {
        ...
        ;
        for (map<string, map<string, vector<string>>>::const_iterator it = rc_data.begin();
             it != rc_data.end();
             ++it) {
          rc_names.push_back(it->first);
          const map<string, vector<string>>& rc_map = it->second;
          for (map<string, vector<string>>::const_iterator it1 = rc_map.begin();
               it1 != rc_map.end();
               ++it1) {
            AddToTable(table,
  ...
  for (map<string, string>::iterator it = amr_controls.begin(); it != amr_controls.end(); ++it) {
    if (it->first != "amr_levels") {
      if (it->first == "refinement_ratio" || it->first == "regrid_interval" ||
          it->first == "blocking_factor" || it->first == "number_error_buffer_cells" ||
...
void
InputConverterS::ParseMesh_()
{
  list<ParmParse::PP_entry> table;
  bool found;

  DOMElement* dimension =
    static_cast<DOMElement*>(GetUniqueElementByTagsString_("mesh, dimension", found));
  {
  ...
  DOMElement* generate =
    static_cast<DOMElement*>(GetUniqueElementByTagsString_("mesh, generate", found));
  if (found) {
    bool found;
  ...
  } else ThrowErrorMisschild_("mesh", "generate", "mesh");

  ParmParse::appendTable(table);
}

static void
MakeBox(list<ParmParse::PP_entry>& table,
        const string& name,
...
InputConverterS::ParseRegions_()
{
  ...
  for (int d = 0; d < dim_; ++d) max_size = max(max_size, hi_coords_[d] - lo_coords_[d]);
  ...
  hi = hi_coords_;
  hi[0] = lo[0];
  MakeBox(table, name, "xlobc", lo, hi);
  region_names.push_back(name);
  name = "XHIBC";
  lo = lo_coords_;
  hi = hi_coords_;
  lo[0] = hi[0];
  MakeBox(table, name, "xhibc", lo, hi);
  region_names.push_back(name);
  name = "YLOBC";
  lo = lo_coords_;
  hi = hi_coords_;
  hi[1] = lo[1];
  MakeBox(table, name, "ylobc", lo, hi);
  region_names.push_back(name);
  name = "YHIBC";
  lo = lo_coords_;
  hi = hi_coords_;
  lo[1] = hi[1];
  MakeBox(table, name, "yhibc", lo, hi);
  region_names.push_back(name);
  if (dim_ > 2) {
    name = "ZLOBC";
    lo = lo_coords_;
    hi = hi_coords_;
    hi[2] = lo[2];
    MakeBox(table, name, "zlobc", lo, hi);
    region_names.push_back(name);
    name = "ZHIBC";
    lo = lo_coords_;
    hi = hi_coords_;
    lo[2] = hi[2];
    MakeBox(table, name, "zhibc", lo, hi);
    region_names.push_back(name);
  }
  ...
  if (found) {
    ...
    for (size_t i = 0; i < boxes.size(); ++i) {
      ...
      AddToTable(
        table, MakePPPrefix("geometry", region_name, "lo_coordinate"), MakePPEntry(lo_coords));
      AddToTable(
        table, MakePPPrefix("geometry", region_name, "hi_coordinate"), MakePPEntry(hi_coords));
      ...
      AddToTable(table, MakePPPrefix("geometry", region_name, "type"), MakePPEntry(type));
    ...
    if (dim_ == 2) {
      ...
      for (size_t i = 0; i < polygons.size(); ++i) {
        ...
        if (!found || points.size() != num_points) {
          std::cout << "points.size() != num_points " << points.size() << " " << num_points
                    << std::endl;
        ...
        for (size_t j = 0; j < points.size(); ++j) {
          ...
          if (dim_ == 3) v3.push_back(coords[2]);
        }
    ...
    for (size_t i = 0; i < logicals.size(); ++i) {
      ...
      if (operation == "complement") {
        AddToTable(
          table, MakePPPrefix("geometry", region_name, "region"), MakePPEntry(region_list));
      } else {
        vector<string> tokens = BoxLib::Tokenize(region_list, ",");
...
void
InputConverterS::ParseGeochemistry_()
{
  list<ParmParse::PP_entry> table;
  ...
  if (state == "on") {
    ...
    if (engine == "amanzi") {
      ...
      node =
        GetUniqueElementByTagsString_("geochemistry, amanzi_chemistry, reaction_network", flag);
      if (flag) {
      ...
      AddToTable(
        table, MakePPPrefix("Chemistry", "Thermodynamic_Database_File"), MakePPEntry(bgdfilename));
      AddToTable(
        table, MakePPPrefix("Chemistry", "Thermodynamic_Database_Format"), MakePPEntry(format));
      ...
      if (valid_engine) {
        ...
        if (flag) {
          element = static_cast<DOMElement*>(node);
          std::string inpfilename = (element->hasAttribute(mm.transcode("input_filename")) ?
                                       GetAttributeValueS_(element, "input_filename") :
                                       CreateINFile_(xmlfilename_, rank_));
          AddToTable(
            table, MakePPPrefix("Chemistry", "Engine_Input_File"), MakePPEntry(inpfilename));
        } else {
          Errors::Message msg;
...
void
InputConverterS::ParseMaterials_(bool& do_tracer_diffusion)
{
  list<ParmParse::PP_entry> table;
  bool found;
  ...
  if (found) {
    ...
    for (size_t i = 0; i < mats.size(); ++i) {
      ...
      if (found) {
        ...
        ParseMechProperty_(mech_prop,
                           mat_name,
                           "particle_density",
                           table,
                           false); // FIXME: Should be true for required!
      ...
      AddToTable(table, MakePPPrefix("rock", mat_name, "regions"), MakePPEntry(assigned_regions));
      ...
      if (!k_found && !K_found) {
        Errors::Message msg;
        msg << "Neither permeability nor hydraulic_conductivity was found for material \""
            << mat_name << "\".\n";
        msg << "Please correct and try again.\n";
        Exceptions::amanzi_throw(msg);
        ...
        if (!x.empty()) {
          AddToTable(table,
                     MakePPPrefix("rock", mat_name, "permeability", "horizontal", "vals"),
                     MakePPEntry(x));

          string y = GetAttributeValueS_(permeability, "y");
          if (dim_ < 3) {
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "vertical", "vals"),
                       MakePPEntry(y));
          } else {
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "horizontal", "vals"),
                       MakePPEntry(x));
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "horizontal1", "vals"),
                       MakePPEntry(y));
          ...
          AddToTable(
            table, MakePPPrefix("rock", mat_name, "permeability", "type"), MakePPEntry(type));
          if (type == "file") {
            string filename = GetAttributeValueS_(permeability, "filename");
            string attribute = GetAttributeValueS_(permeability, "attribute");
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "filename"),
                       MakePPEntry(filename));
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "attribute"),
            ...
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "parameter_file"),
                       MakePPEntry(parameter_file));
            AddToTable(
              table, MakePPPrefix("rock", mat_name, "permeability", "value"), MakePPEntry(value));
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "permeability", "data_file"),
                       MakePPEntry(data_file));
          } else ThrowErrorIllformed_("materials", "type", "permeability");
        }
      } else {
        string x = GetAttributeValueS_(conductivity, "x", TYPE_NUMERICAL, false);
        if (!x.empty()) {
          AddToTable(table,
                     MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "horizontal", "vals"),
                     MakePPEntry(x));
          string y = GetAttributeValueS_(conductivity, "y");
          if (dim_ < 3) {
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "vertical", "vals"),
                       MakePPEntry(y));
          } else {
            AddToTable(
              table,
              MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "horizontal1", "vals"),
              MakePPEntry(y));
          ...
          string type = GetAttributeValueS_(permeability, "type");
          AddToTable(table,
                     MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "type"),
                     MakePPEntry(type));
          if (type == "gslib") {
            ...
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "parameter_file"),
                       MakePPEntry(parameter_file));
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "value"),
                       MakePPEntry(value));
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "hydraulic_conductivity", "data_file"),
                       MakePPEntry(data_file));
          } else ThrowErrorIllformed_("materials", "type", "hydraulic_conductivity");
        }
      }
      ...
      if (found) {
        ...
        if (model == "van_genuchten")
          AddToTable(
            table, MakePPPrefix("rock", mat_name, "cpl", "type"), MakePPEntry("VanGenuchten"));
        else if (model == "brooks_corey")
          AddToTable(
            table, MakePPPrefix("rock", mat_name, "cpl", "type"), MakePPEntry("BrooksCorey"));
        else if (model != "none") ThrowErrorIllformed_("materials", "type", "cap_pressure");
        ...
        if ((model == "van_genuchten") || (model == "brooks_corey")) {
          ...
          string optional_krel_smoothing_interval = GetAttributeValueS_(
            parameters, "optional_krel_smoothing_interval", TYPE_NUMERICAL, false);
          if (!optional_krel_smoothing_interval.empty()) {
            AddToTable(table,
                       MakePPPrefix("rock", mat_name, "cpl", "Kr_smoothing_max_pcap"),
      ...
      if (found) {
        ...
        else if (model != "none") ThrowErrorIllformed_("materials", "type", "rel_perm");
      ...
      if (found) {
        ...
        for (size_t i = 0; i < solutes.size(); ++i) {
          ...
          if (found) {
            ...
            if (!kd.empty()) {
              AddToTable(table,
                         MakePPPrefix("rock", mat_name, "sorption_isotherms", solute_name, "Kd"),
                         MakePPEntry(kd));
            } else {
              ...
              if (!b.empty()) {
                AddToTable(
                  table,
                  MakePPPrefix("rock", mat_name, "sorption_isotherms", solute_name, "Langmuir b"),
                  MakePPEntry(b));
              } else {
                ...
                if (!n.empty()) {
                  AddToTable(table,
                             MakePPPrefix(
                               "rock", mat_name, "sorption_isotherms", solute_name, "Freundlich n"),
                             MakePPEntry(n));
                } else
...
void
InputConverterS::ParseProcessKernels_(string& flow_model, bool& do_tracer_diffusion)
{
  list<ParmParse::PP_entry> table;
  bool found;
  ...
  DOMElement* flow =
    static_cast<DOMElement*>(GetUniqueElementByTagsString_("process_kernels, flow", found));
  if (!found) ThrowErrorMisschild_("process_kernels", "flow", "process_kernels");
  DOMElement* transport =
    static_cast<DOMElement*>(GetUniqueElementByTagsString_("process_kernels, transport", found));
  if (!found) ThrowErrorMisschild_("process_kernels", "transport", "process_kernels");
  GetUniqueElementByTagsString_("process_kernels, chemistry", found);
  if (!found) ThrowErrorMisschild_("process_kernels", "chemistry", "process_kernels");
  ...
  AddToTable(
    table, MakePPPrefix("prob", "do_tracer_advection"), MakePPEntry((transport_state == "on")));
...
void
InputConverterS::ParseSoluteNames_()
{
  bool found;
  DOMElement* liquid_phase =
    static_cast<DOMElement*>(GetUniqueElementByTagsString_("phases, liquid_phase", found));
  if (found) {
    bool found;
...
void
InputConverterS::ParsePhases_(bool& do_tracer_diffusion)
{
  list<ParmParse::PP_entry> table;
  ...
  if (found) {
    ...
    if (found) {
      ...
      for (size_t i = 0; i < sols.size(); ++i) {
        ...
        string diff_coeff =
          GetAttributeValueS_(solute, "coefficient_of_diffusion", TYPE_NONE, false);
        if (diff_coeff != "") {
          AddToTable(table,
...
void
InputConverterS::ParseInitialConditions_()
{
  list<ParmParse::PP_entry> table;
  bool found;
      ...
      for (size_t i = 0; i < ics.size(); ++i) {
        ...
        if (pfound) {
          ...
          if (cfound) {
            ...
            string ic_type_labels[5] = { "uniform_pressure",
                                         "linear_pressure",
                                         "uniform_saturation",
                                         "linear_saturation",
                                         "velocity" };
            ...
            for (int i = 0; i < 5 && !tfound; ++i) {
              ...
              if (tfound) {
                  ...
                  if (ic_type_labels[i] == "linear_pressure" ||
                    ...
                    if (gvalues.size() != dim_)
                      ThrowErrorIllformed_(
                        "initial_conditions->linear_pressure", "coordinate array", "gradient");

                    rpoints = GetAttributeVectorD_(elt, "reference_coord", dim_, "m", true);
                    if (rpoints.size() != dim_)
                      ThrowErrorIllformed_("initial_conditions->linear_pressure",
                                           "coordinate array",
                                           "reference_coord");
                  }
                }
                ...
                AddToTable(table,
                           MakePPPrefix("comp", "ics", ic_name, "type"),
                           MakePPEntry(ic_type_labels[i]));

                if (values.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "ics", ic_name, "val"), MakePPEntry(values));
                if (vel.size() > 0)
                  AddToTable(table, MakePPPrefix("comp", "ics", ic_name, "vel"), MakePPEntry(vel));
                if (gvalues.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "ics", ic_name, "grad"), MakePPEntry(gvalues));
                if (rvalues.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "ics", ic_name, "vals"), MakePPEntry(rvalues));
                if (rpoints.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "ics", ic_name, "loc"), MakePPEntry(rpoints));
              }
            }
          } else {
            Errors::Message msg;
          ...
          if (scfound) {
            ...
            if (sols.size() > 0) {
              for (size_t si = 0; si < sols.size(); ++si) {
                ...
                if (!found) {
                  Errors::Message msg;
                  msg << "\"uniform_conc\" section names nonexisting solute in initial_condition \""
                      << ic_name << "\".\n";
                  msg << "Please correct and try again.\n";
                  Exceptions::amanzi_throw(msg);
                }
              ...
              for (int s = 0; s < solutes_.size(); ++s) {
                AddToTable(table,
                           MakePPPrefix("tracer", solutes_[s], "ic", ic_name, "val"),
                           MakePPEntry(conc[s]));
                AddToTable(table,
                           MakePPPrefix("tracer", solutes_[s], "ic", ic_name, "type"),
                           MakePPEntry("concentration"));
              }
            }
          }
              ...
              if (!constraint_name.empty()) {
                ...
                if (known_cond) {
                  for (int s = 0; s < solutes_.size(); ++s) {
                    AddToTable(
                      table,
                      MakePPPrefix("tracer", solutes_[s], "ic", ic_name, "geochemical_condition"),
                      MakePPEntry(constraint_name));
                    AddToTable(table,
                               MakePPPrefix("tracer", solutes_[s], "ic", ic_name, "type"),
                               MakePPEntry("concentration"));
                  }
                  ...
                  Errors::Message msg;
                  msg << "Undeclared constraint name \"" << constraint_name
                      << "\" in initial_condition \"" << ic_name << "\".\n";
                  msg << "Please correct and try again.\n";
                  Exceptions::amanzi_throw(msg);
                }
              } else {
                Errors::Message msg;
                msg << "No \"name\" attibute specified for geochemical constraint in "
                       "initial_condition \""
                    << ic_name << "\".\n";
                msg << "Please correct and try again.\n";
                Exceptions::amanzi_throw(msg);
          ...
          if (rfound) {
            AddToTable(table,
                       MakePPPrefix("comp", "ics", ic_name, "regions"),
                       MakePPEntry(assigned_regions));
            if (sfound) {
              for (size_t i = 0; i < solutes_.size(); ++i) {
                AddToTable(table,
                           MakePPPrefix("tracer", solutes_[i], "ic", ic_name, "regions"),
                           MakePPEntry(assigned_regions));
              }
            }
          } else {
...
void
InputConverterS::ParseBoundaryConditions_()
{
  list<ParmParse::PP_entry> table;
  bool found;
    ...
    if (bfound) {
      ...
      for (size_t i = 0; i < bcs.size(); ++i) {
        ...
        if (pfound) {
          ...
          if (cfound) {
            vector<DOMNode*> nodes;
            string bc_type_labels[10] = { "inward_mass_flux",       "outward_mass_flux",
                                          "inward_volumetric_flux", "outward_volumetric_flux",
                                          "uniform_pressure",       "linear_pressure",
                                          "seepage_face",           "hydrostatic",
                                          "linear_hydrostatic",     "no_flow" };
            bool tfound = false;
            for (int i = 0; i < 10 && !tfound; ++i) {
              ...
              if (tfound) {
                  ...
                  if (bc_type_labels[i] != "linear_pressure" &&
                    ...
                    if (bc_type_labels[i] == "inward_mass_flux" ||
                      ...
                      map<string, string>::const_iterator iter =
                        labeled_area_mass_fluxes_.find(this_value);
                      if (iter != labeled_area_mass_fluxes_.end() ) values.push_back(iter->second);
                      else values.push_back(this_value);
                    } else {
                    ...
                    else starts.push_back(ConvertTimeToSeconds(this_start));
                  }
                ...
                AddToTable(table,
                           MakePPPrefix("comp", "bcs", bc_name, "type"),
                           MakePPEntry(bc_type_labels[i]));

                if (functions.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "forms"), MakePPEntry(functions));
                if (starts.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "times"), MakePPEntry(starts));
                if (values.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "vals"), MakePPEntry(values));
                if (imf.size() > 0)
                  AddToTable(table,
                             MakePPPrefix("comp", "bcs", bc_name, "inward_mass_flux"),
                             MakePPEntry(imf));
                if (gvalues.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "grad"), MakePPEntry(gvalues));
                if (rvalues.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "vals"), MakePPEntry(rvalues));
                if (rpoints.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "loc"), MakePPEntry(rpoints));
                if (csys.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "coord_sys"), MakePPEntry(csys));
                if (rwths.size() > 0)
                  AddToTable(
                    table, MakePPPrefix("comp", "bcs", bc_name, "wt"), MakePPEntry(functions));
                if (sms.size() > 0)
                  AddToTable(table,
                             MakePPPrefix("comp", "bcs", bc_name, "submodel"),
                             MakePPEntry(functions));
              }
            }
          } else {
            Errors::Message msg;
          ...
          if (scfound) {
            ...
            if (sols.size() > 0) {
              for (size_t si = 0; si < sols.size(); ++si) {
                ...
                for (int s = 0; s < solutes_.size() && !found; ++s) {
                  if (this_spec == solutes_[s]) {
                    ...
                    if (iterV != labeled_numbers_.end() ) conc[s].push_back(iterV->second);
                    else conc[s].push_back(this_conc);
                    ...
                    else ctime[s].push_back(ConvertTimeToSeconds(this_time));
                ...
                if (!found) {
                  Errors::Message msg;
                  msg
                    << "\"aqueous_conc\" section names nonexisting solute in boundary_condition \""
                    << bc_name << "\".\n";
                  msg << "Please correct and try again.\n";
                  Exceptions::amanzi_throw(msg);
                }
              ...
              for (int s = 0; s < solutes_.size(); ++s) {
                AddToTable(table,
                           MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "vals"),
                           MakePPEntry(conc[s]));
                AddToTable(table,
                           MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "type"),
                           MakePPEntry("concentration"));
                if (conc[s].size() > 1) {
                  AddToTable(table,
                             MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "times"),
                             MakePPEntry(ctime[s]));
                  AddToTable(table,
                             MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "forms"),
                             MakePPEntry(cfunc[s]));
                }
              }
            }
            ...
            if (gfound) {
                  ...
                  if (known_cond) {
                    ...
                    if (!condition_form.empty()) {
                      if (condition_form != "constant") {
                        Errors::Message msg;
                        msg << "Unsupport non-constant constraint interval specified in "
                               "boundary_condition \""
                            << bc_name << "\".\n";
                        msg << "Please correct and try again.\n";
                        Exceptions::amanzi_throw(msg);
              ...
              for (int g = 0; g < gicdata.size(); ++g) {
                if (gicdata[g].size() > 1) {
                  ...
                  else gicdata[g][1] = ConvertTimeToSeconds(this_time);
                }
              }
              ...
              if (gicdata.size() > 1) {
                for (int g = 0; g < gicdata.size(); ++g) {
                  if (gicdata[g].size() < 2) {
                    Errors::Message msg;
                    msg << "When using multiple geochemical constraints on a boundary, the "
                           "\"start\" attribule is required for boundary_condition \""
                        << bc_name << "\".\n";
                    msg << "Please correct and try again.\n";
                    Exceptions::amanzi_throw(msg);
                  }
              ...
              if (gicdata.size() > 1) {
                Errors::Message msg;
                msg << "Multiple time intervals not yet supported in AmanziS for geochemical "
                       "constraint boundary_condition \""
                    << bc_name << "\".\n";
                msg << "Please correct and try again.\n";
                Exceptions::amanzi_throw(msg);
              } else {
                ...
                if (gicdata.size() == 1) {
                  for (int s = 0; s < solutes_.size(); ++s) {
                    AddToTable(
                      table,
                      MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "geochemical_conditions"),
                      MakePPEntry(gicdata[0][0]));
                  }
                } else {
                  ...
                  for (int s = 0; s < solutes_.size(); ++s) {
                    AddToTable(
                      table,
                      MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "geochemical_conditions"),
                      MakePPEntry(gicnames));
                    AddToTable(table,
                               MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "times"),
                               MakePPEntry(gictimes));
                    AddToTable(table,
                               MakePPPrefix("tracer", solutes_[s], "bc", bc_name, "forms"),
                               MakePPEntry(gicforms));
                  }
                }
              }
            }
          ...
          if (rfound) {
            AddToTable(table,
                       MakePPPrefix("comp", "bcs", bc_name, "regions"),
                       MakePPEntry(assigned_regions));
            if (sfound) {
              tbc_names.push_back(bc_name);
              for (size_t i = 0; i < solutes_.size(); ++i) {
                AddToTable(table,
                           MakePPPrefix("tracer", solutes_[i], "bc", bc_name, "regions"),
                           MakePPEntry(assigned_regions));
              }
            }
          } else {
...
void
InputConverterS::ParseMisc_()
{
  // FIXME: Not yet supported.
}

void
InputConverterS::Translate(int rank)
{
  rank_ = rank;
