...
* Returns the name of this file.  
****************************************************************** */
std::string InputConverterS::CreateINFile(std::string& filename)
{
  MemoryManager mm;
  DOMNode* node;
  DOMElement* element;
  ...
  // get species names
  if (!solute_names_parsed_) {
    Errors::Message msg;
    msg << "parsing error: Must parse tracers/solutes/primaries prior to writing IN file.\n";
    msg << "Please correct and try again.\n";
    Exceptions::amanzi_throw(msg);
  }
  int nsolutes = solutes_.size();
  for (int i = 0; i < nsolutes; ++i) {
    primaries << "    " << solutes_[i] << "\n";
  }
  ...
  if (rank_ == 0) {
    ...
    if (!status) {
      std::cout << "File \"" << infilename.c_str() 
		<< "\" exists, skipping its creation." << std::endl;
    } else {
