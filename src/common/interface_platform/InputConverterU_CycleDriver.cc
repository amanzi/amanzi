...
InputConverterU::TranslateCycleDriver_()
{
  ...
  // parse process_kernels
  int tp_id(0);
  std::string model, state, pkname, mpcname;
  std::map<std::string, bool> pk_state;
  std::map<std::string, MPC> mpcs;
  ...
  for (int i = 0; i < pks->getLength(); ++i) {
    ...
    for (int j = 0; j < children->getLength(); ++j) {
      ...
      if (strcmp(tagname, "flow") == 0) {
        ...
        model = (flow_model_ == "richards") ? "richards" : "darcy";
        mpcs[tagname].submodel = model;
        mpcs[tagname].is_pk = true;
        mpcs[tagname].matrix_fracture = coupled_flow_;

        pk_model_["flow"].push_back(model);
        ...
        std::string ext = (model == "amanzi") ? "amanzi" : "alquimia";
        mpcs[tagname].submodel = "chemistry " + ext;
        mpcs[tagname].is_pk = true;
        mpcs[tagname].matrix_fracture = coupled_flow_;
        ...
        ic_time_chemistry_ = tp_t0[mode];

      } else if (strcmp(tagname, "transport") == 0) {
        model = "transport";
        if (transport_implicit_) model += " implicit";
        mpcs[tagname].submodel = model;
        mpcs[tagname].is_pk = true;
        mpcs[tagname].matrix_fracture = coupled_flow_;

        GetAttributeValueS_(jnode, "state", "on");
        pk_model_["transport"].push_back(model);
        ...
        pk_domain_["mechanics"] = "domain";

        GetAttributeValueS_(jnode, "state", "on");

      } else if (strcmp(tagname, "strongly_coupled") == 0) {
        ...
        mpcs[pkname].mpc_type = "strongly";
        mpcs[pkname].matrix_fracture = coupled_flow_;

      } else if (strcmp(tagname, "weakly_coupled") == 0) {
        pkname = GetAttributeValueS_(jnode, "name");
    ...
    // -- single physics pks
    for (auto& mpc : mpcs) {
      auto& params = mpc.second;

      if (params.is_pk && !params.matrix_fracture) {
        Teuchos::ParameterList& aux = pk_tree_list.sublist(Keys::merge(mode, mpc.first, delimiter));
        aux.set<std::string>("PK type", params.submodel);
        params.basename = mpc.first;
        params.parsed = true;
      }

      else if (params.is_pk && params.matrix_fracture) {
        mpcname = mpc.first + " matrix fracture";
        if (params.submodel == "transport implicit") mpcname += " implicit";
        Teuchos::ParameterList& aux = pk_tree_list.sublist(Keys::merge(mode, mpcname, delimiter));
        aux.set<std::string>("PK type", mpcname);
        params.basename = mpcname;

        pkname = mpc.first + " matrix";
        aux.sublist(Keys::merge(mode, pkname, delimiter))
          .set<std::string>("PK type", *pk_model_[mpc.first].rbegin());

        pkname = mpc.first + " fracture";
        aux.sublist(Keys::merge(mode, pkname, delimiter))
          .set<std::string>("PK type", *pk_model_[mpc.first].rbegin());
        params.parsed = true;
      }
    ...
    while (flag) {
      ...
      for (auto& mpc : mpcs) {
        auto& params = mpc.second;
        if (params.parsed) continue;

        if (!mpcs[params.pks[0]].parsed || !mpcs[params.pks[1]].parsed) {
          flag = true;
          continue;
        }
...
InputConverterU::TranslatePKs_(Teuchos::ParameterList& glist)
{
  ...
  for (auto it = pks.crbegin(); it != pks.crend(); ++it) {
    ...
    if (out_list.isSublist(pk)) {
      ...
      bool is_mpc = out_list.sublist(pk).get<bool>("MPC type");
      ...
      if (!is_mpc) {
        out_list.sublist(pk) = TranslateSinglePhysicsPK_(prefix, domain, pkname, pk_model, glist);
      } else {
        if (pk_model == pkname + " matrix fracture") {
          pk_names.push_back(Keys::merge(prefix, pkname + " matrix", delimiter));
          pk_names.push_back(Keys::merge(prefix, pkname + " fracture", delimiter));
          out_list.sublist(pk).set<Teuchos::Array<std::string>>("PKs order", pk_names);
        } else {
          out_list.sublist(pk).set<std::string>("domain name", domain);
          pk_names = out_list.sublist(pk).get<Teuchos::Array<std::string>>("PKs order").toVector();
        }

        if (pkname == "transport" && transport_implicit_) {
      ...
      if (basename == "shallow water and transport") {
...
{
  if (pkname == "flow") {
    return TranslateFlow_(prefix, domain, pk_model);
  } else if (pkname == "energy") {
    return TranslateEnergy_(domain, pk_model);
  } else if (pkname == "transport") {
    return TranslateTransport_(domain);
