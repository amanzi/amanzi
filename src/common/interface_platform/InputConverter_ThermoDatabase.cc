...
InputConverter::TranslateThermodynamicDatabase_()
{
  ...
  bool flag;
  node =
    GetUniqueElementByTagsString_("geochemistry, thermodynamic_database, primary_species", flag);
  if (flag) {
  ...
  node = GetUniqueElementByTagsString_(
    "geochemistry, thermodynamic_database, aqueous_equilibrium_complexes", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
    ...
    for (int i = 0; i < children->getLength(); ++i) {
      ...
      out_list.sublist("aqueous equilibrium complexes")
        .sublist(name)
        .set<double>("ion size parameter", ion_size)
        .set<int>("charge", z)
        .set<double>("gram molecular weight", mol_weight)
  ...
  node =
    GetUniqueElementByTagsString_("geochemistry, thermodynamic_database, mineral_kinetics", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
  ...
  node =
    GetUniqueElementByTagsString_("geochemistry, thermodynamic_database, general_kinetics", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
    ...
    for (int i = 0; i < children->getLength(); ++i) {
      ...
      double brate =
        GetAttributeValueD_(knode, "backward_rate", TYPE_NUMERICAL, DVAL_MIN, DVAL_MAX);

      out_list.sublist("general kinetics")
        .sublist("general_" + std::to_string(i))
  ...
  node =
    GetUniqueElementByTagsString_("geochemistry, thermodynamic_database, ion_exchange_sites", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
  ...
  node = GetUniqueElementByTagsString_(
    "geochemistry, thermodynamic_database, ion_exchange_complexes", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
    children = element->getElementsByTagName(mm.transcode("complex"));
    ...
    for (int i = 0; i < children->getLength(); ++i) {
      ...
      out_list.sublist("ion exchange complexes")
        .sublist(name)
        .set<std::string>("reaction", reaction)
        .set<double>("equilibrium constant", lnKeq);
    }
  ...
  node = GetUniqueElementByTagsString_(
    "geochemistry, thermodynamic_database, surface_complex_sites", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
    children = element->getElementsByTagName(mm.transcode("site"));
    ...
    for (int i = 0; i < children->getLength(); ++i) {
      ...
      out_list.sublist("surface complex sites").sublist(name).set<double>("density", density);
    }
  }

  node =
    GetUniqueElementByTagsString_("geochemistry, thermodynamic_database, surface_complexes", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
    children = element->getElementsByTagName(mm.transcode("complex"));
    ...
    for (int i = 0; i < children->getLength(); ++i) {
      ...
      out_list.sublist("surface complexes")
        .sublist(name)
        .set<int>("charge", z)
        .set<std::string>("reaction", reaction)
        .set<double>("equilibrium constant", lnKeq);
  ...
  // radioactive decay
  node =
    GetUniqueElementByTagsString_("geochemistry, thermodynamic_database, radioactive_decay", flag);
  if (flag) {
    element = static_cast<DOMElement*>(node);
    ...
    for (int i = 0; i < children->getLength(); ++i) {
      ...
      out_list.sublist("radioactive decay")
        .sublist("complex_" + std::to_string(i))
