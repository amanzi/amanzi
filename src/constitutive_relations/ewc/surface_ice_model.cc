...
SurfaceIceModel::InitializeModel(const Teuchos::Ptr<State>& S) {
  ...
  Teuchos::RCP<Flow::Flow::IcyHeightEvaluator> icy_h_me =
      Teuchos::rcp_dynamic_cast<Flow::Flow::IcyHeightEvaluator>(me);
  ASSERT(icy_h_me != Teuchos::null);
  pd_ = icy_h_me->get_IcyModel();

  // -- unfrozen fraction evaluator
  me = S->GetFieldEvaluator("unfrozen_fraction");
  Teuchos::RCP<Flow::Flow::UnfrozenFractionEvaluator> uf_me =
      Teuchos::rcp_dynamic_cast<Flow::Flow::UnfrozenFractionEvaluator>(me);
