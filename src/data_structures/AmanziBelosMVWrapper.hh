...
public:
  // Constructors
  CompositeMultiVector(Teuchos::RCP<Vector> cmv) { cmv_.resize(1); cmv_[0] = cmv; }
  ...
  // Update methods
  void MvTimesMatAddMv(double alpha, const Belos::MultiVec<double> &A,
      const Teuchos::SerialDenseMatrix<int, double> &B,
      double beta);
  void MvAddMv(ScalarType alpha, const Belos::MultiVec<double>& A,
               ScalarType beta,  const Belos::MultiVec<double>& B);
  void MvScale(ScalarType alpha);
  void MvScale(const std::vector<ScalarType>& alpha);
  void MvTransMv(ScalarType alpha, const Belos::MultiVec<double>& A,
...
template<class Vector>
CompositeMultiVector<Vector>* CompositeMultiVector<Vector>::Clone(const int numvecs) const
{
  std::vector<Teuchos::RCP<Vector> > newcmv(numvecs);
  for(int i=0; i<numvecs; i++)
    newcmv[i] = Teuchos::rcp(new Vector(*cmv_[0]));
...
template<class Vector>
CompositeMultiVector<Vector>* CompositeMultiVector<Vector>::CloneCopy() const
{
  std::vector<Teuchos::RCP<Vector> > newcmv(cmv_.size());
  for(int i=0; i<cmv_.size(); i++)
    newcmv[i] = Teuchos::rcp(new Vector(*cmv_[i]));
  CompositeMultiVector* temp = new CompositeMultiVector(newcmv);
...
template<class Vector>
CompositeMultiVector<Vector>* CompositeMultiVector<Vector>::CloneCopy(const std::vector<int>& index) const
{
  std::vector<Teuchos::RCP<Vector> > newcmv(index.size());
  for(int i=0; i<index.size(); i++)
    newcmv[i] = Teuchos::rcp(new Vector(*cmv_[index[i]]));
  CompositeMultiVector* temp = new CompositeMultiVector(newcmv);
...
template<class Vector>
CompositeMultiVector<Vector>* CompositeMultiVector<Vector>::CloneViewNonConst(const std::vector<int> &index)
{
  std::vector<Teuchos::RCP<Vector> > newcmv(index.size());
  for(int i=0; i<index.size(); i++)
    newcmv[i] = cmv_[index[i]];
  CompositeMultiVector* temp = new CompositeMultiVector(newcmv);
...
template<class Vector>
const CompositeMultiVector<Vector>* CompositeMultiVector<Vector>::CloneView(const std::vector<int> &index) const
{
  std::vector<Teuchos::RCP<Vector> > newcmv(index.size());
  for(int i=0; i<index.size(); i++)
    newcmv[i] = cmv_[index[i]];
...
template<class Vector>
void CompositeMultiVector<Vector>::MvTimesMatAddMv(double alpha,
    const Belos::MultiVec<double> &A,
    const Teuchos::SerialDenseMatrix<int, double> &B,
    double beta)
{
  const CompositeMultiVector* cmvA = dynamic_cast<const CompositeMultiVector*>(&A);
  TEUCHOS_TEST_FOR_EXCEPTION(cmvA == 0,std::invalid_argument,
...
template<class Vector>
void CompositeMultiVector<Vector>::MvAddMv(double alpha,
    const Belos::MultiVec<double>& A,
    double beta,
    const Belos::MultiVec<double>& B)
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(cmvB == 0,std::invalid_argument,
...
template<class Vector>
void CompositeMultiVector<Vector>::MvTransMv(ScalarType alpha, const Belos::MultiVec<double>& A,
               Teuchos::SerialDenseMatrix<int, ScalarType>& B) const
{
  const CompositeMultiVector* cmvA = dynamic_cast<const CompositeMultiVector*>(&A);
  TEUCHOS_TEST_FOR_EXCEPTION(cmvA == 0,std::invalid_argument,
...
template<class Vector>
void CompositeMultiVector<Vector>::MvDot(const Belos::MultiVec<double>& A, std::vector<ScalarType> &b) const
{
  const CompositeMultiVector* cmvA = dynamic_cast<const CompositeMultiVector*>(&A);
  TEUCHOS_TEST_FOR_EXCEPTION(cmvA == 0,std::invalid_argument,
       "CompositeMultiVector::MvAddMv: A must be a CompositeMultiVector");
...
template<class Vector>
void CompositeMultiVector<Vector>::SetBlock (const Belos::MultiVec<double>& A, const std::vector<int> &index)
{
  const CompositeMultiVector* cmvA = dynamic_cast<const CompositeMultiVector*>(&A);
  TEUCHOS_TEST_FOR_EXCEPTION(cmvA == 0,std::invalid_argument,
  ...
  for(int i=0; i<index.size(); i++)
    *cmv_[index[i]] = *cmvA->cmv_[i];
}

template<class Vector>
void CompositeMultiVector<Vector>::MvRandom()
{
  for(int i=0; i<cmv_.size(); i++)
    cmv_[i]->Random();
}
...
template<class Vector>
void CompositeMultiVector<Vector>::MvPrint(std::ostream &os) const
{
  for(int i=0; i<cmv_.size(); i++)
    cmv_[i]->Print(os);
}
