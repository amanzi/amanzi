...
class CompositeMultiVector : public Belos::MultiVec<double> {
 ...
 public:
  ...
  void
  MvDot(const Belos::MultiVec<double>& A, std::vector<ScalarType>& b) const;

  // Norm method
  void MvNorm(std::vector<MagnitudeType>& normvec,
              Belos::NormType type = Belos::TwoNorm) const;

  // Initialization methods
  void
  SetBlock(const Belos::MultiVec<double>& A, const std::vector<int>& index);
  void MvRandom();
...
template <class Vector>
void
CompositeMultiVector<Vector>::MvTimesMatAddMv(
  double alpha, const Belos::MultiVec<double>& A,
  const Teuchos::SerialDenseMatrix<int, double>& B, double beta)
{
  const CompositeMultiVector* cmvA =
    dynamic_cast<const CompositeMultiVector*>(&A);
                                      ...
                                      const Belos::MultiVec<double>& B)
{
  ...
  const CompositeMultiVector* cmvB =
    dynamic_cast<const CompositeMultiVector*>(&B);
  TEUCHOS_TEST_FOR_EXCEPTION(
    cmvB == 0,
    std::invalid_argument,
...
template <class Vector>
void
CompositeMultiVector<Vector>::MvTransMv(
  ScalarType alpha, const Belos::MultiVec<double>& A,
  Teuchos::SerialDenseMatrix<int, ScalarType>& B) const
{
  const CompositeMultiVector* cmvA =
    dynamic_cast<const CompositeMultiVector*>(&A);
