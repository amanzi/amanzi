...
class BlockSpace {
 public:
  ...
  bool isSameAs(const BlockSpace& other) const;
  bool locallySameAs(const BlockSpace& other) const;
  bool isSubsetOf(const BlockSpace& other) const;
  ...
  BlockMap_ptr_type getComponentMap(const std::string& name, bool ghosted = false) const;
  Import_ptr_type getImporter(const std::string& name) const { return importers_.at(name); }
