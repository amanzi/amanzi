...
class CSR {
 ...
 public:
  ...
  CSR(const CSR<T2, D, MEMSPACE>& other) : row_map_(other.row_map_), sizes_(other.sizes_)
  {
  ...
  {
    row_map_.modify_host();
    row_map_.sync_device();
  }
  ...
  {
    if (row_map_.view_host().size() != row_map_size + 1) row_map_.realloc(row_map_size + 1);
    if (entries_.view_host().size() != entries_size) entries_.realloc(entries_size);
    if (sizes_.view_host().extent(0) != row_map_size) sizes_.realloc(row_map_size, D);
  }

  int size_host(int idx, int d) const { return sizes_.view_host()(idx, d); }
  int size_host(int i) const { return row_map_.view_host()(i + 1) - row_map_.view_host()(i); }
  /**
  ...
  {
    return Kokkos::subview(entries_.view_host(),
                           Kokkos::make_pair(row_map_.view_host()(i), row_map_.view_host()(i + 1)));
  }
  void set_row_map_host(const int& i, const int& v) { row_map_.view_host()[i] = v; }
  void set_entries_host(const int& i, const double& v) { entries_.view_host()[i] = v; }
  ...
  KOKKOS_INLINE_FUNCTION
  int size(int i) const { return row_map_.view_device()(i + 1) - row_map_.view_device()(i); }
  /**
   * @brief Return subview corresponding to element i
   * @param i index of the element in row_map_
   */
  ...
  Kokkos::View<T*, MEMSPACE> at(const int& i) const
  {
    return Kokkos::subview(
      ...
      Kokkos::make_pair(row_map_.view_device()(i), row_map_.view_device()(i + 1)));
  }

  KOKKOS_INLINE_FUNCTION
  void set_row_map(const int& i, const int& v) { row_map_.view_device()[i] = v; }

  KOKKOS_INLINE_FUNCTION
  void set_entries(const int& i, const double& v) { entries_.view_device()[i] = v; }

  KOKKOS_INLINE_FUNCTION
  void set_sizes(const int& i, const int& j, const int& v) { sizes_.view_device()(i, j) = v; }

  KOKKOS_INLINE_FUNCTION
  int row_map(const int& i) { return row_map_.view_device()[i]; }
  ...
  void assign(const CSR& csr)
  {
    ...
    sizes_.realloc(csr.sizes_.view_host().extent(0), csr.sizes_.view_host().extent(1));
    Kokkos::deep_copy(sizes_, csr.sizes_);
    // return *this;
  }

  void putScalar(T val) { Kokkos::deep_copy(entries_.view_device(), val); }
...
KOKKOS_INLINE_FUNCTION CT<DeviceOnlyMemorySpace>
getFromCSR(const CSR<double, 1, DeviceOnlyMemorySpace>& csr, const int& i)
{
  return std::move(CT<DeviceOnlyMemorySpace>(csr.at(i), csr.size(i, 0)));
}
template <template <class> class CT>
CT<Kokkos::HostSpace>
getFromCSR_host(const CSR<double, 1, DeviceOnlyMemorySpace>& csr, const int& i)
{
...
template <template <class> class CT>
KOKKOS_INLINE_FUNCTION CT<DeviceOnlyMemorySpace>
getFromCSR(const CSR<double, 2, DeviceOnlyMemorySpace>& csr, const int& i)
{
  return std::move(CT<DeviceOnlyMemorySpace>(csr.at(i), csr.size(i, 0), csr.size(i, 1)));
}
template <template <class> class CT>
CT<Kokkos::HostSpace>
getFromCSR_host(const CSR<double, 2, DeviceOnlyMemorySpace>& csr, const int& i)
{
...
template <template <class> class CT>
KOKKOS_INLINE_FUNCTION CT<DeviceOnlyMemorySpace>
getFromCSR(const CSR<double, 3, DeviceOnlyMemorySpace>& csr, const int& i)
{
  return std::move(
    CT<DeviceOnlyMemorySpace>(csr.at(i), csr.size(i, 0), csr.size(i, 1), csr.size(i, 2)));
}
template <template <class> class CT>
CT<Kokkos::HostSpace>
getFromCSR_host(const CSR<double, 3, DeviceOnlyMemorySpace>& csr, const int& i)
{
  return std::move(CT<Kokkos::HostSpace>(
    csr.at_host(i), csr.size_host(i, 0), csr.size_host(i, 1), csr.size_host(i, 2)));
}
