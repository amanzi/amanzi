...
{
  return std::make_pair(mesh.getMap(location, false), mesh.getMap(location, true));
}
...
CompositeSpace::CompositeSpace(const Teuchos::RCP<const AmanziMesh::Mesh>& mesh,
                               ...
                               bool ghosted)
  : BlockSpace(mesh->getComm(), names, master_maps, ghost_maps, num_vectors),
...
CompositeSpace::getLocation(const std::string& name) const
{
