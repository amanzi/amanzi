  ...
  if (includes_ghosted_) {
    ...
    offproc_row_map_ =
      Teuchos::rcp(new Map_type(Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid(),
                                ...
                                0,
                                ghosted_row_map_->getComm()));
    offproc_graph_ = Teuchos::rcp(new Graph_type(
      offproc_row_map_, col_map_, max_nnz_per_row.view(n_owned_, n_used_ - n_owned_)));
...
void
GraphFE::insertLocalIndices(LO row, std::size_t count, LO const * indices)
{
  if (row < n_owned_) {
...
void
GraphFE::insertGlobalIndices(GO row, std::size_t count, GO const * indices)
{
...
// fill graph using local indices
void
GraphFE::insertLocalIndices(std::size_t row_count, LO const * row_inds, std::size_t count, LO const * indices)
{
  for (std::size_t n = 0; n != row_count; ++n) insertLocalIndices(row_inds[n], count, indices);
}

// fill graph using global indices
void
GraphFE::insertGlobalIndices(std::size_t row_count, GO const * row_inds, std::size_t count, GO const * indices)
{
  for (std::size_t n = 0; n != row_count; ++n) insertGlobalIndices(row_inds[n], count, indices);
}
...
// finish fill
void
GraphFE::fillComplete(const Map_ptr_type& domain_map, const Map_ptr_type& range_map)
{
