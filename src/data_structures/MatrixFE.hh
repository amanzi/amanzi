...
class MatrixFE {
 public:
  ...
  {
    return offproc_matrix_.get() ? offproc_matrix_->getLocalMatrixDevice() :
                                   LocalMatrix_device_type();
  }
  ...
  // fill graph
  void sumIntoLocalValues(int row, int count, const double* values, const int* indices);

  // local matrix sum
  void sumIntoLocalValues(const int* row_inds,
                          const int* col_inds,
                          const Teuchos::SerialDenseMatrix<int, double>& vals);
  void sumIntoLocalValuesTransposed(const int* row_inds,
                                    const int* col_inds,
                                    const Teuchos::SerialDenseMatrix<int, double>& vals);

  void
  sumIntoLocalValues(const int* row_inds, const int* col_inds, const WhetStone::DenseMatrix<>& vals)
  {
  ...
  using cLocalIndices_host_view_type = Matrix_type::local_inds_host_view_type;
  using cValues_host_view_type = Matrix_type::values_host_view_type;
  void getLocalRowView(
    int row,
    cLocalIndices_host_view_type& indices,
    cValues_host_view_type& values)
    const;
