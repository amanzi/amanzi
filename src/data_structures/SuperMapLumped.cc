...
SuperMapLumped::SuperMapLumped(const Teuchos::RCP<const BlockSpace>& space)
    : comp_maps_(space),
      indices_(std::make_unique<BlockVector<LO> >(space))
{
  CreateIndexing_();
}
...
SuperMapLumped::CreateIndexing_()
{
  // fill the offsets
  {
    ...
    for (const auto& compname : *comp_maps_) {
      counts_[compname] = comp_maps_->ComponentMap(compname, false)->getNodeNumElements();
      ghosted_counts_[compname] = comp_maps_->ComponentMap(compname, true)->getNodeNumElements() - counts_[compname];
  ...
  for (const auto& compname : *comp_maps_) {
    ...
    auto index_view = indices_->ViewComponent<AmanziDefaultHost>(compname, false);
