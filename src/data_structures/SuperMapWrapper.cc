...
Teuchos::RCP<SuperMapWrapper> createSuperMap(const CompositeVectorSpace& cv) {
  return Teuchos::rcp(new SuperMapWrapper({cv}));
}


Teuchos::RCP<SuperMapWrapper> createSuperMap(const TreeVectorSpace& tvs) {
  if (tvs.Data() != Teuchos::null) {
    ...
    return Teuchos::rcp(new SuperMapWrapper(cvss));
  }
}


SuperMapWrapper::SuperMapWrapper(const std::vector<CompositeVectorSpace>& cvss)
{
  ...
  if (cvss.size() > 0) {
    smap_ = std::make_unique<SuperMap>(cvss[0].Comm(), names, dofnums, maps, ghost_maps);
  }
}
