...
TreeVector::TreeVector(std::string name, const TreeVector& other) : name_(name) {
  ...
  for (std::vector< Teuchos::RCP<TreeVector> >::const_iterator other_subvec =
    ...
    Teuchos::RCP<Amanzi::TreeVector> new_subvec
      = Teuchos::rcp(new TreeVector((*other_subvec)->name_, **other_subvec));
    subvecs_.push_back(new_subvec);
  }
};

TreeVector::TreeVector(const TreeVector& other) : name_(std::string("")) {
  data_ = Teuchos::rcp(new CompositeVector(*other.data_));
