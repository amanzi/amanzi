...
namespace Amanzi {
  ...
  getMultiVector (Impl::LocalAccess<const Tpetra::MultiVector<SC, LO, GO, NT>, MemorySpace, access_mode> LA)
  {
    ...
    Tpetra::MultiVector<SC, LO, GO, NT>& G = const_cast<Tpetra::MultiVector<SC, LO, GO, NT>& > (LA.G_);
    Impl::LocalAccess<Tpetra::MultiVector<SC, LO, GO, NT>, MemorySpace, access_mode> LA2 (G, LA.space_, LA.valid_);
  ...
  template<class SC, class LO, class GO, class NT,
           ...
           const AccessMode access_mode>
  auto
  getVector (Impl::LocalAccess<const Tpetra::MultiVector<SC, LO, GO, NT>, MemorySpace, access_mode> LA,
             const int whichColumn = 0)
    -> decltype (Kokkos::subview (getMultiVector (LA), Kokkos::ALL (), whichColumn))
  {
    using nc_global_object_type = Tpetra::MultiVector<SC, LO, GO, NT>;
    nc_global_object_type& G = const_cast<nc_global_object_type&> (LA.G_);
    Impl::LocalAccess<nc_global_object_type, MemorySpace, access_mode> LA2 (G, LA.space_, LA.valid_);
  ...
  template<class SC, class LO, class GO, class NT,
           ...
           const AccessMode access_mode>
  auto
  getVector (Impl::LocalAccess<const Tpetra::Vector<SC, LO, GO, NT>, MemorySpace, access_mode> LA)
    -> decltype (getVector (Impl::LocalAccess<Tpetra::Vector<SC, LO, GO, NT>, MemorySpace, access_mode> (const_cast<Tpetra::Vector<SC, LO, GO, NT>& > (LA.G_), LA.space_,  LA.valid_)))
  {
    return getVector (Impl::LocalAccess<Tpetra::Vector<SC, LO, GO, NT>, MemorySpace, access_mode> (const_cast<Tpetra::Vector<SC, LO, GO, NT>& > (LA.G_), LA.space_,  LA.valid_));
  }
  ...
  namespace Impl {
    ...
    template<class SC, class LO, class GO, class NT,
             class MemorySpace,
             const AccessMode am>
    struct GetMasterLocalObject<LocalAccess<const Tpetra::MultiVector<SC, LO, GO, NT>, MemorySpace, am> > {
    public:
      using local_access_type = LocalAccess<const Tpetra::MultiVector<SC, LO, GO, NT>, MemorySpace, am>;

    private:
      using global_object_type = typename std::remove_const<typename local_access_type::global_object_type>::type;
      using memory_space = typename local_access_type::memory_space;
      static constexpr AccessMode access_mode = local_access_type::access_mode;
      using non_const_value_type = typename Tpetra::MultiVector<SC, LO, GO, NT>::impl_scalar_type;
      using value_type = typename std::conditional<
    ...
    public:
      ...
      static master_local_object_type get (local_access_type LA) {
        ...
        if (access_mode != ReadWrite) {
          G.template modify<memory_space> ();
        }
        // FIXME (mfh 22 Oct 2018) This might break if we need copy-back
        // semantics, e.g., for a memory space for which the
        // Tpetra::MultiVector does not store data.  In that case, we
