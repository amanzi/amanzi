...
TEST(SUPERMAP_BLOCKMAP_NULL_PROC)
{
  ...
  if (getSize > 1) {
    ...
    auto owned_map1 =
      Teuchos::rcp(new Epetra_BlockMap(3 * (getSize - 1), n_cells, &gids[0], &size[0], 0, *comm));
    if (getRank != 1) {
      CHECK_EQUAL(0, owned_map1->FirstPointInElement(0));
      CHECK_EQUAL(1, owned_map1->FirstPointInElement(1));
      CHECK_EQUAL(3, owned_map1->FirstPointInElement(2));
    ...
    auto ghosted_map1 = Teuchos::rcp(new Epetra_BlockMap(
      3 * (getSize - 1) + 2 * (getSize - 2), n_cells_g, &gids[0], &size[0], 0, *comm));
...
TEST(SUPERMAP_BLOCK_MANUAL)
{
  ...
  auto owned_map2 =
    Teuchos::rcp(new Epetra_BlockMap(5 * getSize, 5, &gids2[0], &size2[0], 0, *comm));
