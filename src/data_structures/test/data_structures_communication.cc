...
TEST(COMMUNICATION_INTERMEDIATE)
{
  ...
  auto ghost = Teuchos::rcp(new MultiVector_type(mesh->getMap(AmanziMesh::Entity_kind::FACE,true), 1));
  auto owned = ghost->offsetViewNonConst(mesh->getMap(AmanziMesh::Entity_kind::FACE,false), 0);
  auto vand = Teuchos::rcp(new MultiVector_type(mesh->getMap(AmanziMesh::Entity_kind::BOUNDARY_FACE,false), 1));

  Import_type importer(mesh->getMap(AmanziMesh::Entity_kind::FACE,false), mesh->getMap(AmanziMesh::Entity_kind::FACE,true));
  owned->putScalar((double)(rank + 1));
  ghost->doImport(*owned, importer, Tpetra::INSERT);
  ...
  // check the vandelay map
  Import_type vimporter(mesh->getMap(AmanziMesh::Entity_kind::FACE,false), mesh->getMap(AmanziMesh::Entity_kind::BOUNDARY_FACE,false));
...
TEST(IMPORTERS_BOUNDARY_FACES)
{
  ...
  IntVector_type vec(boundary_face_map);
  {
    ...
    stream << "Rank " << rank << ": len=" << import_view.extent(0) << "," << import_view.extent(1)
           << ": ";
    for (int i = 0; i != importer.getNumSameIDs(); ++i) stream << i << "-->" << i << ", ";
