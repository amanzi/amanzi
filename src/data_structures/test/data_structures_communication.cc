...
TEST(COMMUNICATION_INTERMEDIATE)
{
  ...
  auto ghost =
    Teuchos::rcp(new MultiVector_type(mesh->getMap(AmanziMesh::Entity_kind::FACE, true), 1));
  auto owned = ghost->offsetViewNonConst(mesh->getMap(AmanziMesh::Entity_kind::FACE, false), 0);
  auto vand = Teuchos::rcp(
    new MultiVector_type(mesh->getMap(AmanziMesh::Entity_kind::BOUNDARY_FACE, false), 1));

  Import_type importer(mesh->getMap(AmanziMesh::Entity_kind::FACE, false),
  ...
  // check the vandelay map
  Import_type vimporter(mesh->getMap(AmanziMesh::Entity_kind::FACE, false),
