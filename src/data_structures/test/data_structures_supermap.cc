...
void
CHECK_OWNED_SUBSET_GHOST(cVectorView_type_<MirrorHost, int> owned,
                         int n_owned,
                         cVectorView_type_<MirrorHost, int> ghost,
                         int n_ghost)
{
...
void
CHECK_UNIQUE(std::vector<cVectorView_type_<MirrorHost, int>> index_lists,
             int total)
{
  std::set<int> all;
...
void
CHECK_SAME(cVectorView_type_<MirrorHost, int> one,
           cVectorView_type_<MirrorHost, int> two)
{
...
TEST(SUPERMAP_MANUAL)
{
  ...
  // check the indices
  {
    ...
    auto inds_m2_d1 = map.Indices<MirrorHost>("map2", 1);
  ...
  {
    auto inds_m1_d0 = map.GhostIndices<MirrorHost>("map1", 0);
    CHECK_EQUAL(0, inds_m1_d0[0]);
    CHECK_EQUAL(2, inds_m1_d0[1]);
    CHECK_EQUAL(4, inds_m1_d0[2]);

    auto inds_m1_d1 = map.GhostIndices<MirrorHost>("map1", 1);
    CHECK_EQUAL(1, inds_m1_d1[0]);
    CHECK_EQUAL(3, inds_m1_d1[1]);
    CHECK_EQUAL(5, inds_m1_d1[2]);

    auto inds_m2_d0 = map.GhostIndices<MirrorHost>("map2", 0);
    CHECK_EQUAL(6, inds_m2_d0[0]);
    CHECK_EQUAL(8, inds_m2_d0[1]);
    CHECK_EQUAL(10, inds_m2_d0[2]);
    CHECK_EQUAL(12, inds_m2_d0[3]);
    ...
    auto inds_m2_d1 = map.GhostIndices<MirrorHost>("map2", 1);
    CHECK_EQUAL(7, inds_m2_d1[0]);
    CHECK_EQUAL(9, inds_m2_d1[1]);
    CHECK_EQUAL(11, inds_m2_d1[2]);
    CHECK_EQUAL(13, inds_m2_d1[3]);
...
TEST(SUPERMAP_FROM_SINGLE_COMPOSITEVECTOR)
{
  ...
  const auto& inds_c0 = map->GhostIndices<MirrorHost>(0, "cell", 0);
  const auto& inds_c1 = map->GhostIndices<MirrorHost>(0, "cell", 1);
  const auto& inds_f0 = map->GhostIndices<MirrorHost>(0, "face", 0);
  const auto& inds_f1 = map->GhostIndices<MirrorHost>(0, "face", 1);

  const auto& inds_c0_owned = map->Indices<MirrorHost>(0, "cell", 0);
  const auto& inds_c1_owned = map->Indices<MirrorHost>(0, "cell", 1);
  const auto& inds_f0_owned = map->Indices<MirrorHost>(0, "face", 0);
  const auto& inds_f1_owned = map->Indices<MirrorHost>(0, "face", 1);
  ...
  CHECK_UNIQUE(
    std::vector<cVectorView_type_<MirrorHost, int>>{
...
TEST(SUPERMAP_FROM_SINGLE_COMPOSITEVECTOR_REPEATED_MAPS)
{
  ...
  // check ordering is as expected
  const auto& inds_c0 = map->GhostIndices<MirrorHost>(0, "cellA", 0);
  const auto& inds_c1 = map->GhostIndices<MirrorHost>(0, "cellB", 0);

  const auto& inds_c0_owned = map->Indices<MirrorHost>(0, "cellA", 0);
  const auto& inds_c1_owned = map->Indices<MirrorHost>(0, "cellB", 0);
  ...
  // check lists are non-overlapping and of sufficient size to cover all values
  CHECK_UNIQUE(
    std::vector<cVectorView_type_<MirrorHost, int>>{ inds_c0_owned,
                                                            inds_c1_owned },
    2 * ncells_owned);
  CHECK_UNIQUE(
    std::vector<cVectorView_type_<MirrorHost, int>>{ inds_c0, inds_c1 },
    2 * ncells_used);
...
TEST(SUPERMAP_FROM_TWO_IDENTICAL_COMPOSITEVECTORS)
{
  ...
  // same indices!
  CHECK_SAME(map->Indices<MirrorHost>(0, "cell", 0),
             map2->Indices<MirrorHost>(0, "cell", 0));
  CHECK_SAME(map->GhostIndices<MirrorHost>(0, "cell", 0),
             map2->GhostIndices<MirrorHost>(0, "cell", 0));
  CHECK_SAME(map->Indices<MirrorHost>(1, "cell", 0),
             map2->Indices<MirrorHost>(0, "cell", 1));
  CHECK_SAME(map->GhostIndices<MirrorHost>(1, "cell", 0),
             map2->GhostIndices<MirrorHost>(0, "cell", 1));
}
...
TEST(SUPERMAP_FROM_CELL_PLUS_FACE_IS_CELLFACE)
{
  ...
  CHECK_SAME(map->GhostIndices<MirrorHost>(0, "cell", 0),
             map2->GhostIndices<MirrorHost>(0, "cell", 0));
  CHECK_SAME(map->Indices<MirrorHost>(1, "face", 0),
             map2->Indices<MirrorHost>(0, "face", 0));
  CHECK_SAME(map->GhostIndices<MirrorHost>(1, "face", 0),
             map2->GhostIndices<MirrorHost>(0, "face", 0));
}
...
//   // not the same map!
//   CHECK(map->Indices<MirrorHost>(0, "cell", 0).size() == ncells);
//   CHECK(map->Indices<MirrorHost>(1, "cell", 0).size() ==
//   block_cell_map->NumMyPoints()); CHECK_EQUAL(1 + ncells,
//   block_cell_map->NumMyPoints());

//   const auto& inds1 = map->Indices<MirrorHost>(0, "cell", 0);
//   const auto& inds2 = map->Indices<MirrorHost>(1, "cell", 0);
...
//   CHECK_UNIQUE(std::vector<cVectorView_type_<MirrorHost,int> >{ inds1,
//   inds2 }, 2*ncells + 1);
