...
void CHECK_OWNED_SUBSET_GHOST(cVectorView_type_<AmanziDefaultHost,int> owned,
        int n_owned, cVectorView_type_<AmanziDefaultHost,int> ghost, int n_ghost) {
...
void CHECK_UNIQUE(std::vector<cVectorView_type_<AmanziDefaultHost,int> > index_lists, int total) {
  std::set<int> all;
  int expected_count = 0;
  for (auto index_list : index_lists) {
    expected_count += index_list.size();
    for (std::size_t i=0; i!=index_list.extent(0); ++i) all.emplace(index_list(i));
  }
  CHECK_EQUAL(expected_count, all.size());
  CHECK_EQUAL(total, expected_count);
}

void CHECK_SAME(cVectorView_type_<AmanziDefaultHost,int> one, cVectorView_type_<AmanziDefaultHost,int> two) {
...
TEST(SUPERMAP_MANUAL) {
  ...
  std::map<std::string, std::size_t> dofnums; dofnums["map1"] = 2; dofnums["map2"] = 2;
  std::map<std::string,Teuchos::RCP<const BlockMap_type> > maps;
  maps["map1"] = owned_map1; maps["map2"] = owned_map2;

  std::map<std::string,Teuchos::RCP<const BlockMap_type> > gmaps;
  gmaps["map1"] = ghosted_map1; gmaps["map2"] = ghosted_map2;

  Operators::SuperMapLumped map(Teuchos::rcp(new BlockSpace(comm, names, maps, gmaps, dofnums)));
...
TEST(SUPERMAP_FROM_SINGLE_COMPOSITEVECTOR) {
  ...
  Teuchos::RCP<Operators::SuperMap> map = createSuperMap(cv.CreateSpace().ptr());
  ...
  // check basic sizes
  CHECK_EQUAL(2*ncells_owned + 2*nfaces_owned, map->Map()->getNodeNumElements());
  CHECK_EQUAL(2*ncells_used + 2*nfaces_used, map->GhostedMap()->getNodeNumElements());
  ...
  CHECK_UNIQUE(std::vector<cVectorView_type_<AmanziDefaultHost,int> >{ inds_c0_owned, inds_c1_owned, inds_f0_owned, inds_f1_owned }, 2*ncells_owned + 2*nfaces_owned);
  CHECK_UNIQUE(std::vector<cVectorView_type_<AmanziDefaultHost,int> >{ inds_c0, inds_c1, inds_f0, inds_f1 }, 2*ncells_used + 2*nfaces_used);
...
TEST(SUPERMAP_FROM_SINGLE_COMPOSITEVECTOR_REPEATED_MAPS) {
  ...
  // check lists are non-overlapping and of sufficient size to cover all values
  CHECK_UNIQUE(std::vector<cVectorView_type_<AmanziDefaultHost,int> >{ inds_c0_owned, inds_c1_owned }, 2*ncells_owned);
  CHECK_UNIQUE(std::vector<cVectorView_type_<AmanziDefaultHost,int> >{ inds_c0, inds_c1 }, 2*ncells_used);
  ...
  // create a SuperMapLumped from this space
  Teuchos::RCP<Operators::SuperMap> map2 = createSuperMap(cv2.CreateSpace().ptr());
...
TEST(SUPERMAP_FROM_TWO_IDENTICAL_COMPOSITEVECTORS) {
  ...
  // create a SuperMapLumped from this space
  Teuchos::RCP<Operators::SuperMap> map = Teuchos::rcp(new SuperMap(comm, {cvA.CreateSpace().ptr(), cvB.CreateSpace().ptr()}));
  ...
  // same indices!
  CHECK_SAME(map->Indices<AmanziDefaultHost>(0, "cell", 0), map2->Indices<AmanziDefaultHost>(0, "cell", 0));
  CHECK_SAME(map->GhostIndices<AmanziDefaultHost>(0, "cell", 0), map2->GhostIndices<AmanziDefaultHost>(0, "cell", 0));
  CHECK_SAME(map->Indices<AmanziDefaultHost>(1, "cell", 0), map2->Indices<AmanziDefaultHost>(0, "cell", 1));
  CHECK_SAME(map->GhostIndices<AmanziDefaultHost>(1, "cell", 0), map2->GhostIndices<AmanziDefaultHost>(0, "cell", 1));
}  
...
TEST(SUPERMAP_FROM_CELL_PLUS_FACE_IS_CELLFACE) {
  ...
  // create a SuperMapLumped from this space
  Teuchos::RCP<Operators::SuperMap> map = Teuchos::rcp(new SuperMap(comm, {cvA.CreateSpace().ptr(), cvB.CreateSpace().ptr()}));
  Teuchos::RCP<Operators::SuperMap> map2 = Teuchos::rcp(new SuperMap(comm, {cv2.CreateSpace().ptr()}));
  ...
  CHECK_SAME(map->Indices<AmanziDefaultHost>(0, "cell", 0), map2->Indices<AmanziDefaultHost>(0, "cell", 0));
  CHECK_SAME(map->GhostIndices<AmanziDefaultHost>(0, "cell", 0), map2->GhostIndices<AmanziDefaultHost>(0, "cell", 0));
  CHECK_SAME(map->Indices<AmanziDefaultHost>(1, "face", 0), map2->Indices<AmanziDefaultHost>(0, "face", 0));
  CHECK_SAME(map->GhostIndices<AmanziDefaultHost>(1, "face", 0), map2->GhostIndices<AmanziDefaultHost>(0, "face", 0));
}  
...
TEST(SUPERMAP_FROM_SAME_NAME_DIFFERENT_MAP) {
  ...
  CHECK_SAME(map->Indices<AmanziDefaultHost>(0, "cell", 0), map2->Indices<AmanziDefaultHost>(0, "cell", 0));
  CHECK_SAME(map->GhostIndices<AmanziDefaultHost>(0, "cell", 0), map2->GhostIndices<AmanziDefaultHost>(0, "cell", 0));
  CHECK_SAME(map->Indices<AmanziDefaultHost>(1, "cell", 0), map2->Indices<AmanziDefaultHost>(0, "face", 0));
  CHECK_SAME(map->GhostIndices<AmanziDefaultHost>(1, "cell", 0), map2->GhostIndices<AmanziDefaultHost>(0, "face", 0));
}  
...
//   Teuchos::RCP<const Epetra_BlockMap> block_cell_map = Teuchos::rcp(new Epetra_BlockMap(cell_map.NumGlobalElements(), cell_map.getNodeNumElements(), gids, &element_size[0], 0, comm));
...
//   Teuchos::RCP<const Epetra_BlockMap> block_cell_map_g = Teuchos::rcp(new Epetra_BlockMap(cell_mapg.NumGlobalElements(), cell_mapg.getNodeNumElements(), gids, &element_sizeg[0], 0, comm));
...
//   CHECK(map->Indices<AmanziDefaultHost>(1, "cell", 0).size() == block_cell_map->NumMyPoints());
//   CHECK_EQUAL(1 + ncells, block_cell_map->NumMyPoints());
...
//   CHECK_UNIQUE(std::vector<cVectorView_type_<AmanziDefaultHost,int> >{ inds1, inds2 }, 2*ncells + 1);
