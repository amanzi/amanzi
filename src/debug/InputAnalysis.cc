...
void InputAnalysis::RegionAnalysis() 
{
  ...
  if (alist.isParameter("used source regions")) {
    ...
    for (int i = 0; i < regions.size(); i++) {
      ...
      block = mesh_->getSetEntitiesAndVolumeFractions(regions[i], AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_type::OWNED, &vofs);
  ...
  if (alist.isParameter("used boundary condition regions")) {
    ...
    for (int i = 0; i < regions.size(); i++) {
      AmanziMesh::Entity_ID_List block;
      std::vector<double> vofs;
      block = mesh_->getSetEntitiesAndVolumeFractions(regions[i], AmanziMesh::Entity_kind::FACE, AmanziMesh::Parallel_type::OWNED, &vofs);
      int nblock = block.size();
      int nvofs = vofs.size();
      ...
      for (int n = 0; n < nblock; ++n) {
        mesh_->getFaceCells(block[n], AmanziMesh::Parallel_type::ALL, cells);
  ...
  if (alist.isParameter("used observation regions")) {
    ...
    for (int i = 0; i < regions.size(); i++) {
      ...
      try {
        ...
        type = "cells";
        for (int n = 0; n < nblock; n++) 
          volume += mesh_->getCellVolume(block[n]);
      ...
      if (nblock == 0) {
        ...
        type = "faces";
        for (int n = 0; n < nblock; n++) 
            volume += mesh_->getFaceArea(block[n]);
