...
plantMesh(const Teuchos::RCP<Epetra_MpiComm>& comm,
  ...
  double n_cap = 100; // number of capillary tubes
  ...
  AmanziMesh::Entity_ID_List stem_to_leaf(2);
  ...
  AmanziMesh::Entity_ID_List stem_to_troot(2);
  ...
  for (int i=0; i!=n_root; ++i) {
    // troot to aroot
    AmanziMesh::Entity_ID_List troot_to_aroot(2);
    ...
    if (include_soil) {
      // aroot to soil
      AmanziMesh::Entity_ID_List aroot_to_soil(2);
