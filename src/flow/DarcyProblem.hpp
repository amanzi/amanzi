...
public:
  ...
  DarcyProblem(Teuchos::RCP<Mesh_maps_base> &mesh, Teuchos::RCP<FlowBC> &bc);
  ~DarcyProblem();
  ...
  void SetPermeability(std::vector<double> &k);
  ...
  Epetra_Vector* CreateCellView(const Epetra_Vector&);
  Epetra_Vector* CreateFaceView(const Epetra_Vector&);
...
private:  // Auxillary functions
  ...
  DiffusionMatrix* create_diff_matrix_(Teuchos::RCP<Mesh_maps_base>&, Teuchos::RCP<FlowBC>&) const;
