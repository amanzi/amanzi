...
FunctionBilinear::FunctionBilinear(const std::vector<double>& x,
...
void
FunctionBilinear::check_args(const std::vector<double>& x,
                             const std::vector<double>& y,
...
double
FunctionBilinear::operator()(const std::vector<double>& x) const
{
  double v;
  int nx = x_.size();
  int ny = y_.size();
  ...
  if (xv <= x_[0] && yv <= y_[0]) {
    ...
    if (xv <= x_[0] && yv > y_[0] && yv < y_[ny - 1]) {
      ...
      v = v_[j1][k1] * (x_[j2] - xv) * (y_[k2] - yv) + v_[j2][k1] * (xv - x_[j1]) * (y_[k2] - yv) +
