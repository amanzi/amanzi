...
SUITE(malformed_parameter_list)
{
  TEST(no_function_sublist)
  {
    ...
    CHECK_THROW(auto f = fact.Create(list), Errors::Message);
  }
...
SUITE(tabular_factory)
{
  TEST(create)
  {
    ...
    FunctionFactory fact;
    auto f = fact.Create(list);
    Kokkos::View<double**, Kokkos::HostSpace> t("t", 1, 1);
    t(0,0) = 0.5;
    CHECK_EQUAL((*f)(t), 2.5);
  }
...
SUITE(bilinear_factory)
{
  TEST(create)
  {
    ...
    FunctionFactory fact;
    auto f = fact.Create(list);
    Kokkos::View<double**, Kokkos::HostSpace> t("t", 2, 1);
    t(0,0) = 2.;
    t(1,0) = 2.;
    CHECK_EQUAL((*f)(t), 14.);
  }
...
SUITE(separable_factory)
{
  TEST(create)
  {
    ...
    auto f = factory.Create(list);
    Kokkos::View<double**, Kokkos::HostSpace> x("x", 3, 1);
    x(0,0) = 0.;
    x(1,0) = 1.;
    x(2,0) = -1.;
    CHECK_EQUAL(-2.0, (*f)(x));
  ...
  TEST(missing_sublist)
  {
    ...
    CHECK_THROW(auto f = factory.Create(list), Errors::Message);
...
SUITE(static_head_factory)
{
  TEST(create)
  {
    ...
    FunctionFactory factory;
    auto f = factory.Create(list);
    Kokkos::View<double**, Kokkos::HostSpace> x("x", 4, 1);
    x(0,0) = 0.;
    x(1,0) = 0.;
    x(2,0) = 0.;
    x(3,0) = 1.;
    CHECK_EQUAL(5.0, (*f)(x));
    Kokkos::View<double**, Kokkos::HostSpace> y("y", 4, 1);
    y(0,0) = 1.;
    y(1,0) = 1.;
    y(2,0) = 1.;
    y(3,0) = 4.;
...
SUITE(distance_factory)
{
  TEST(create)
  {
    ...
    metric[0] = 1.0;
    metric[1] = 1.0;
    sublist.set<Teuchos::Array<double>>("x0", x0).set<Teuchos::Array<double>>("metric", metric);
    FunctionFactory fact;
    auto f = fact.Create(list);
    Kokkos::View<double**, Kokkos::HostSpace> x("x", 2, 1);
    x(0,0) = 2.;
