...
// Constructor
// -------------------------------------------------------------------
RegionLineSegment::RegionLineSegment(const std::string& name, const int id,
    ...
    p1_(p1)
{
  ...
  double eps = 1e-15;
  if (norm(p0_ - p1_) < eps) {
    msg << " Zero length line segment \"" << Region::name()
        << "\" is NOT allowed.";
    Exceptions::amanzi_throw(msg);
  }
}
                             ...
                             const std::vector<std::vector<int>>& faces) const
{
  ...
  if ((mdim == 3) && (sdim == 3)) {
    ...
    if (num_int == 0) {
      ...
      Errors::Message mesg("More than two intersection points in "
                           "RegionLineSegment intersect function");
      Exceptions::amanzi_throw(mesg);
    }
  }
...
void
RegionLineSegment::ComputeInterLinePoints(
  const std::vector<Point>& polytope,
  const std::vector<std::vector<int>>& faces, Point& res_point) const
{
  int mdim, sdim;
  double eps = 1e-12;
