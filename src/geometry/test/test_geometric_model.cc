...
TEST(GEOMETRIC_MODEL)
{
  ...
  for (Teuchos::ParameterList::ConstIterator i = reg_spec.begin();
    ...
    if (shape == "region: plane") {
      ...
      Teuchos::ParameterList plane_params =
        reg_params.sublist(reg_params.name(j));
      ...
      Teuchos::RCP<const Amanzi::AmanziGeometry::RegionPlane> plane =
        Teuchos::rcp_dynamic_cast<const Amanzi::AmanziGeometry::RegionPlane>(
          reg);
      ...
      if (p.dim() == 3) CHECK_EQUAL(p.z(), in_xyz[2]);
      ...
      if (p.dim() == 3) CHECK_EQUAL(n.z(), in_nrm[2] / len);
      ...
      Teuchos::ParameterList box_params =
        reg_params.sublist(reg_params.name(j));
      ...
      Teuchos::ParameterList labset_params =
        reg_params.sublist(reg_params.name(j));
      ...
      Teuchos::RCP<const Amanzi::AmanziGeometry::RegionLabeledSet> lsreg =
        Teuchos::rcp_dynamic_cast<
          const Amanzi::AmanziGeometry::RegionLabeledSet>(reg);
