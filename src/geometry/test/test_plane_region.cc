...
TEST(PLANE_REGION)
{
  ...
  //   Teuchos::ParameterList expert_params = plane_params.sublist("expert
  //   parameters"); tolerance = expert_params.get<double>("tolerance");
  // }
  ...
  double len =
    sqrt(in_nrm[0] * in_nrm[0] + in_nrm[1] * in_nrm[1] + in_nrm[2] * in_nrm[2]);
  ...
  p2.set(p.x() + 0.1, p.y() + 0.2, p.z() + 0.3); // create a point p2 that will
                                                 // probably be off the plane
