                                          ...
                                          const Epetra_Map& map_use) const
{
  ...
  if (this->comm_->NumProc() == 1) {
    ...
    if (mesh_->getSetSize(set_name, kind, Parallel_kind::ALL) != set_own.size()) {
...
template <class Mesh_type>
int
MeshAudit_Base<Mesh_type>::isSameFace_(const cEntity_ID_View fnode1,
                                       const cEntity_ID_View fnode2) const
{
  int nn = fnode1.size();
