...
MeshAudit_Geometry<Mesh_type>::check_cell_to_faces_to_nodes() const
{
  ...
  for (Entity_ID j = 0; j < ncells_all_; ++j) {
    ...
    // to check against
    if (ctype_enum == Cell_type::UNKNOWN || ctype_enum == Cell_type::POLYGON ||
  ...
  for (Entity_ID j = nface_own; j < nface_use; ++j) {
    ...
    if (bad_data) {
      ...
      case 0: // completely bad -- different face
        ...
        for (int k = 0; k < fnode.size(); ++k) os_ << fnode[k] << " ";
        os_ << std::endl;
...
MeshAudit_Maps<Mesh_type>::check_cell_to_nodes_ghost_data() const
{
  ...
  if (!bad_cells.empty()) {
    ...
    writeList_(bad_cells);
    os_ << "       The ghost cells don't have the same nodes as their respective masters."
        << std::endl;
    error = true;
  }
                                          ...
                                          const Epetra_Map& map_use) const
{
  ...
  if (this->comm_->NumProc() == 1) {
    ...
    if (mesh_->get_set_size(set_name, kind, Parallel_type::ALL) != set_own.size()) {
...
template <class Mesh_type>
bool
MeshAudit_Base<Mesh_type>::areDistinctValues_(const Entity_ID_List& list) const
{
...
template <class Mesh_type>
int
MeshAudit_Base<Mesh_type>::isSameFace_(const Entity_ID_List fnode1,
                                       const Entity_ID_List fnode2) const
{
  int nn = fnode1.size();
...
template <class Mesh_type>
void
MeshAudit_Base<Mesh_type>::writeList_(const std::vector<Entity_ID>& list) const
{
