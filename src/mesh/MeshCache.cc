    ...
    if (mesh_->space_dimension() == 2) { // 2D mesh
      ...
      *centroid = 0.5 * (fcoords[0] + fcoords[1]);
      ...
      for (int i = 0; i < cellids.extent(0); i++) {
        ...
        cell_get_faces_and_dirs<Kokkos::HostSpace>(cellids(i), cellfaceids, cellfacedirs);
...
MeshCache::build_columns(const std::string& setname) const
{
  ...
  for (int i = 0; i < ncolumns; i++) {
    ...
    Kokkos::View<Entity_ID*,Kokkos::HostSpace> fcells;
    face_get_cells_host(f, Parallel_type::ALL, fcells);
    ...
    AmanziGeometry::Point normal = face_normal_host(f, false, fcells(0));
    normal /= norm(normal);
    if (normal[2] < 1.e-10) {
      std::cerr << "Mesh: Provided set for build_columns() includes faces that "
                   "don't point upward.\n";
...
MeshCache::build_columns() const
{
  ...
  for (int i = 0; i < nf; i++) {
    Kokkos::View<Entity_ID*,Kokkos::HostSpace> fcells;
    face_get_cells_host(i, Parallel_type::ALL, fcells);

    // Is it a boundary face?
    if (fcells.extent(0) != 1) continue;

    // Is it pointing up?
    AmanziGeometry::Point normal = face_normal_host(i, false, fcells(0));
    normal /= norm(normal);
...
{
  Kokkos::View<Entity_ID*,Kokkos::HostSpace> fcells;
  face_get_cells_host(top_face, Parallel_type::ALL, fcells);
  ...
  Kokkos::View<Entity_ID*,Kokkos::HostSpace> fcells2;
  ...
  while (!done) {
    ...
    for (int j = 0; j < cfaces.extent(0); j++) {
      AmanziGeometry::Point normal = face_normal_host(cfaces(j));
    ...
    face_get_cells_host(bot_face, Parallel_type::ALL, fcells2);
    ...
    double even_odd_product = face_normal_host(bot_face)[mesh_->space_dimension() - 1] *
                              face_normal_host(top_face)[mesh_->space_dimension() - 1];
