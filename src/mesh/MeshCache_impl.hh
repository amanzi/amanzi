    ...
    if (getSpaceDimension() == getManifoldDimension()) {
      ...
      if (c < 0) {
        ...
        if (fcells.size() != 2) {
          ...
          int pos_i = getEntityGID(Entity_kind::CELL, fcells[0]) >
                          getEntityGID(Entity_kind::CELL, fcells[1]) ?
                        0 :
                        1;
          normal =
            (data_.face_normals.get<MEM>(f, 1 - pos_i) - data_.face_normals.get<MEM>(f, pos_i)) / 2;
        }
      } else if (orientation) {
        *orientation = MeshAlgorithms::getFaceDirectionInCell(*this, f, cc);
      }
...
MeshCache<MEM>::cacheEdgeGeometry()
{
  ...
  Kokkos::deep_copy(view<MemSpace_kind::DEVICE>(data_.edge_centroids),
                    view<MemSpace_kind::HOST>(data_.edge_centroids));
  Kokkos::deep_copy(view<MemSpace_kind::DEVICE>(data_.edge_vectors),
                    view<MemSpace_kind::HOST>(data_.edge_vectors));
