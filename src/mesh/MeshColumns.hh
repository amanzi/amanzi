...
struct MeshColumns {
 public:
  ...
  // Note these MUST be constructed on HOST caches.
  //
  void initialize(const MeshCache<MemSpace_kind::HOST>& mesh, const std::vector<std::string>& regions);
 ...
 private:
  void buildColumn_(const MeshCache<MemSpace_kind::HOST>& mesh, const Entity_ID f, int col);
...
//
Entity_ID findOpposingCell(const MeshCache<MemSpace_kind::HOST>& mesh, const Entity_ID c, const Entity_ID f);
