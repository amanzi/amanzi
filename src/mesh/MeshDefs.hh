...
//
template <typename T>
using View_type = Kokkos::View<T*, Kokkos::DefaultHostExecutionSpace>;
...
template <typename T>
using DualView_type = Kokkos::DualView<T*, Kokkos::DefaultHostExecutionSpace>;
  ...
  get(bool cached, DATA& d, MF& mf, FF&& f, CFD&& cd, CF&& c, const Entity_ID n)
  {
    ...
    if constexpr (MEM == MemSpace_type::HOST) {
      ...
      if constexpr (!std::is_same<FF, decltype(nullptr)>::value)
        if (mf.get()) return f(n);
      if constexpr (!std::is_same<CF, decltype(nullptr)>::value) return c(c);
      assert(false);
