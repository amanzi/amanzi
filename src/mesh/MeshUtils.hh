...
enum class MemSpace_type { HOST, DEVICE };
...
template <typename T, typename... Args>
Impl::View_iter<T, Args...> KOKKOS_INLINE_FUNCTION
...
KOKKOS_INLINE_FUNCTION auto& // Kokkos::View of the same type as DV, on M
view(DualView& dv)
{
...
template <typename T, typename... Args>
void
my_deep_copy(Kokkos::View<T*, Args...>& out, const std::vector<T>& in)
{
...
struct RaggedArray_DualView {
  ...
  using constview = Kokkos::View<const T*,
                                 std::conditional<MEM == MemSpace_type::DEVICE,
...
asRaggedArray_DualView(Func mesh_func, int count)
{
  ...
  for (int i = 0; i != count; ++i) {
    mesh_func(i, ents);
    Kokkos::View<T*, Kokkos::DefaultHostExecutionSpace> row_view =
      adj.template getRow<MemSpace_type::HOST>(i);
