...
AmanziMesh::Data::Parameters* read_parameters(Exodus_file file)
{
    ...
    if (num_element_blocks > 0) {
        ...
        ret_val = ex_get_ids(file.id, EX_ELEM_BLOCK, &element_block_ids [0]);
        if (ret_val < 0) {
          std::string msg = 
    ...
    if (num_node_sets > 0) {
        ...
        ret_val = ex_get_ids(file.id, EX_NODE_SET, &node_set_ids [0]);
        if (ret_val < 0) {
          std::string msg = 
    ...
    if (num_side_sets > 0) {
        ...
        ret_val = ex_get_ids(file.id, EX_SIDE_SET, &side_set_ids [0]);
        if (ret_val < 0) {
          std::string msg = 
...
AmanziMesh::Data::Node_set* read_node_set(Exodus_file file, int set_id)
{
    ...
    int ret_val = ex_get_set_param(file.id, EX_NODE_SET, set_id, &num_nodes, &num_dist_factors);
    if (ret_val < 0) {
      std::string msg = 
    ...
    ret_val = ex_get_set(file.id, EX_NODE_SET, set_id, &node_list [0], NULL);
    if (ret_val < 0) {
      std::string msg = 
    ...
    if (num_dist_factors > 0) {
        ret_val = ex_get_set_dist_fact(file.id, EX_NODE_SET, set_id, &node_dist_factors [0]);
        if (ret_val < 0) {
          std::string msg = 
...
AmanziMesh::Data::Side_set* read_side_set(Exodus_file file, int set_id)
{
    ...
    int ret_val = ex_get_set_param(file.id, EX_SIDE_SET, set_id, &num_sides, &num_dist_factors);
    if (ret_val < 0) {
      std::string msg = 
    ...
    ret_val = ex_get_set(file.id, EX_SIDE_SET, set_id, &element_list [0], &side_list [0]);
    if (ret_val < 0) {
      std::string msg = 
...
AmanziMesh::Data::Element_block* read_element_block(Exodus_file file, int block_id)
{
    ...
    int ret_val = ex_get_block(file.id, EX_ELEM_BLOCK, block_id, element_name_data,
                               &num_elements, &num_nodes_per_element, 0, 0, &num_attributes);
    if (ret_val < 0) {
      std::string msg = 
    ...
    ret_val = ex_get_conn(file.id, EX_ELEM_BLOCK, block_id, &connectivity_map [0], 0, 0);
    if (ret_val < 0) {
      std::string msg = 
    ...
    if (num_attributes > 0) {
        ...
        ret_val = ex_get_attr(file.id, EX_ELEM_BLOCK, block_id, &attribute_map [0]);
        if (ret_val < 0) {
          std::string msg = 
