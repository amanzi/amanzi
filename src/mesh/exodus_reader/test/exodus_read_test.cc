...
int main(int argc, char *argv[])
{
    ...
    ret_val = ex_get_ids(id, EX_SIDE_SET, &side_set_ids [0]);
    AMANZI_ASSERT(ret_val >= 0);
    ...
    for (std::vector<int>::const_iterator side_set = side_set_ids.begin();
         side_set != side_set_ids.end(); ++side_set) {
        std::cout << "Loading side set: " << *side_set << std::endl;

        int num_sides(0);
        int num_dist_factors(0);
        
        ret_val = ex_get_set_param(id, EX_SIDE_SET, *side_set, &num_sides, &num_dist_factors);
        AMANZI_ASSERT(ret_val >= 0);
