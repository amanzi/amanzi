...
MeshFrameworkColumnAlgorithms::computeCellGeometry(const MeshFramework& mesh,
...
// -----------------------------------------------------------------------------
void
MeshFrameworkColumn::computeSpecialNodeCoordinates_()
{
  ...
  face_in_column_.resize(col3D_mesh.getNumEntities(Entity_kind::FACE, Parallel_type::ALL), -1);
  ...
  for (int n = 0; n != nnodes; ++n) setNodeCoordinate(n, node_coordinates[n]);
}
