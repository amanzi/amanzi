...
struct MeshFactory : public MeshFrameworkFactory {
  ...
  //
  Teuchos::RCP<MeshHost> create(const std::string& filename)
  {
    Teuchos::RCP<MeshFramework> mesh_fw = MeshFrameworkFactory::create(filename);
    auto mesh = Teuchos::rcp(
      new MeshHost(mesh_fw, Teuchos::rcp(new AmanziMesh::MeshAlgorithms()), Teuchos::null));
    return mesh;
  }

  Teuchos::RCP<MeshHost> create(const double x0,
                            const double y0,
                            const double z0,
                            const double x1,
                            const double y1,
  ...
  Teuchos::RCP<MeshHost> create(const Teuchos::ParameterList& gen_plist)
  {
  ...
  Teuchos::RCP<MeshHost> create(const Teuchos::RCP<const MeshHost>& parent_mesh,
  ...
  Teuchos::RCP<MeshHost> createLogical(Teuchos::ParameterList& log_plist);


  // Create a 1D Column Mesh from a columnar structured volume mesh.
  //
  Teuchos::RCP<MeshHost> createColumn(const Teuchos::RCP<MeshHost>& parent,
                                  int col_id,
                                  const Teuchos::RCP<Teuchos::ParameterList>& plist);

  // Create a MeshSurfaceCell from a MeshFrameworkColumn
  Teuchos::RCP<MeshHost> createSurfaceCell(const Teuchos::RCP<const MeshHost>& parent);
};
