...
SUITE(MeshFramework)
{
  TEST(Generate3D)
  {
    ...
    if (parallel) {
      ...
      CHECK_THROW(mesh = meshfactory.create(x0, y0, z0, x1, y1, z1, nx, ny, nz), Message);
    } else {
      Teuchos::RCP<MeshFramework> mesh;
    ...
    if (framework_enabled(Framework::MSTK)) {
      ...
      meshfactory.set_preference(pref);
      Teuchos::RCP<MeshFramework> mesh = meshfactory.create(x0, y0, z0, x1, y1, z1, nx, ny, nz);
      CHECK(!mesh.is_null());
    }
    ...
    if (framework_enabled(Framework::MOAB)) {
      ...
      meshfactory.set_preference(pref);
      CHECK_THROW(auto mesh = meshfactory.create(x0, y0, z0, x1, y1, z1, nx, ny, nz), Message);
    }
  }
