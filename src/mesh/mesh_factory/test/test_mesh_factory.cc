...
SUITE(MeshFramework)
{
  ...
  TEST(Generate2D)
  {
    ...
    CHECK_THROW(mesh = meshfactory.create(x0, y0, x1, y1, nx, ny),
                Amanzi::AmanziMesh::Message);
    mesh.reset();
  ...
  TEST(Extract3)
  {
    ...
    if (parallel) {
      ...
      CHECK_THROW(newmesh = meshfactory.create(
                    mesh, setnames, Amanzi::AmanziMesh::FACE, flatten, extrude),
                  Amanzi::AmanziMesh::Message);
    }
    ...
    if (Amanzi::AmanziMesh::framework_enabled(
      ...
      newmesh =
        meshfactory.create(mesh, setnames, Amanzi::AmanziMesh::FACE, flatten);
