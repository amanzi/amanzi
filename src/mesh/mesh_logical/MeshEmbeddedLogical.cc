...
MeshEmbeddedLogical::MeshEmbeddedLogical(
  ...
  : Mesh(comm, Teuchos::null, plist, true, false), bg_mesh_(bg_mesh), log_mesh_(log_mesh)
{
  set_space_dimension(3);
  set_manifold_dimension(3); // mixed?

  // ensure cached properties are available
  if (!bg_mesh->cell_geometry_precomputed_) bg_mesh->compute_cell_geometric_quantities_();
  if (!bg_mesh->face_geometry_precomputed_) bg_mesh->compute_face_geometric_quantities_();
  if (!bg_mesh->cell2face_info_cached_ || !bg_mesh->face2cell_info_cached_)
    bg_mesh->cache_cell2face_info_();

  // ensure cached properties are available
  if (!log_mesh->cell_geometry_precomputed_) log_mesh->compute_cell_geometric_quantities_();
  if (!log_mesh->face_geometry_precomputed_) log_mesh->compute_face_geometric_quantities_();
  ...
  face_cell_ids_.insert(face_cell_ids_.end(), face_cell_ids_copy.begin(), face_cell_ids_copy.end());
  ...
  face_cell_ids_.insert(
    face_cell_ids_.end(), bg_mesh->face_cell_ids_.begin(), bg_mesh->face_cell_ids_.end());
  ...
  for (int i = 0; i < face_area_normals.size(); i++)
    face_normals_.emplace_back(
      std::vector<AmanziGeometry::Point>({ face_area_normals[i], -face_area_normals[i] }));

  face_normals_.insert(
    face_normals_.end(), bg_mesh->face_normals_.begin(), bg_mesh->face_normals_.end());
  ...
  for (int f = 0; f != nfaces_extra; ++f) { face_cell_ptype_.push_back(extra_ptypes); }
  face_cell_ptype_.insert(
    face_cell_ptype_.end(), bg_mesh->face_cell_ptype_.begin(), bg_mesh->face_cell_ptype_.end());
  ...
  face_areas_.insert(face_areas_.end(), bg_mesh->face_areas_.begin(), bg_mesh->face_areas_.end());
  ...
  cell_face_ids_.insert(
    cell_face_ids_.end(), bg_mesh->cell_face_ids_.begin(), bg_mesh->cell_face_ids_.end());
  ...
  for (int c = ncells_log; c != cell_face_ids_.size(); ++c) {
    ...
    for (int i = 0; i != n_faces; ++i) { cell_face_ids_[c][i] += nfaces_log + nfaces_extra; }
  }

  // directions
  cell_face_dirs_ = log_mesh->cell_face_dirs_;
  cell_face_dirs_.insert(
    cell_face_dirs_.end(), bg_mesh->cell_face_dirs_.begin(), bg_mesh->cell_face_dirs_.end());
  ...
  cell_volumes_.insert(
    cell_volumes_.end(), bg_mesh->cell_volumes_.begin(), bg_mesh->cell_volumes_.end());
  ...
  if (bg_mesh->cell_centroids_.size() > 0) {
    ...
    cell_centroids_.insert(
      cell_centroids_.end(), bg_mesh->cell_centroids_.begin(), bg_mesh->cell_centroids_.end());
    ...
    for (int f = nfaces_log; f != nfaces_log + nfaces_extra; ++f) {
      Entity_ID c0 = face_cell_ids_[f][0];
      Entity_ID c1 = ~face_cell_ids_[f][1];
      face_centroids_.push_back((cell_centroids_[c0] + cell_centroids_[c1]) / 2.0);
    }
    face_centroids_.insert(
      face_centroids_.end(), bg_mesh->face_centroids_.begin(), bg_mesh->face_centroids_.end());
  }
...
MeshEmbeddedLogical::init_maps()
{
  // owned maps
  // -- cell map
  maps_owned_[CELL] = Teuchos::rcp(new Epetra_Map(-1, num_entities_owned_[CELL], 0, *comm_));
  ...
  for (int f = 0; f != nfaces_owned; ++f) {
    if (face_cell_ids_[f].size() == 1) { extface_ids.push_back(face_map->GID(f)); }
  }
  maps_owned_[BOUNDARY_FACE] =
    Teuchos::rcp(new Epetra_Map(-1, extface_ids.size(), &extface_ids[0], 0, *comm_));
  exterior_face_importer_ = Teuchos::rcp(new Epetra_Import(*maps_owned_[BOUNDARY_FACE], *face_map));
  ...
  if (bg_mesh_->get_comm()->NumProc() == 1) {
    ...
    for (int c = 0; c != ncells_log; ++c) { cells_my_used[c] = my_cell_map.GID(c); }
    for (int c = 0; c != ncells_bg_used; ++c) { cells_my_used[c + ncells_log] = bg_gids_used_c[c]; }

    // -- create the map
    maps_used_[CELL] =
      Teuchos::rcp(new Epetra_Map(-1, ncells_my_used, &cells_my_used[0], 0, *comm_));
    ...
    for (int f = 0; f != nfaces_log_extra; ++f) { faces_my_used[f] = my_face_map.GID(f); }
    ...
    // -- create the map
    maps_used_[FACE] =
      Teuchos::rcp(new Epetra_Map(-1, nfaces_my_used, &faces_my_used[0], 0, *comm_));
  }
}
...
Parallel_type
MeshEmbeddedLogical::entity_get_ptype(const Entity_kind kind, const Entity_ID entid) const
{
...
unsigned int
MeshEmbeddedLogical::num_entities(const Entity_kind kind, const Parallel_type ptype) const
{
...
// Get nodes of a cell
void
MeshEmbeddedLogical::cell_get_nodes(const Entity_ID cellid, Entity_ID_List* nodeids) const
{
...
// In 2D, nfnodes is 2
void
MeshEmbeddedLogical::face_get_nodes(const Entity_ID faceid, Entity_ID_List* nodeids) const
{
  Errors::Message mesg("No nodes in MeshEmbeddedLogical.");
  Exceptions::amanzi_throw(mesg);
}
...
{
  Errors::Message mesg(
    "Not implemented."); // this could be implemented, just not sure we need it yet. --etc
  Exceptions::amanzi_throw(mesg);
}
...
void
MeshEmbeddedLogical::node_set_coordinates(const Entity_ID nodeid, const double* ncoord)
{
  Errors::Message mesg("No nodes in MeshEmbeddedLogical.");
  Exceptions::amanzi_throw(mesg);
}
                                      ...
                                      Entity_ID_List* entids) const
{
  ...
  if (rgn->get_type() == AmanziGeometry::RegionType::ENUMERATED) {
    ...
    // shift local indices
    Entity_ID shift = num_entities(kind, ptype) - bg_mesh_->num_entities(kind, ptype);
    for (Entity_ID_List::iterator i = entids->begin(); i != entids->end(); ++i) { (*i) += shift; }
  }
  return;
}
...
void
MeshEmbeddedLogical::get_set_entities_and_vofs(const std::string& setname,
                                               const Entity_kind kind,
                                               const Parallel_type ptype,
...
// framework. The results are cached in the base class.
void
MeshEmbeddedLogical::cell_get_edges_internal_(const Entity_ID cellid, Entity_ID_List* edgeids) const
{
  Errors::Message mesg("No edges in MeshEmbeddedLogical.");
  Exceptions::amanzi_throw(mesg);
}
