      ...
      extra_face_area_normals_(face_area_normals)
{
  ...
  int nfaces_extra = extra_face_cell_ids_.size();

  // compute "extra" bisectors
  extra_face_cell_bisectors_.resize(nfaces_extra, Point_List(2));
  ...
  for (int fi=0; fi!=nfaces_extra; ++fi) {
    ...
    extra_face_cell_bisectors_[fi][0] = extra_face_cell_lengths_[fi][0] / AmanziGeometry::norm(normal0) * normal0;

    auto normal1 = extra_face_area_normals_[fi];
    extra_face_cell_bisectors_[fi][1] = face_cell_lengths[fi][1] / AmanziGeometry::norm(normal1) * normal1;
  }
  ...
  for (int c=0; c!=ncells_bg_owned; ++c) bg_cell_owned_vec[c] = cell_owned_map.GID(c+ncells_log);
  ...
  for (int f=0; f!=nfaces_bg_owned; ++f) bg_face_owned_vec[f] = face_owned_map.GID(f+nfaces_log+nfaces_extra);
  ...
  // -- copy ghost GIDs into the renumbered vector
  Entity_GID_List face_gids(nfaces_bg_all + nfaces_log);
        ...
        const Parallel_type ptype) const
{
  ...
  if (kind == Entity_kind::FACE) count += extra_face_cell_ids_.size();
  return count;
}
  ...
  Entity_Direction_List * const dirs) const
{
  ...
  if (c < ncells_log) {
    ...
    for (int f=0; f!=extra_face_cell_ids_.size(); ++f) {
      if (c == extra_face_cell_ids_[f][0]) {
    ...
    auto nfaces_log = log_mesh_->getNumEntities(Entity_kind::FACE, Parallel_type::OWNED);
    auto nfaces_extra = extra_face_cell_ids_.size();
    ...
    for (int f=0; f!=extra_face_cell_ids_.size(); ++f) {
      if (c == (extra_face_cell_ids_[f][1]+ncells_log)) {
        faces.push_back(f + nfaces_log);
        if (dirs) dirs->push_back(-1); // direction always from log to bg
      }
    }
  ...
  Point_List * const bisectors) const
{
  ...
  if (c < ncells_log) {
    ...
    for (int f=0; f!=extra_face_cell_ids_.size(); ++f) {
      if (c == extra_face_cell_ids_[f][0]) {
        faces.push_back(f + nfaces_log);
        if (bisectors) bisectors->push_back(extra_face_cell_bisectors_[f][0]);
      }
    }
    ...
    for (int f=0; f!=extra_face_cell_ids_.size(); ++f) {
      if (c == (extra_face_cell_ids_[f][1]+ncells_log)) {
        faces.push_back(f + nfaces_log);
        if (bisectors) bisectors->push_back(extra_face_cell_bisectors_[f][1]);
      }
    }
  }
}
        ...
        Entity_ID_List& cells) const
{
  ...
  if (f < nfaces_log) {
    ...
    cells = extra_face_cell_ids_[f - nfaces_log];
    cells[1] += ncells_log;
  }
}
...
MeshEmbeddedLogical::getFaceCentroid(const Entity_ID f) const
{
  ...
  if (f < nfaces_log) {
    ...
    auto log_cc = log_mesh_->getCellCentroid(extra_face_cell_ids_[f - nfaces_log][0]);
    auto bg_cc = bg_mesh_->getCellCentroid(extra_face_cell_ids_[f - nfaces_log][1]);
