...
void
MeshLogical::getLogicalGeometry(
...
void
MeshLogical::setLogicalGeometry(
...
MeshLogical::operator==(const MeshLogical& other)
{
  ...
  if (cell_face_bisectors_.size<MemSpace_type::HOST>() !=
      other.cell_face_bisectors_.size<MemSpace_type::HOST>())
    return false;
  for (size_t i = 0; i != cell_face_bisectors_.size<MemSpace_type::HOST>(); ++i) {
    if (cell_face_bisectors_.getRow<MemSpace_type::HOST>(i).size() !=
        other.cell_face_bisectors_.getRow<MemSpace_type::HOST>(i).size())
      return false;
    for (size_t j = 0; j != cell_face_bisectors_.getRow<MemSpace_type::HOST>(i).size(); ++j)
      if (AmanziGeometry::norm(cell_face_bisectors_.get<MemSpace_type::HOST>(i, j) -
                               other.cell_face_bisectors_.get<MemSpace_type::HOST>(i, j)) > _eps)
        return false;
  }
...
std::size_t
MeshLogical::getNumEntities(const Entity_kind kind, const Parallel_type ptype) const
{
...
//
void
MeshLogical::getFaceCells(const Entity_ID f, const Parallel_type ptype, Entity_ID_List& cells) const
{
