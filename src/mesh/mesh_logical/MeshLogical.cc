...
// cell geometry cannot be done without the framework.  Either caching or
// keeping the framework is required for all geoemtric operations
std::pair<double, AmanziGeometry::Point>
MeshLogicalAlgorithms::computeCellGeometry(const Mesh& mesh, const Entity_ID c) const
{
  ...
  return computeCellGeometry(*mesh.getMeshFramework(), c);
}
...
MeshLogicalAlgorithms::computeFaceGeometry(const Mesh& mesh, const Entity_ID f) const
{
  ...
  return computeFaceGeometry(*mesh.getMeshFramework(), f);
}
...
MeshLogicalAlgorithms::computeEdgeGeometry(const Mesh& mesh, const Entity_ID e) const
{
  ...
  return computeEdgeGeometry(*mesh.getMeshFramework(), e);
}
...
MeshLogicalAlgorithms::computeBisectors(const Mesh& mesh, const Entity_ID c, const Entity_ID_List& faces) const
{
  ...
  Entity_ID_List faces2;
  Point_List bisectors;
  mesh.getMeshFramework()->getCellFacesAndBisectors(c, faces2, &bisectors);
  return bisectors;
}
                            ...
                            const Parallel_type ptype) const
{
  if (kind == Entity_kind::CELL) {
    return cell_face_ids_.size();
  } else if (kind == Entity_kind::FACE) {
    return face_cell_ids_.size();
  } else {
    return 0;
  }
}
...
{
  faces = cell_face_ids_[c];
  if (dirs) (*dirs) = cell_face_dirs_[c];
}
