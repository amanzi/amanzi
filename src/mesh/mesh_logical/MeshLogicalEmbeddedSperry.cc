...
MeshLogicalEmbeddedSperry::CreateLogical(const std::string& name, int n_stem,
...
Teuchos::RCP<MeshLogical>
MeshLogicalEmbeddedSperry::CreateEmbedded(
  const std::string& name, int n_stem, double max_rooting_depth,
  int n_rheizosphere_shells, const Teuchos::RCP<AmanziMesh::Mesh>& bg_mesh)
{
  name_ = name;
  n_stem_ = n_stem;
  max_rooting_depth_ = max_rooting_depth;
...
Teuchos::RCP<MeshLogical>
MeshLogicalEmbeddedSperry::CreateLogical_(const std::string& pftname,
                                          int column) const
{
  ...
  AMANZI_ASSERT(ground_surface[2] >
                bg_mesh_->cell_centroid(cells_of_col[0])[2]);
  ...
  cells_dzs.back() =
    bg_mesh_->face_centroid(faces_of_col[n_cells_root_zone - 1])[2] -
  ...
  for (int cc = 0; cc != n_cells_root_zone; ++cc) {
    ...
    rheizosphere_to_bg_.emplace_back(
      Entity_ID_List{ shell_cells.back(), cells_in_col[cc] });
  }
};
