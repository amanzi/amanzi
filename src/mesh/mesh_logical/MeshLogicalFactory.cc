...
* `"calculate cell volumes from lengths and areas`" ``[bool]`` **true** If true,
calculate via the above formula.  Otherwise each segment must provide the `"cell
volume`" or
...
* `"cell volumes [m^3]`" ``[Array(double)]`` list of volumes, [m^{2,3}],
depending upon mesh dimension.
...
* `"cross sectional area [m^2]`" ``[double]`` cross sectional area of all faces
in the segment.
...
MeshLogicalFactory::Create(Teuchos::ParameterList& plist)
{
  ...
  if (plist.isParameter("calculate cell volumes from lengths and areas"))
    calculated_volume_ =
      plist.get<bool>("calculate cell volumes from lengths and areas");
  if (plist.isParameter("cell volumes provided")) {
    Errors::Message msg(
      "MeshLogicalFactory: deprecated parameter \"cell volumes provided\", "
      "please use \"calculate cell volumes from lengths and areas\".");
  ...
  for (auto& set_it : sets) {
    ...
    for (auto& seg_name : set_list) {
      ...
      if (seg_cell == seg_cells_.end()) {
        Errors::Message msg(
          "MeshLogicalFactory: set requests segment not created.");
        Exceptions::amanzi_throw(msg);
      }

      set_ents.insert(
        set_ents.end(), seg_cell->second.begin(), seg_cell->second.end());
    }
...
void
MeshLogicalFactory::AddSegment(int n_cells, const AmanziGeometry::Point& begin,
...
MeshLogicalFactory::AddSegment(
  ...
  MeshLogicalFactory::LogicalTip_t first_tip_type,
  MeshLogicalFactory::LogicalTip_t last_tip_type, std::string const& seg_name,
  std::vector<Entity_ID>* const cells, std::vector<Entity_ID>* const faces)
{
  // manage output
  ...
  // insert cell lengths
  cell_lengths_.insert(
    cell_lengths_.end(), cell_lengths.begin(), cell_lengths.end());

  // make space for cell volumes
  if (cell_volumes) {
    cell_volumes_.insert(
      cell_volumes_.end(), cell_volumes->begin(), cell_volumes->end());
  } else {
    cell_volumes_.resize(cell_volumes_.size() + n_cells, 0.);
  }

  // ensure orientation is unit normal
  AmanziGeometry::Point my_orientation =
  ...
  for (int j = 1; j != n_cells; ++j) {
    face_cell_list_.emplace_back(
      std::vector<int>{ new_cells[j - 1], new_cells[j] });
    face_cell_lengths_.emplace_back(
      std::vector<double>{ cell_lengths[j - 1] / 2, cell_lengths[j] / 2 });
    if (calculated_volume_) {
      cell_volumes_[new_cells[j - 1]] +=
        face_areas[i_face] * cell_lengths[j - 1] / 2;
  ...
  if (last_tip_type == LogicalTip_t::BOUNDARY) {
    ...
    face_cell_lengths_.emplace_back(
      std::vector<double>{ cell_lengths.back() / 2 });
    if (calculated_volume_) {
      cell_volumes_[new_cells.back()] +=
        face_areas[i_face] * cell_lengths.back() / 2;
    }
    face_area_normals_.emplace_back(face_areas[i_face] * my_orientation);
    i_face++;
  }
  ...
  if (cell_centroids) {
    cell_centroids_.insert(
      cell_centroids_.end(), cell_centroids->begin(), cell_centroids->end());
  }
...
MeshLogicalFactory::AddSegment(Teuchos::ParameterList& plist)
{
  ...
  if (plist.isParameter("cell lengths [m]")) {
    cell_lengths =
      plist.get<Teuchos::Array<double>>("cell lengths [m]").toVector();
  ...
  } else if (plist.isParameter("first tip") && plist.isParameter("last tip") &&
             plist.isParameter("number of cells")) {
    ...
    msg << "MeshLogicalFactory (segment \"" << seg_name
        << "\"): unable to get number of cells and segment length.  See "
           "documentation.";
    Exceptions::amanzi_throw(msg);
  }
  ...
  if (!calculated_volume_) {
    if (plist.isParameter("cell volume [m]")) {
      ...
      cell_volumes =
        plist.get<Teuchos::Array<double>>("cell volumes [m^3]").toVector();
      if (cell_volumes.size() != n_cells) {
        Errors::Message msg;
        msg << "MeshLogicalFactory (segment \"" << seg_name
            << "\"): number of cells inconsistent with number of cell volumes "
               "provided.";
        Exceptions::amanzi_throw(msg);
      }
    } else {
      Errors::Message msg;
      msg << "MeshLogicalFactory (segment \"" << seg_name
          << "\"): unable to get cell volumes for mesh with non-calculated "
             "volumes.  "
          << "See documentation";
      Exceptions::amanzi_throw(msg);
    }
  ...
  if (plist.isParameter("cross sectional area [m^2]")) {
    ...
    face_areas_mine =
      plist.get<Teuchos::Array<double>>("cross sectional areas [m^2]")
        .toVector();
    if (face_areas_mine.size() != n_faces_mine) {
      Errors::Message msg;
      msg << "MeshLogicalFactory (segment \"" << seg_name
          << "\"): Incorrect number of cross sectional areas provided.";
      Exceptions::amanzi_throw(msg);
    }
  } else {
    Errors::Message msg;
    msg << "MeshLogicalFactory (segment \"" << seg_name
        << "\"): cross sectional areas not specified.";
    Exceptions::amanzi_throw(msg);
  }
  ...
  if (plist.isParameter("cell centroids")) {
    auto centroids_raw =
      plist.get<Teuchos::Array<double>>("cell centroids").toVector();
    if (centroids_raw.size() != n_cells * 3) {
      Errors::Message msg;
      msg << "MeshLogicalFactory (segment \"" << seg_name
          << "\"): Specified cell centroid vector must be of size 3 * n_cells.";
      Exceptions::amanzi_throw(msg);
    }
    for (int i = 0; i != n_cells; ++i) {
      cell_centroids.emplace_back(
        AmanziGeometry::Point(centroids_raw[3 * i],
                              centroids_raw[3 * i + 1],
                              centroids_raw[3 * i + 2]));
    }
  } else if (plist.isParameter("first tip") && plist.isParameter("last tip")) {
    ...
    if (first_tip_type == LogicalTip_t::BOUNDARY) {
      ...
      if (first_tip_type != LogicalTip_t::BRANCH) {
        Errors::Message msg;
        msg << "MeshLogicalFactory (segment \"" << seg_name
            << "\"): In tracking centroids mode, all \"first tip type\" values "
               "must be BOUNDARY or BRANCH";
        Exceptions::amanzi_throw(msg);
      }
    ...
    tracking_end_points_[seg_name] =
      my_centroid + ds * cell_lengths[n_cells - 1] / 2;
  }

  // Now start doing the add

  // -- if a branch, reserve space for the first face.  This keeps faces in some
  // reasonable
  ...
  if (first_tip_type == LogicalTip_t::BRANCH) {
    ...
    if (branch_from_seg == seg_cells_.end()) {
      Errors::Message msg;
      msg << "MeshLogicalFactory (segment \"" << seg_name
          << "\"): branches from segment \"" << branch_from
          << "\" but this segment has not yet been specified.  Junctions must "
             "come before Branches!";
      Exceptions::amanzi_throw(msg);
    }

    // note cell_volumes_.size() will be the LID of the first cell of this
    // segment!
    ...
    if (branch_from_tip == "first") {
      ...
      Errors::Message msg;
      msg << "MeshLogicalFactory (segment \"" << seg_name
          << "\"): \"first tip branch segment tip\" must be \"first\" or "
             "\"last\"";
      Exceptions::amanzi_throw(msg);
    }
...
int
MeshLogicalFactory::AddFace(int f, const Entity_ID_List& cells,
                            const AmanziGeometry::Point& normal,
                            const std::vector<double>& lengths, double area)
{
  if (cells.size() != 2) {
    Errors::Message msg("MeshLogicalFactory: connection added is improperly "
                        "formed -- all connections need two cells.");
    Exceptions::amanzi_throw(msg);
  }
...
int
MeshLogicalFactory::AddSet(const std::string& set_name, const std::string& ent,
                           const Entity_ID_List& ents)
{
  ...
  Teuchos::RCP<AmanziGeometry::RegionEnumerated> enum_rgn =
    Teuchos::rcp(new AmanziGeometry::RegionEnumerated(
      set_name, gm_->RegionSize(), ent, ents));
...
MeshLogicalFactory::GetTipType_(Teuchos::ParameterList& plist,
...
MeshLogicalFactory::GetPoint_(Teuchos::ParameterList& plist,
                              const std::string& pname)
{
  auto point_a = plist.get<Teuchos::Array<double>>(pname);
  if (point_a.size() != 3) {
    Errors::Message msg("MeshLogicalFactory: logical meshes must be in 3D, so "
                        "points must be of length 3.");
    Exceptions::amanzi_throw(msg);
  }
