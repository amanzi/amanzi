...
demoMeshLogicalYManual()
{
  ...
  cell_vols.push_back(cross_section_coarse * half_cell_coarse +
                      4 * cross_section_fine * half_cell_coarse); // junction cell
  ...
  // make the mesh
  Teuchos::RCP<MeshLogical> m = Teuchos::rcp(new MeshLogical(
    comm, face_cells, face_cell_dirs, cell_vols, face_areas, face_cell_bisectors, &cell_centroids));
...
demoMeshLogicalYEmbedded()
{
  ...
  Teuchos::RCP<Mesh_MSTK> m_bg_fw = Teuchos::rcp(new Mesh_MSTK(X0[0],
                                                               X0[1],
                                                               X0[2],
                                                               X1[0],
                                                               X1[1],
  ...
  auto m_bg = Teuchos::rcp(new Mesh(
    m_bg_fw, Teuchos::rcp(new Amanzi::AmanziMesh::MeshAlgorithms()), Teuchos::null));
