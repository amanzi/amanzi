  ...
  : Mesh(comm, gm, Teuchos::null, request_faces, request_edges)
{
  ...
  if (!serial_run) {
    ...
    result =
      mbcore_->load_file(filename.c_str(),
                         NULL,
                         "PARALLEL=READ_DELETE;PARALLEL_RESOLVE_SHARED_ENTS;"
                         "PARTITION=PARALLEL_PARTITION;PARALLEL_GHOSTS=3.0.1.2",
                         NULL,
                         NULL,
                         0);
  ...
  if (result != MB_SUCCESS) {
    ...
    std::cerr << "Failed to load " << filename << " on processor " << rank
              << std::endl;
...
Mesh_MOAB::init_id_handle_maps()
{
  ...
  // -- nodes
  int tagval = 0;
  result = mbcore_->tag_get_handle("LOCAL_ID",
                                   1,
                                   MB_TYPE_INTEGER,
                                   lid_tag,
...
Mesh_MOAB::init_global_ids()
{
  ...
  if (!serial_run) {
    ...
    result =
      mbcomm_->assign_global_ids(0, largest_dim, start_id, largest_dim_only);
    ...
    int tagval = 0;
    result = mbcore_->tag_get_handle("GLOBAL_ID",
                                     1,
                                     MB_TYPE_INTEGER,
                                     gid_tag,
                                     MB_TAG_CREAT | MB_TAG_DENSE,
...
Mesh_MOAB::init_pface_lists()
{
  ...
  for (auto it = AllCells.begin(); it != AllCells.end(); it++) {
    ...
    result =
      mbcore_->get_adjacencies(&cell, 1, facedim, true, cfaces, Core::UNION);
  ...
  // Get not owned faces
  result =
    mbcomm_->get_pstatus_entities(facedim, PSTATUS_NOT_OWNED, NotOwnedFaces);
...
Mesh_MOAB::init_pface_dirs()
{
  ...
  result = mbcore_->tag_get_handle("TMP_FC0_TAG",
                                   1,
                                   MB_TYPE_INTEGER,
                                   tmp_fc0_tag,
                                   MB_TAG_CREAT | MB_TAG_DENSE,
  ...
  result = mbcore_->tag_get_handle("TMP_FC1_TAG",
                                   1,
                                   MB_TYPE_INTEGER,
                                   tmp_fc1_tag,
                                   MB_TAG_CREAT | MB_TAG_DENSE,
  ...
  for (auto it = OwnedFaces.begin(); it != OwnedFaces.end(); it++) {
    ...
    result =
      mbcore_->get_adjacencies(&face, 1, celldim, false, fcells, Core::UNION);
  ...
  for (auto it = NotOwnedFaces.begin(); it != NotOwnedFaces.end(); it++) {
    ...
    if (ghost_cell0_gid == master_cell1_gid ||
      ...
      if (ghost_cell0_gid != master_cell0_gid &&
        ...
        std::cout << "Master cells " << master_cell0_gid << " "
                  << master_cell1_gid << std::endl;
        std::cout << "Ghost cells " << ghost_cell0_gid << " " << ghost_cell1_gid
                  << std::endl;
      }
    }
  }
...
Mesh_MOAB::init_set_info()
{
  ...
  result =
    mbcore_->tag_get_handle(NEUMANN_SET_TAG_NAME, 1, MB_TYPE_INTEGER, sstag);
  ...
  for (int i = 0; i < ngr; i++) {
    ...
    if (rgn->type() == AmanziGeometry::LABELEDSET) {
      ...
      result = mbcore_->tag_get_handle(internal_name.c_str(),
                                       1,
                                       MB_TYPE_INTEGER,
                                       tag,
    ...
    //
    //  result = mbcore_->tag_get_handle(internal_name.c_str(), 1,
    //  MB_TYPE_INTEGER,
    //                                  tag, MB_TAG_CREAT|MB_TAG_SPARSE);
    //    if (result != MB_SUCCESS) {
    //      std::cerr << "Could not create tag with name " << rgn->name() <<
    //      std::endl; AMANZI_ASSERT(result != MB_SUCCESS);
    //    }
    //  }
    //}
  }
...
//--------------------------------------
unsigned int
Mesh_MOAB::num_entities(Entity_kind kind, Parallel_type ptype) const
{
...
//--------------------------------------------------------------------
void
Mesh_MOAB::cell_get_faces_and_dirs_internal_(const Entity_ID cellid,
                                             ...
                                             const bool ordered) const
{
  ...
  result = mbcore_->get_adjacencies(
    &cell, 1, facedim, true, cell_faces, moab::Interface::INTERSECT);
  ...
  if (ordered && nf == 6) { // Hex
    ...
    if (face_dirs) {
      for (int i = 0; i < nf; i++) {
        ...
        result =
          mbcore_->side_number(cell, face, sidenum, cell_facedirs[i], offset);
...
//--------------------------------------------------------------------
void
Mesh_MOAB::cell_get_nodes(Entity_ID cellid, Entity_ID_List* cnodes) const
{
  moab::EntityHandle cell;
  ...
  for (int i = 0; i < nn; i++) {
    result =
      mbcore_->tag_get_data(lid_tag, &(cell_nodes[i]), 1, &(cell_nodeids[i]));
    ErrorCheck_(result, "Problem getting tag data");
  }
...
//--------------------------------------------------------------------
void
Mesh_MOAB::face_get_nodes(Entity_ID faceid, Entity_ID_List* fnodes) const
{
  ...
  if (faceflip[faceid]) {
    for (int i = nn - 1; i >= 0; i--) {
      result = mbcore_->tag_get_data(
        lid_tag, &(face_nodes[i]), 1, &(face_nodeids[nn - i - 1]));
      ErrorCheck_(result, "Problem getting tag data");
    }
  } else {
    for (int i = 0; i < nn; i++) {
      result =
        mbcore_->tag_get_data(lid_tag, &(face_nodes[i]), 1, &(face_nodeids[i]));
      ErrorCheck_(result, "Problem getting tag data");
    }
  }
...
//--------------------------------------------------------------------
void
Mesh_MOAB::node_get_coordinates(Entity_ID node_id,
...
//--------------------------------------------------------------------
void
Mesh_MOAB::node_set_coordinates(const AmanziMesh::Entity_ID nodeid,
                                const double* coords)
{
...
//--------------------------------------------------------------------
void
Mesh_MOAB::cell_get_coordinates(
...
//--------------------------------------------------------------------
void
Mesh_MOAB::face_get_coordinates(
  Entity_ID faceid, std::vector<AmanziGeometry::Point>* fcoords) const
{
  moab::EntityHandle face;
  std::vector<moab::EntityHandle> face_nodes;
  double* coords;
                     ...
                     const Entity_kind kind) const
{
  ...
  case CELL: // cellsets
    if (region->type() == AmanziGeometry::BOX ||
      ...
      mbcore_->tag_get_handle(internal_name.c_str(),
                              1,
                              MB_TYPE_INTEGER,
                              tag,
                              MB_TAG_CREAT | MB_TAG_SPARSE);
      ...
      Errors::Message mesg(
        "Region type not applicable/supported for cell sets");
      amanzi_throw(mesg);
    }
  ...
  case FACE: // sidesets
    if (region->type() == AmanziGeometry::BOX) {
      ...
      Errors::Message mesg(
        "Region type not applicable/supported for face sets");
      amanzi_throw(mesg);
    }
    break;
  ...
  case NODE: // Nodesets
    if (region->type() == AmanziGeometry::BOX ||
      ...
      Errors::Message mesg(
        "Region type not applicable/supported for node sets");
      amanzi_throw(mesg);
    }
  ...
  if (region->type() == AmanziGeometry::LOGICAL) {
    ...
    for (int r = 0; r < nreg; r++) {
      ...
      mbcore_->tag_get_handle(
        internal_name.c_str(), 1, MB_TYPE_INTEGER, tag1, MB_TAG_SPARSE);
      if (!tag1) tag1 = build_set(rgn1, kind); // Recursive call
    ...
    if (boolregion->operation() == AmanziGeometry::COMPLEMENT) {
      ...
      case CELL:
        for (int i = 0; i < tags.size(); i++) {
          mbcore_->get_entities_by_type_and_tag(
            0, MBHEX, &(tags[i]), (void**)values, 1, entset2);
      ...
      case FACE:
        for (int i = 0; i < tags.size(); i++) {
          mbcore_->get_entities_by_type_and_tag(
            0, MBQUAD, &(tags[i]), (void**)values, 1, entset2);
      ...
      case NODE:
        for (int i = 0; i < tags.size(); i++) {
          mbcore_->get_entities_by_type_and_tag(
            0, MBVERTEX, &(tags[i]), (void**)values, 1, entset2);
      ...
      case CELL:
        for (int i = 0; i < tags.size(); i++) {
          int* values[1] = { &one };
          mbcore_->get_entities_by_type_and_tag(
            0, MBHEX, &(tags[i]), (void**)values, 1, entset1);
      ...
      case FACE:
        for (int i = 0; i < tags.size(); i++) {
          int* values[1] = { &one };
          mbcore_->get_entities_by_type_and_tag(
            0, MBQUAD, &(tags[i]), (void**)values, 1, entset1);
      ...
      case NODE:
        for (int i = 0; i < tags.size(); i++) {
          int* values[1] = { &one };
          mbcore_->get_entities_by_type_and_tag(
            0, MBVERTEX, &(tags[i]), (void**)values, 1, entset1);
      ...
      case CELL:
        mbcore_->get_entities_by_type_and_tag(
          0, MBHEX, &(tags[0]), (void**)values, 1, entset);
      ...
      case FACE:
        mbcore_->get_entities_by_type_and_tag(
          0, MBQUAD, &(tags[0]), (void**)values, 1, entset);
        for (int i = 1; i < tags.size(); i++) {
          mbcore_->get_entities_by_type_and_tag(
            0, MBQUAD, &(tags[i]), (void**)values, 1, entset1);
      ...
      case NODE:
        mbcore_->get_entities_by_type_and_tag(
          0, MBVERTEX, &(tags[0]), (void**)values, 1, entset);
        for (int i = 1; i < tags.size(); i++) {
          mbcore_->get_entities_by_type_and_tag(
            0, MBVERTEX, &(tags[i]), (void**)values, 1, entset1);
    ...
    mbcore_->tag_get_handle(new_internal_name.c_str(),
                            1,
                            MB_TYPE_INTEGER,
                            tag,
                            MB_TAG_CREAT | MB_TAG_SPARSE);
...
//--------------------------------------------------------------------
void
Mesh_MOAB::get_set_entities_and_vofs(const std::string setname,
                                     ...
                                     Kokkos::View<double*>* vofs) const
{
  ...
  if (rgn->type() == AmanziGeometry::LABELEDSET) {
    ...
    std::string internal_name = internal_name_of_set(rgn, kind);
    mbcore_->tag_get_handle(
      internal_name.c_str(), 1, MB_TYPE_INTEGER, tag, MB_TAG_SPARSE);
    if (!tag) tag = build_set(rgn, kind);

    if (kind == NODE) {
      mbcore_->get_entities_by_type_and_tag(
        0, MBVERTEX, &tag, (void**)values, 1, mset1);
    } else if (space_dim_ == 3 && kind == CELL) {
      mbcore_->get_entities_by_type_and_tag(
        0, MBHEX, &tag, (void**)values, 1, mset1);
    } else if (space_dim_ == 3 && kind == FACE) {
      mbcore_->get_entities_by_type_and_tag(
        0, MBQUAD, &tag, (void**)values, 1, mset1);
    } else if (space_dim_ == 2 && kind == CELL) {
      mbcore_->get_entities_by_type_and_tag(
        0, MBQUAD, &tag, (void**)values, 1, mset1);
    } else if (space_dim_ == 2 && kind == FACE) {
      mbcore_->get_entities_by_type_and_tag(
        0, MBEDGE, &tag, (void**)values, 1, mset1);
    }
  }
...
//--------------------------------------------------------------------
void
Mesh_MOAB::node_get_cells(const Entity_ID nodeid, const Parallel_type ptype,
                          Entity_ID_List* cellids) const
{
  ...
  for (int i = 0; i < nids; ++i) {
    ...
    if (ptype == Parallel_type::ALL) { cellids->push_back(lid); }
    if (ptype == Parallel_type::OWNED) {
      mbcomm_->get_pstatus(ids[i], pstatus);
      if ((pstatus & PSTATUS_NOT_OWNED) == 0) { cellids->push_back(lid); }
    } else if (ptype == Parallel_type::GHOST) {
      mbcomm_->get_pstatus(ids[i], pstatus);
      if ((pstatus & PSTATUS_NOT_OWNED) == 1) { cellids->push_back(lid); }
    }
  }
}
...
//--------------------------------------------------------------------
void
Mesh_MOAB::node_get_faces(const Entity_ID nodeid, const Parallel_type ptype,
...
//--------------------------------------------------------------------
void
Mesh_MOAB::node_get_cell_faces(const Entity_ID nodeid, const Entity_ID cellid,
                               const Parallel_type ptype,
                               Entity_ID_List* faceids) const
{
  throw std::exception();
...
//--------------------------------------------------------------------
void
Mesh_MOAB::face_get_cells_internal_(const Entity_ID faceid,
                                    const Parallel_type ptype,
                                    Entity_ID_List* cellids) const
{
  int result;
  ...
  result =
    mbcore_->get_adjacencies(&face, 1, celldim, true, fcells, Core::UNION);
  if (result != MB_SUCCESS) {
...
//--------------------------------------------------------------------
void
Mesh_MOAB::cell_get_node_adj_cells(const Entity_ID cellid,
                                   const Parallel_type ptype,
...
Mesh_MOAB::init_cell_map()
{
  ...
  if (!serial_run) {
    ...
    cell_map_wo_ghosts_ =
      Teuchos::rcp(new Map_type(-1, cell_gids, ncell, 0, get_comm()));
...
Mesh_MOAB::init_face_map()
{
  ...
  if (!serial_run) {
    ...
    nface = nowned;
    face_map_wo_ghosts_ =
      Teuchos::rcp(new Map_type(-1, face_gids, nface, 0, get_comm()));
    ...
    for (int f = 0; f < nface; ++f) {
      ...
      if (fcells.size() == 1) extface_gids[n_extface++] = face_gids[f];

      if (f == nowned - 1) n_extface_owned = n_extface;
    }

    extface_map_wo_ghosts_ = Teuchos::rcp(
      new Map_type(-1, extface_gids, n_extface_owned, 0, get_comm()));
    ...
    extface_map_wo_ghosts_->RemoteIDList(n_extface_notowned,
                                         &(extface_gids[n_extface_owned]),
                                         pr_id.data(),
                                         lc_id.data());
    ...
    extface_map_w_ghosts_ =
      Teuchos::rcp(new Map_type(-1, extface_gids, n_extface, 0, get_comm()));
  } else {
    // all faces
    nface = AllFaces.size();
...
Mesh_MOAB::init_node_map()
{
  ...
  if (!serial_run) {
    ...
    node_map_wo_ghosts_ =
      Teuchos::rcp(new Map_type(-1, vert_gids, nvert, 0, get_comm()));
...
//--------------------------------------------------------------------
Map_ptr_type
Mesh_MOAB::cell_map(bool include_ghost) const
{
  if (serial_run)
...
//--------------------------------------------------------------------
Map_ptr_type
Mesh_MOAB::face_map(bool include_ghost) const
{
  if (serial_run)
...
//--------------------------------------------------------------------
Map_ptr_type
Mesh_MOAB::node_map(bool include_ghost) const
{
  if (serial_run)
...
//--------------------------------------------------------------------
Map_ptr_type
Mesh_MOAB::exterior_face_map(bool include_ghost) const
{
  if (serial_run)
...
//--------------------------------------------------------------------
Parallel_type
Mesh_MOAB::entity_get_ptype(const Entity_kind kind, const Entity_ID entid) const
{
  moab::EntityHandle ent;
  unsigned char pstatus;
  ...
  mbcomm_->get_pstatus(ent, pstatus);
  return ((pstatus & PSTATUS_NOT_OWNED) == 1) ? Parallel_type::GHOST :
                                                Parallel_type::OWNED;
}
...
//--------------------------------------------------------------------
Cell_type
Mesh_MOAB::cell_get_type(const Entity_ID cellid) const
{
...
//--------------------------------------------------------------------
std::string
Mesh_MOAB::internal_name_of_set(
...
//--------------------------------------------------------------------
int
Mesh_MOAB::deform(const std::vector<double>& target_cell_volumes_in,
