...
void
Mesh_MSTK::init_mesh_from_file_(const std::string& filename, const Partitioner_type partitioner)
{
  int ok = 0;
  ...
  if (filename.find(".exo") != std::string::npos) { // Exodus file
    ...
    mesg_stream << "Cannot identify file type from extension of input file " << filename
                << " on processor " << myprocid << std::endl;
...
Mesh_MSTK::Mesh_MSTK(const std::string& filename,
  ...
  : Mesh(comm, gm, plist, request_faces, request_edges),
    ...
    extnode_map_w_ghosts_(nullptr),
    extnode_map_wo_ghosts_(nullptr),
...
#else
    ok =
      ok & MSTK_Mesh_Distribute(globalmesh, &mesh_, &topo_dim, ring, with_attr, method, mpicomm_);
...
Mesh_MSTK::Mesh_MSTK(const Teuchos::RCP<const Mesh>& parent_mesh,
                     ...
                     const bool request_edges)
  : Mesh(comm,
         gm == Teuchos::null ? parent_mesh->geometric_model() : gm,
...
//---------------------------------------------------------
void
Mesh_MSTK::extract_mstk_mesh(List_ptr src_entities,
...
//---------------------------------------------------------
Entity_ID
Mesh_MSTK::entity_get_parent(const Entity_kind kind, const Entity_ID entid) const
{
  int ival;
  double rval;
  void* pval = nullptr;
...
Mesh_MSTK::init_cell_map()
{
  ...
  if (!serial_run) {
    ...
    idx = 0;
    i = 0;
    while ((ment = MSet_Next_Entry(OwnedCells, &idx))) cell_gids[i++] = MEnt_GlobalID(ment) - 1;
    ...
    idx = 0;
    while ((ment = MSet_Next_Entry(GhostCells, &idx))) cell_gids[i++] = MEnt_GlobalID(ment) - 1;
    ...
    idx = 0;
    i = 0;
    while ((ment = MSet_Next_Entry(OwnedCells, &idx))) cell_gids[i++] = MEnt_ID(ment) - 1;
...
Mesh_MSTK::init_face_map()
{
  ...
  if (!serial_run) {
    ...
    for (int k = 0; k < n_extface; k++) { global_id_ghosted[k] = extface_gids[k]; }
    ...
    extface_map_w_ghosts_ =
      new Epetra_Map(-1, n_extface_w_ghosts, global_id_ghosted.data(), 0, *comm_);
...
Mesh_MSTK::init_edge_map()
{
  ...
  if (!serial_run) {
    ...
    idx = 0;
    while ((ment = MSet_Next_Entry(NotOwnedEdges, &idx))) edge_gids[i++] = MEnt_GlobalID(ment) - 1;
...
Mesh_MSTK::init_node_map()
{
  ...
  if (!serial_run) {
    ...
    while ((ment = MSet_Next_Entry(OwnedVerts, &idx))) {
      ...
      if (is_boundary_node_(ment)) extnode_gids[j++] = gid - 1;
    }
    n_extnode = j;
    nnode = nowned;
    ...
    while ((ment = MSet_Next_Entry(NotOwnedVerts, &idx))) {
      int gid = MEnt_GlobalID(ment);
      if (is_boundary_node_(ment)) gl_id[nnotowned_bnd++] = gid - 1;
    }
    ...
    // Add to maping only external faces (which belong to local mapping on other processors
    for (int k = 0; k < nnotowned_bnd; k++) {
      if (pr_id[k] >= 0) { n_extnode_w_ghosts++; }
    }

    std::vector<int> global_id_ghosted(n_extnode_w_ghosts);
    for (int k = 0; k < n_extnode; k++) { global_id_ghosted[k] = extnode_gids[k]; }
    ...
    extnode_map_w_ghosts_ =
      new Epetra_Map(-1, n_extnode_w_ghosts, global_id_ghosted.data(), 0, *comm_);
...
//---------------------------------------------------------
void
Mesh_MSTK::post_create_steps_(const bool request_faces, const bool request_edges)
{
  label_celltype();
...
#else
      Errors::Message mesg("Mesh contains a degenerate edge. MSTK version 3.0.0 or later is "
...
int
Mesh_MSTK::generate_regular_mesh(Mesh_ptr mesh,
                                 double x0,
                                 double y0,
                                 double z0,
                                 double x1,
