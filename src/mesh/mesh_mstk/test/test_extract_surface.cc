...
TEST(Extract_Surface_MSTK1)
{
  ...
  int ncells_surf = surfmesh.get_set_size(setnames[0],Amanzi::AmanziMesh::Entity_kind::CELL,Amanzi::AmanziMesh::Parallel_type::OWNED);
  ncells_surf += surfmesh.get_set_size(setnames[1],Amanzi::AmanziMesh::Entity_kind::CELL,Amanzi::AmanziMesh::Parallel_type::OWNED);
  //  int ncells_surf = surfmesh.getNumEntities(Amanzi::AmanziMesh::Entity_kind::CELL,Amanzi::AmanziMesh::Parallel_type::OWNED);
  CHECK_EQUAL(18,ncells_surf);
      
  // Number of "faces" (edges) in surface mesh

  int nfaces_surf = surfmesh.getNumEntities(Amanzi::AmanziMesh::Entity_kind::FACE,Amanzi::AmanziMesh::Parallel_type::OWNED);
  CHECK_EQUAL(45,nfaces_surf);

  // Number of nodes in surface mesh

  int nnodes_surf = surfmesh.getNumEntities(Amanzi::AmanziMesh::Entity_kind::NODE,Amanzi::AmanziMesh::Parallel_type::OWNED);
...
struct test_fixture {
  ...
  Teuchos::RCP<Amanzi::AmanziMesh::MeshFramework> extract(bool flatten) {
    // get the surface set
    std::vector<std::string> setnames;
    setnames.push_back(std::string("Top Surface"));
    Amanzi::AmanziMesh::Entity_ID_List ids1;
    ...
    surfmesh = Teuchos::rcp(new Amanzi::AmanziMesh::Mesh_MSTK(mesh,ids1,Amanzi::AmanziMesh::Entity_kind::FACE, flatten, mesh->get_comm()));
    return surfmesh;
  }
  ...
  Teuchos::RCP<Amanzi::AmanziMesh::MeshFramework> surfmesh;
};
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_BASIC_EXTRACTION) {
  ...
  // parent ids are the getEntityGIDs of the parent mesh's corresponding entities
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_SIDE_FACES) {
  ...
  //
  bool is_valid = surfmesh->valid_set_name("Side Surface", Amanzi::AmanziMesh::Entity_kind::FACE);
  CHECK(is_valid);
  Amanzi::AmanziMesh::Entity_ID_List setents;
  // In this case, a face set in the parent becomes a face set in the surface
  surfmesh->getSetEntities("Side Surface",Amanzi::AmanziMesh::Entity_kind::FACE,
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_SIDE_FACES_NO_CELLS) {
  ...
  // cells.  This is either valid, and getSetEntities() does not throw but
  // returns an empty list, or is not valid.
  bool is_valid = surfmesh->valid_set_name("Side Surface", Amanzi::AmanziMesh::Entity_kind::CELL);
  if (is_valid) {  
    Amanzi::AmanziMesh::Entity_ID_List setents;
    surfmesh->getSetEntities("Side Surface",Amanzi::AmanziMesh::Entity_kind::CELL,
            Amanzi::AmanziMesh::Parallel_type::OWNED,&setents);
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_TOP_FACES) {
  ...
  bool is_valid = surfmesh->valid_set_name("Top Surface", Amanzi::AmanziMesh::Entity_kind::CELL);
  CHECK(is_valid);
  Amanzi::AmanziMesh::Entity_ID_List setents;
  surfmesh->getSetEntities("Top Surface",Amanzi::AmanziMesh::Entity_kind::CELL,
                            Amanzi::AmanziMesh::Parallel_type::OWNED,&setents);
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_TOP_FACES_NO_FACES) {
  ...
  // valid, and getSetEntities() does not throw but returns an empty list, or
  // is not valid.
  bool is_valid = surfmesh->valid_set_name("Side Surface", Amanzi::AmanziMesh::Entity_kind::FACE);
  if (is_valid) {  
    Amanzi::AmanziMesh::Entity_ID_List setents;
    surfmesh->getSetEntities("Top Surface",Amanzi::AmanziMesh::Entity_kind::FACE,
            Amanzi::AmanziMesh::Parallel_type::OWNED,&setents);
    CHECK_EQUAL(0, setents.size());
  }
}
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_TOP_CELLS) {
  ...
  bool is_valid = surfmesh->valid_set_name("Region 1", Amanzi::AmanziMesh::Entity_kind::CELL);
  CHECK(is_valid);
  Amanzi::AmanziMesh::Entity_ID_List setents;
  surfmesh->getSetEntities("Region 1",Amanzi::AmanziMesh::Entity_kind::CELL,
                            Amanzi::AmanziMesh::Parallel_type::OWNED,&setents);
  CHECK_EQUAL(9, setents.size());
}
...
TEST_FIXTURE(test_fixture, Extract_Surface_MSTK3_BOTTOM_CELLS) {
  ...
  bool is_valid = surfmesh->valid_set_name("Region 2", Amanzi::AmanziMesh::Entity_kind::CELL);
  if (is_valid) {  
    Amanzi::AmanziMesh::Entity_ID_List setents;
    surfmesh->getSetEntities("Region 2",Amanzi::AmanziMesh::Entity_kind::CELL,
            Amanzi::AmanziMesh::Parallel_type::OWNED,&setents);
    CHECK_EQUAL(0, setents.size());
  }
}
...
TEST_FIXTURE(test_fixture, Extract_Flatten_Surface_MSTK3_BAD_NAME) {
  ...
  // but there is no intersection.  This is either valid, but doesn't throw and
  // returns an empty set, or is not valid.  Either is OK?
  bool is_valid = surfmesh->valid_set_name("Region Not Here", Amanzi::AmanziMesh::Entity_kind::CELL);
  CHECK(!is_valid);
  is_valid = surfmesh->valid_set_name("Region Not Here", Amanzi::AmanziMesh::Entity_kind::FACE);
  CHECK(!is_valid);
}
