...
TEST(MSTK_HEX_2x2x1)
{
  ...
  Kokkos::View<Amanzi::AmanziMesh::Entity_ID*> faces("", 6), cnodes("", 8),
    fnodes("", 6);
  Kokkos::View<int*> facedirs("", 6);
  Kokkos::View<Amanzi::AmanziGeometry::Point*> fcoords("", 4), ccoords("", 8);
  ...
  Amanzi::AmanziMesh::Entity_ID cfstd[6][4] = {
  ...
  Teuchos::RCP<Amanzi::AmanziMesh::Mesh> mesh(
    new Amanzi::AmanziMesh::Mesh_MSTK("test/hex_2x2x1_ss.exo", comm));
  ...
  for (i = 0; i < nc; i++) {
    ...
    for (j = 0; j < 6; j++) {
      ...
      for (k = 0; k < 4; k++) expfacenodes[k] = cnodes(cfstd[j][k]);
