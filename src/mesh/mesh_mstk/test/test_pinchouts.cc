...
TEST(ELIM_DEGEN_INLINE_PARTITION)
{
  ...
  if (mesh_plist.isSublist("surface mesh")) {
    ...
    if (surface_plist.isParameter("surface sideset name")) {
      setnames.push_back(
        surface_plist.get<std::string>("surface sideset name"));
    } else if (surface_plist.isParameter("surface sideset names")) {
      setnames =
        surface_plist.get<Teuchos::Array<std::string>>("surface sideset names")
          .toVector();
    } else {
      Errors::Message message(
        "Surface mesh sublist missing parameter \"surface sideset names\".");
      Exceptions::amanzi_throw(message);
    }

    if (mesh->manifold_dimension() == 3) {
      surface3D_mesh =
        meshfactory.create(mesh, setnames, Amanzi::AmanziMesh::FACE, false);
      surface_mesh =
        meshfactory.create(mesh, setnames, Amanzi::AmanziMesh::FACE, true);
    } else {
      surface3D_mesh = mesh;
      surface_mesh =
        meshfactory.create(mesh, setnames, Amanzi::AmanziMesh::CELL, true);
    }
    ...
    if (surf_verify) {
      ...
      if (num_procs == 1) {
        ...
        if (status == 0) {
          std::cout << "Mesh Audit confirms that surface mesh is ok"
                    << std::endl;
        } else {
          Errors::Message msg(
            "Mesh Audit could not verify correctness of surface mesh.");
          Exceptions::amanzi_throw(msg);
        }
      } else {
        std::ostringstream ofile;
        ofile << "surf_mesh_audit_" << std::setfill('0') << std::setw(4) << rank
              << ".txt";
        std::ofstream ofs(ofile.str().c_str());
        if (rank == 0)
          std::cout << "Writing Surface Mesh Audit output to " << ofile.str()
                    << ", etc." << std::endl;
      ...
      std::cout << "Verifying the surface mesh using the internal MSTK check..."
                << std::endl;
