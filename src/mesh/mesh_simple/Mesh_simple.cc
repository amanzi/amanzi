...
//---------------------------------------------------------
std::size_t
Mesh_simple::getNumEntities(AmanziMesh::Entity_kind kind, AmanziMesh::Parallel_type ptype) const
{
  switch (kind) {
...
//---------------------------------------------------------
void
Mesh_simple::getCellFacesAndDirs(const AmanziMesh::Entity_ID cellid,
...
//---------------------------------------------------------
void
Mesh_simple::getFaceNodes(AmanziMesh::Entity_ID face, AmanziMesh::Entity_ID_List& nodeids) const
{
  unsigned int offset = (unsigned int)4 * face;
  Kokkos::resize(nodeids, 4);
  for (int i = 0; i < 4; i++) { nodeids[i] = face_to_node_[offset + i]; }
}
...
//---------------------------------------------------------
void
Mesh_simple::getFaceEdgesAndDirs(const Entity_ID faceid,
...
//---------------------------------------------------------
void
Mesh_simple::getEdgeNodes(const Entity_ID edgeid, Entity_ID_List& nodes) const
{
...
//---------------------------------------------------------
void
Mesh_simple::getNodeFaces(const AmanziMesh::Entity_ID nodeid,
...
//---------------------------------------------------------
void
Mesh_simple::getFaceCells(const AmanziMesh::Entity_ID faceid,
                          const AmanziMesh::Parallel_type ptype,
                          AmanziMesh::Entity_ID_List& cellids) const
{
  ...
  int cellids_ct = 0;
  if (face_to_cell_[offset] != -1) cellids[cellids_ct++] = face_to_cell_[offset];
  if (face_to_cell_[offset + 1] != -1) cellids[cellids_ct++] = face_to_cell_[offset + 1];
