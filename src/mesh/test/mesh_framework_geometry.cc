...
//
// Tests mesh frameworks on a variety of meshes, ensuring that geometric and
// topologic information is set up correctly.
//
...
TEST(MESH_FRAMEWORK_GEOMETRY_PLANAR)
{
  // only works in MSTK
...
TEST(MESH_FRAMEWORK_GEOMETRY_1BOX_GENERATED)
{
  // only works in MSTK
  std::vector<Framework> frameworks;
  if (framework_enabled(Framework::MSTK)) {
...
TEST(MESH_FRAMEWORK_GEOMETRY_1BOX_EXO)
{
...
TEST(MESH_FRAMEWORK_GEOMETRY_2BOX)
{
...
TEST(MESH_FRAMEWORK_GEOMETRY_3BOX_EXO)
{
  // only works in MSTK or MOAB
  std::vector<Framework> frameworks;
  if (framework_enabled(Framework::MSTK)) {
...
TEST(MESH_FRAMEWORK_GEOMETRY_FRACTURE_EXO)
{
  // only works in MSTK or MOAB
  std::vector<Framework> frameworks;
  if (framework_enabled(Framework::MSTK)) {
