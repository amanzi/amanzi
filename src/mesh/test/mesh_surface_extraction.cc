...
TEST(MESH_SURFACE_EXTRACTION_GENERATED)
{
  ...
  for (const auto& frm : frameworks) {
    ...
    auto surface_framework_mesh = fac.create(parent_mesh,
            top_faces, AmanziMesh::Entity_kind::FACE, true);

    // make a MeshCache
    auto mesh = Teuchos::rcp(new Mesh(surface_framework_mesh, Teuchos::rcp(new AmanziMesh::MeshFrameworkAlgorithms()), Teuchos::null));
...
TEST(MESH_SURFACE_EXTRACTION_GENERATED_EXTRACTED_MANIFOLD)
{
  ...
  for (const auto& frm : frameworks) {
    ...
    auto parent_mesh_cache = Teuchos::rcp(new Mesh(parent_mesh, Teuchos::rcp(new AmanziMesh::MeshFrameworkAlgorithms()), Teuchos::null)); 
