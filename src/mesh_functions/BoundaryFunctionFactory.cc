...
BoundaryFunctionFactory::CreateWithFunction(std::string list_name, std::string function_name) const {
  ...
  if (plist_.isParameter(list_name)) {
    if (plist_.isSublist(list_name)) {
      ...
      mstream << "BoundaryFunctionFactory: " << list_name << " sublist error: not a sublist.";
...
****************************************************************** */
Teuchos::RCP<Functions::BoundaryFunction>
BoundaryFunctionFactory::CreateWithoutFunction(std::string list_name) const {
  Teuchos::RCP<Functions::BoundaryFunction> bc =
      Teuchos::rcp(new Functions::BoundaryFunction(mesh_));
...
****************************************************************** */
void BoundaryFunctionFactory::ProcessListWithFunction_(const Teuchos::ParameterList& list,
...
****************************************************************** */
void BoundaryFunctionFactory::ProcessSpecWithFunction_(const Teuchos::ParameterList& list,
        std::string function_name,
        const Teuchos::RCP<Functions::BoundaryFunction>& bc) const {
  std::stringstream m;
...
* Process Dirichet BC (pressure), step 2.
****************************************************************** */
void BoundaryFunctionFactory::ProcessListWithoutFunction_(const Teuchos::ParameterList& list,
        const Teuchos::RCP<Functions::BoundaryFunction>& bc) const {
  // Iterate through the BC specification sublists in the list.
  // All are expected to be sublists of identical structure.
  for (Teuchos::ParameterList::ConstIterator i = list.begin(); i != list.end(); ++i) {
...
* Process Dirichet BC (pressure), step 3.
****************************************************************** */
void BoundaryFunctionFactory::ProcessSpecWithoutFunction_(const Teuchos::ParameterList& list,
        const Teuchos::RCP<Functions::BoundaryFunction>& bc) const {
  Errors::Message m;
  std::vector<std::string> regions;
