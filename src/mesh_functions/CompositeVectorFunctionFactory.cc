                              ...
                              const CompositeVectorSpace& sample)
{
    ...
    if (plist.isSublist(name)) {
      ...
      if (sublist.isParameter("region")) {
        ...
        if (sublist.isType<Teuchos::Array<std::string>>("regions")) {
          regions =
            sublist.get<Teuchos::Array<std::string>>("regions").toVector();
        } else {
          Errors::Message msg;
          msg << "CompositeVectorFunctionFactory \"" << plist.name() << "("
              << name << ")"
      ...
      if (sublist.isParameter("component")) {
        ...
        if (sublist.isType<Teuchos::Array<std::string>>("components")) {
          components =
            sublist.get<Teuchos::Array<std::string>>("components").toVector();
        } else {
          Errors::Message msg;
          msg << "CompositeVectorFunctionFactory \"" << plist.name() << "("
              << name << ")"
  ...
  // create the function
  return Teuchos::rcp(
    new CompositeVectorFunction(mesh_func, componentname_list));
};
