...
computeMeshFunction(const MultiFunction& f, double time, Patch<Device>& p)
{
  ...
  Kokkos::View<double**,Device> txyz("txyz", mesh->space_dimension(), p.size());
  ...
  Kokkos::View<int*,Device> ids("ids", ids_list.size());
  {
                    ...
                    double time, const MultiPatchSpace& mp, CompositeVector& cv)
{
  ...
  for (int i=0; i!=mp.size(); ++i) {
    std::string compname = AmanziMesh::entity_kind_string(mp[i].entity_kind);
    if (cv.HasComponent(compname)) {
      Patch<Device> p(mp[i]);
      computeMeshFunction<Device>(*f[i], time, p);
      patchToCompositeVector<Device>(p, compname, cv);
    }
  }
}
