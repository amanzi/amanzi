...
TEST_FIXTURE(reference_mesh, values1)
{
  ...
  face_list = mesh->getSetEntities(RIGHT, Entity_kind::FACE, Parallel_type::ALL);
  for (Entity_ID_List::iterator f = face_list.begin(); f != face_list.end(); ++f)
    CHECK_EQUAL(1.0, bf.find(*f)->second);

  face_list = mesh->getSetEntities(FRONT, Entity_kind::FACE, Parallel_type::ALL);
  for (Entity_ID_List::iterator f = face_list.begin(); f != face_list.end(); ++f)
    CHECK_EQUAL(2.0, bf.find(*f)->second);

  face_list = mesh->getSetEntities(BACK, Entity_kind::FACE, Parallel_type::ALL);
  for (Entity_ID_List::iterator f = face_list.begin(); f != face_list.end(); ++f)
