...
TEST_FIXTURE(reference_mesh, basic)
{
  ...
  bf.DefineFromString(RIGHT, f1);
  CHECK_EQUAL(4, bf.size());
...
TEST_FIXTURE(reference_mesh, values1)
{
  ...
  Teuchos::RCP<Function> f3(new ConstantFunction(3.0));
  bf.DefineFromString(RIGHT, f1);
  bf.DefineFromString(FRONT, f2);
  bf.DefineFromString(BACK,  f3);
  CHECK_EQUAL(12, bf.size());
...
TEST_FIXTURE(reference_mesh, bad_input)
{
  ...
  CHECK_THROW(bf.DefineFromString(INVALID, f), Errors::Message);
  bf.DefineFromString(RIGHT, f);

  //bf.Define(RIGHT, f); // overlapping definition
  CHECK_THROW(bf.DefineFromString(RIGHT, f), Errors::Message);
}
