...
class DomainFunction : public MaterialMeshFunction {
 public:
  ...
  // access specs
  const std::map<AmanziMesh::Entity_ID, double>
  get_ids(const AmanziMesh::Entity_kind& kind)
  {
...
TEST(MESH2D)
{
  ...
  Teuchos::RCP<MeshFunction::Domain> domain =
    Teuchos::rcp(new MeshFunction::Domain(rgns, kind));
  Teuchos::RCP<MeshFunction::Spec> spec =
    Teuchos::rcp(new MeshFunction::Spec(domain, f));

  {
    ...
    int n(0);
    for (std::map<AmanziMesh::Entity_ID, double>::const_iterator it =
           ids.begin();
         it != ids.end();
         ++it) {
