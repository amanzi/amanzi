...
bool
reset_info_compfunc(const std::pair<double, double>& x, const std::pair<double, double>& y)
{
...
****************************************************************** */
void
CycleDriver::Init_PK(int time_pr_id)
{
  PKFactory pk_factory;

  Teuchos::RCP<Teuchos::ParameterList> time_periods_list =
    Teuchos::sublist(coordinator_list_, "time periods", true);

  std::ostringstream ss;
  ss << time_pr_id;

  std::string tp_list_name = "TP " + ss.str();
  Teuchos::RCP<Teuchos::ParameterList> tp_list =
    Teuchos::sublist(time_periods_list, tp_list_name.data(), true);
  ...
  if (glist_->isSublist("observation data")) {
    ...
    if (coordinator_list_->isParameter("component names")) {
      std::vector<std::string> names =
        coordinator_list_->get<Teuchos::Array<std::string>>("component names").toVector();
      std::vector<double> mol_masses =
        coordinator_list_->get<Teuchos::Array<double>>("component molar masses").toVector();
  ...
  for (auto mesh = S_->mesh_begin(); mesh != S_->mesh_end(); ++mesh) {
    if (mesh->first == "surface_3d") {
      ...
      if (glist_->isSublist(fail_plist_name)) {
        ...
        Teuchos::RCP<Visualization> fail_vis = Teuchos::rcp(new Visualization(fail_vis_plist));
...
CycleDriver::ReportMemory()
{
  ...
  if (vo_->os_OK(Teuchos::VERB_LOW)) {
    ...
    *vo_->os() << "  Maximum per core:   " << std::setw(7) << max_doubles_count * 8 / 1024 / 1024
               << " MBytes" << std::endl;
    *vo_->os() << "  Minimum per core:   " << std::setw(7) << min_doubles_count * 8 / 1024 / 1024
               << " MBytes" << std::endl;
    *vo_->os() << "  Total:              " << std::setw(7) << global_doubles_count * 8 / 1024 / 1024
               << " MBytes" << std::endl;
  }
}
...
CycleDriver::get_dt(bool after_failure)
{
  ...
  for (it = reset_info_.begin(), it_max = reset_max_.begin(); it != reset_info_.end();
    ...
    if (S_->get_time() == it->first) {
      ...
      if (reset_max_.size() > 0) it_max = reset_max_.erase(it_max);
  ...
  if (S_->get_time() > 0) {
    if (dt / S_->get_time() < 1e-14) {
      Errors::Message message(
        "CycleDriver: error, timestep too small with respect to current time");
      Exceptions::amanzi_throw(message);
    }
  }
...
CycleDriver::Advance(double dt)
{
  ...
  if (tp_end_[time_period_id_] == tp_start_[time_period_id_]) advance = false;
...
****************************************************************** */
void
CycleDriver::Observations(bool force, bool integrate)
{
...
****************************************************************** */
void
CycleDriver::Visualize(bool force, const Tag& tag)
{
  bool dump = force;
  if (!dump) {
    for (auto vis = visualization_.begin(); vis != visualization_.end(); ++vis) {
...
****************************************************************** */
void
CycleDriver::WriteCheckpoint(double dt, bool force)
{
...
void
CycleDriver::WriteWalkabout(bool force)
{
...
CycleDriver::Go()
{
  ...
  if (!restart_requested_) { // No restart
    ...
    if (position == TIME_PERIOD_END)
      if (time_period_id_ > 0) time_period_id_--;
