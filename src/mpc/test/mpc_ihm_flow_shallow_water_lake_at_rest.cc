...
double H_inf = 11.0; // Total depth

// ---- ---- ---- ---- ---- ---- ---- ----
// Exact Solution
// ---- ---- ---- ---- ---- ---- ---- ----
void equilibrium_exact(double x, double y, double &ht, double &u, double &v)
{
  ht = H_inf;
  u = 0;
  v = 0;
...
void equilibrium_exact_field(Teuchos::RCP<const Amanzi::AmanziMesh::Mesh> mesh,
...
TEST(MPC_DRIVER_IHM_FLOW_SHALLOW_WATER) {
...
using namespace Amanzi::AmanziGeometry;
  ...
  std::string xmlInFileName = "test/mpc_ihm_flow_shallow_water_lake_at_rest.xml";
...
//       mesh, "TopSurface", AmanziMesh::FACE, comm, gm, mesh_list, true, true, true));
  ...
  equilibrium_exact_field(mesh_surface, ht_ex, vel_ex);
        
  double err_max, err_L1, hmax;
        
  error(mesh_surface, ht_ex, vel_ex, ht, vel, err_max, err_L1, hmax);
