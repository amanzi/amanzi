...
TEST(MPC_TWO_PHASE)
{
  ...
  // read parameter list
  std::string xmlFileName = "test/mpc_two_phasePH_sink.xml";
  ...
  CHECK(state[(int)TSPH_t::RGN][0] == 4);
  CHECK(state[(int)TSPH_t::RGN][1] == 4);

  int itrs(0);
  double t(0.0), dt(5.0), t1(160.0e+3);
  while (t < t1) {
