...
class BoundaryFluxFn {
 public:
  ...
  BoundaryFluxFn(const double trans_f, const double lambda, double cell_p,
                 double bc_flux, double g_flux, int dir, double patm,
                 Teuchos::RCP<const Model> model, NonlinFunc nonlinfunc)
    : trans_f_(trans_f),
      lambda_(lambda),
      cell_p_(cell_p),
      bc_flux_(bc_flux),
...
class BoundaryFaceSolver {
 public:
  ...
  BoundaryFaceSolver(double trans_f, double g_f, double cell_val, double lambda,
                     double bnd_flux, int dir, double patm, double min_val,
                     double max_val, double eps,
  ...
  double SolveBisection(double face_val, double min_val, double max_val,
                        BoundaryFluxFn<Model>& func, Tol_& tol,
                        boost::uintmax_t max_it, boost::uintmax_t& actual_it);
  ...
  double FaceValue()
  {
    ...
    return SolveBisection(
      lambda_, min_val_, max_val_, *func_, tol, max_it, actual_it);
  }
...
template <class Model>
double
BoundaryFaceSolver<Model>::SolveBisection(double face_val, double min_val,
                                          double max_val,
                                          BoundaryFluxFn<Model>& func,
                                          Tol_& tol, boost::uintmax_t max_it,
                                          boost::uintmax_t& actual_it)
{
...
#if DEBUG_FLAG
  std::cout << "STaRT interval: (" << min_val << " " << max_val
            << "),  fval=" << face_val << "\n";
#endif
  if (res > 0.0) {
    left = face_val;
    ...
    rres = func(right);
#if DEBUG_FLAG
    std::cout << "---set right:      " << left << " (" << lres << "), " << right
              << " (" << rres << ")\n";
#endif
    while (rres > 0.0) {
    ...
    lres = func(left);
#if DEBUG_FLAG
    std::cout << "---set left:       " << left << " (" << lres << "), " << right
              << " (" << rres << ")\n";
#endif
    while (lres < 0.0) {
...
#if DEBUG_FLAG
  std::cout << "+++bracket (func): " << left << " (" << lres << "), " << right
            << " (" << rres << ")\n";
#endif
  ...
  if (tol(left, right)) {
    ...
    result = boost::math::tools::toms748_solve(
      func, left, right, lres, rres, tol, actual_it);
    // if (actual_it >= max_it) {
