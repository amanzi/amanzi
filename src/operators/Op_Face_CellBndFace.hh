...
class Op_Face_CellBndFace : public Op {
 public:
         ...
         mesh)
  {
    ...
    for (int f = 0; f != nfaces_owned; ++f) { A.set_shape(f, 2, 2); }
    A.Init();
  }
  ...
  virtual void SumLocalDiag(CompositeVector& X) const
  {
    ...
    Kokkos::parallel_for(
      "Op_Face_CellBndFace::GetLocalDiagCopy loop 1", A.size(), KOKKOS_LAMBDA(const int f) {
        auto cells = mesh_->getFaceCells(f);
        auto lA = A[f];
        Xc(cells(0), 0) += lA(0, 0);
        if (cells.extent(0) > 1) { Kokkos::atomic_add(&Xc(cells(1), 0), lA(1, 1)); }
      });

    auto Xbf = X.viewComponent("boundary_face", true);
    auto import_same_face = mesh->getBoundaryFaceImporter().getNumSameIDs();
    Kokkos::parallel_for(
    ...
    auto import_permute_face =
      mesh->getBoundaryFaceImporter().getPermuteFromLIDs_dv().view<DefaultDevice>();
    Kokkos::parallel_for(
  ...
  virtual void Rescale(const CompositeVector& scaling)
  {
    if ((scaling.hasComponent("cell")) && (scaling.hasComponent("boundary_face"))) {
      { // context for views
        ...
        Kokkos::parallel_for(
          "Op_Face_CellBndFace::Rescale loop 1", A.size(), KOKKOS_LAMBDA(const int f) {
            auto cells = mesh_->getFaceCells(f);
            auto lA = A[f];
...

      {
        ...
        Kokkos::parallel_for(
          "Op_Face_CellBndFace::Rescale loop 2", import_same_face, KOKKOS_LAMBDA(const int bf) {
            auto lA = A[bf];
            lA(0, 1) *= s_bnd(bf, 0);
            lA(1, 1) *= s_bnd(bf, 0);
          });
        ...
        Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(const int bf_offset) {
            int f = import_permute_face(bf_offset);
            auto lA = A[f];
            lA(0, 1) *= s_bnd(bf_offset + import_same_face, 0);
            lA(1, 1) *= s_bnd(bf_offset + import_same_face, 0);
          });
      }
    }
  }
