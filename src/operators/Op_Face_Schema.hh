/*
  ...
  length nfaces and with dofs given by two schemas.
*/

#ifndef AMANZI_OP_FACE_SCHEMA_HH_
#define AMANZI_OP_FACE_SCHEMA_HH_
...
class Op_Face_Schema : public Op {
 public:
  Op_Face_Schema(const Schema& schema_row, const Schema& schema_col,
  ...
  virtual void ApplyTransposeMatrixFreeOp(const Operator* assembler,
          const CompositeVector& X, CompositeVector& Y) const {
    assembler->ApplyTransposeMatrixFreeOp(*this, X, Y);
  }

  virtual void SymbolicAssembleMatrixOp(const Operator* assembler,
          const SuperMap& map, GraphFE& graph,
          int my_block_row, int my_block_col, bool multi_domain) const {
    assembler->SymbolicAssembleMatrixOp(*this, map, graph, my_block_row, my_block_col, multi_domain);
  }
