...
assimilating more ``Op``s. An ``Operator`` knows how to Apply and Assemble all of its
local ``Op``s.

Typically the forward operator is applied using only local ``Op``s -- the inverse
operator typically requires assembling a matrix, which may represent the
entire operator or may be a Schur complement.  

In all ``Operator``s, ``Op``s, and matrices, a key concept is the schema.  A schema
...
X=Y).  A schema may also, in the case of ``Op``s, include information on the base
...
* We note that typically the ``Op`` object is not directly constructed; instead
  ...
  For instance, ``OperatorDiffusion`` is a class that constructs local ``Op``s that
  can be assembled into the global ``Operator`` to form a diffusion matrix.

* For simple operations (i.e. diffusion), ``Operator``s are not constructed
  directly.  Instead, a helper class that contains methods for creating and
  populating the ``Op``s within the ``Operator`` is created. This helper class can
  ...
  sure the ``Operator`` contains the necessary dofs and adds local ``Op``s to the
  ``Operator``'s list of ``Op``s.

Note on implementation for discretization/framework developers:

* ``Op``s work via a visitor pattern.  Assembly (resp Apply, BCs, and
...
* Ops can be shared by ``Operator``s. In combination with CopyShadowToMaster() and
