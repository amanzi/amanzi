                ...
                const typename AmanziMesh::Mesh::cEntity_ID_View& Y_ids)
{
  ...
  Kokkos::parallel_for(
    "OperatorUtils::LocalMatMultAdd", num_rows, KOKKOS_LAMBDA(const int& i) {
      double result = 0.0;
      Kokkos::parallel_reduce(
        "local_mat_mult_inner_reduction",
        X_ids.extent(0),
        KOKKOS_LAMBDA(const int& j, double& lsum) { lsum += mat(elem, j + i * num_cols) * X(X_ids(j), 0); },
        result);
