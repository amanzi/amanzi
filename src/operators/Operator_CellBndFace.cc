...
//   int lid_r[2];
//   int lid_c[2];
//   const std::vector<int>& cell_row_inds = map.viewGhostIndices(my_block_row, "cell", 0);
//   const std::vector<int>& cell_col_inds = map.viewGhostIndices(my_block_col, "cell", 0);
//   const std::vector<int>& bndface_row_inds = map.viewGhostIndices(my_block_row, "boundary_face", 0);
//   const std::vector<int>& bndface_col_inds = map.viewGhostIndices(my_block_col, "boundary_face", 0);
...
// {
//   int nsurf_cells = op.surf_mesh->getNumEntities(AmanziMesh::CELL, AmanziMesh::Parallel_kind::OWNED);
...
//     int f = op.surf_mesh->getEntityParent(AmanziMesh::CELL, sc);
//     int bf = mesh_->exterior_face_map(false).LID(mesh_->face_map(false).GID(f));
...
//                                             const CompositeVector& X, CompositeVector& Y) const
// {
//   int nsurf_faces = op.surf_mesh->getNumEntities(AmanziMesh::FACE, AmanziMesh::Parallel_kind::OWNED);
...
//     op.surf_mesh->face_get_cells(sf, AmanziMesh::Parallel_kind::ALL, &cells);
//     int ncells = cells.size();

//     WhetStone::DenseVector v(ncells), av(ncells);
//     for (int n = 0; n != ncells; ++n) {
//       int f = op.surf_mesh->getEntityParent(AmanziMesh::CELL, cells[n]);
...
//   const std::vector<int>& face_row_inds = map.viewGhostIndices(my_block_row, "boundary_face", 0);
//   const std::vector<int>& face_col_inds = map.viewGhostIndices(my_block_col, "boundary_face", 0);
