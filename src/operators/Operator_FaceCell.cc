        ...
        for (int n = 0; n != cells.extent(0); ++n) {
          if (face_row_inds[mesh->getEntityParent(AmanziMesh::CELL,cells(n))] < nrows_local) {
            for (int m = 0; m != cells.extent(0); ++m) {
              proc_mat.sumIntoValues(face_row_inds(mesh->getEntityParent(AmanziMesh::CELL,cells(n))),
                      &face_col_inds(mesh->getEntityParent(AmanziMesh::CELL,cells(m))), 1,
                      &A_f(n,m), true, true);
            }
          } else {
            for (int m = 0; m != cells.extent(0); ++m) {
              offproc_mat.sumIntoValues(face_row_inds(mesh->getEntityParent(AmanziMesh::CELL,cells(n)))-nrows_local,
                      &face_col_inds(mesh->getEntityParent(AmanziMesh::CELL,cells(m))), 1,
                      &A_f(n,m), true, true);
            }
          }
