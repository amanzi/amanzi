 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::Init_(Teuchos::ParameterList& plist)
{
 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::SetDensity(
  ...
  const Teuchos::Ptr<const CompositeVector>& u)
{
  ...
  if (!exclude_primary_terms_) {
    ...
    const Epetra_MultiVector& gravity_face =
      *gravity_term_->ViewComponent("face", true);
    ...
    for (int c = 0; c != ncells_owned; ++c) {
      ...
      for (int n = 0; n != nfaces; ++n) {
        ...
        rhs_cell[0][c] -=
          dirs[n] * gravity_face[0][f] * (k_face.get() ? (*k_face)[0][f] : 1.0);
      }
    }
  }
}
 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::ApplyBCs(bool primary, bool eliminate,
                                     bool essential_eqn)
{
 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::UpdateFlux(
  ...
  const Teuchos::Ptr<CompositeVector>& darcy_mass_flux)
{
  ...
  if (Krel_face.get()) {
    flux.elementWiseMultiply(
      1.0, *Krel_face, *gravity_term_->ViewComponent("face", false), 1.0);
  } else {
 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::ComputeJacobianLocal_(
  ...
  double* pres, double* dkdp_cell, WhetStone::DenseMatrix& Jpp)
{
  ...
  const Teuchos::Ptr<Epetra_MultiVector> gravity_face =
    gravity_term_.get() ? gravity_term_->ViewComponent("face", true).ptr() :
                          Teuchos::null;
 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::UpdateFluxNonManifold(
  const Teuchos::Ptr<const CompositeVector>& u,
 ...
 ****************************************************************** */
void
PDE_DiffusionFVwithGravity::ComputeTransmissibility_(
  Teuchos::RCP<CompositeVector> g_cv)
{
  Epetra_MultiVector& trans_face =
    *transmissibility_->ViewComponent("face", true);
  ...
  for (int f = 0; f < nfaces_owned; f++) {
    ...
    double rho =
      is_scalar_ ?
        rho_ :
        (ncells == 1 ? (*rho_c)[0][cells[0]] :
                       ((*rho_c)[0][cells[0]] + (*rho_c)[0][cells[1]]) / 2.);
 ...
 ****************************************************************** */
double
PDE_DiffusionFVwithGravity::ComputeGravityFlux(int f) const
{
  const Epetra_MultiVector& gravity_face =
    *gravity_term_->ViewComponent("face", true);
