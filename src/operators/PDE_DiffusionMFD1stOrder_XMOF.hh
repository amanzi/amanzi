...
#ifndef AMANZI_OPERATOR_PDE_DIFFUSION_MFD_1ST_ORDER_XMOF_HH_
#define AMANZI_OPERATOR_PDE_DIFFUSION_MFD_1ST_ORDER_XMOF_HH_
...
namespace Amanzi {
  ...
  namespace Operators {
    ...
    class PDE_DiffusionMFD1stOrder_XMOF : public virtual PDE_Abstract {

    public:

      PDE_DiffusionMFD1stOrder_XMOF(Teuchos::ParameterList& plist, const Teuchos::RCP<Operator>& global_op) 
        : PDE_Abstract(plist, global_op)
        ,  factor_(1.0) {
        operator_type_ = OPERATOR_DIFFUSION_MFD_XMOF;
        InitDiffusion_(plist);
      }

      PDE_DiffusionMFD1stOrder_XMOF(Teuchos::ParameterList& plist, const Teuchos::RCP<const AmanziMesh::Mesh>& mesh)
