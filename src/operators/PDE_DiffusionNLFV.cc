...
PDE_DiffusionNLFV::Init_(Teuchos::ParameterList& plist)
{
  ...
  if (uwname == "none") {
    ...
    // } else {
    //   msg << "PDE_DiffusionNLFV: unknown or not supported upwind scheme
    //   specified."; Exceptions::amanzi_throw(msg);
  }

  // DEPRECATED INPUT -- remove this error eventually --etc
  if (plist.isParameter("newton correction")) {
    Errors::Message msg;
    msg << "PDE_DiffusionNLFV: DEPRECATED: \"newton correction\" has been "
           "removed in favor of \"Newton correction\"";
  ...
  if (jacobian == "none") {
    ...
    Errors::Message msg("PDE_DiffusionNLFV: \"true Jacobian\" not supported -- "
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFV::SetScalarCoefficient(
    ...
    for (int n = 0; n < nfaces; n++) {
      ...
      for (int i = 0; i < dim_; i++) {
        ...
        (*stencil_cells_[k + i])[f] =
          WhetStone::cell_get_face_adj_cell(*mesh_, c, faces[ids[i]]);
      }
    }
  }
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFV::UpdateMatrices(
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFV::UpdateMatricesNewtonCorrection(
  const Teuchos::Ptr<const CompositeVector>& flux,
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFV::OneSidedFluxCorrections_(int i0, const CompositeVector& u,
    ...
    for (int n = 0; n < nfaces; ++n) {
      ...
      for (int i = i0; i < dim_; ++i) {
        ...
        if (c3 >= 0) {
          ...
          sideflux +=
            tmp * (uc[0][c] - MapBoundaryValue_(f1, bc_value[f1])) * dir;
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFV::OneSidedWeightFluxes_(int i0, const CompositeVector& u,
                                         CompositeVector& flux_cv)
{
  // un-rolling composite vectors
  const Epetra_MultiVector& uc = *u.ViewComponent("cell", true);
    ...
    for (int n = 0; n < nfaces; ++n) {
      ...
      for (int i = i0; i < dim_; ++i) {
        ...
        if (c3 >= 0) {
          ...
          sideflux += flux_data[i + k2][f] *
                      (uc[0][c] - MapBoundaryValue_(f1, bc_value[f1]));
        } else if (bc_model[f1] == OPERATOR_BC_NEUMANN) {
          neumann_flux +=
            flux_data[i + k2][f] * bc_value[f1] * mesh_->face_area(f1);
        }
      }
  ...
  for (int f = 0; f < nfaces_owned; f++) {
    if (bc_model[f] != OPERATOR_BC_NONE) {
      ...
      if (bc_model[f] == OPERATOR_BC_DIRICHLET) {
        ...
        rhs_cell[0][c] -=
          (Aface(0, 0) / kf) * bc_value[f] * mesh_->face_area(f);
        Aface = 0.0;
      }
    }
  }
 ...
 ****************************************************************** */
int
PDE_DiffusionNLFV::OrderCellsByGlobalId_(
