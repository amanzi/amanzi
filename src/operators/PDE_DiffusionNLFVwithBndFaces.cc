...
* Initialization
****************************************************************** */
void PDE_DiffusionNLFVwithBndFaces::InitDiffusion_(Teuchos::ParameterList& plist)
{
...
void PDE_DiffusionNLFVwithBndFaces::InitStencils_()
{
  ...
  for (int f = 0; f < nfaces_owned; f++) {
    ...
    if (ncells == 2) {
      ...
      //   p = xc + d*Kn1;
      //   //std::cout<<"point "<<p<<" ***** "<<mesh_->face_centroid(f)<<"\n";
      // }else{
    ...
    const Teuchos::Ptr<const CompositeVector>& u)
{
  ...
  for (int f = 0; f < nfaces_owned; ++f) {
    ...
    if (ncells == 2) {
      ...
      // Aface(1, 1) = matrix[0][f];
      // Aface(0, 1) = -matrix[0][f];
      // Aface(1, 0) = -matrix[0][f];
      // if (bc_model[f] == OPERATOR_BC_NEUMANN){
        Aface(0, 0) = matrix[0][f];
...
****************************************************************** */
void PDE_DiffusionNLFVwithBndFaces::ApplyBCs(bool primary, bool eliminate)
{
  const std::vector<int>& bc_model = bcs_trial_[0]->bc_model();
  const std::vector<double>& bc_value = bcs_trial_[0]->bc_value();
...
int PDE_DiffusionNLFVwithBndFaces::NLTPFAContributions_(int f, double& tc1, double& tc2){
   ...
   //ASSERT(bc_model[f] == OPERATOR_BC_NEUMANN);
   mesh_->face_get_cells(f, AmanziMesh::USED, &cells);
   c = cells[0];
   tc1=0.;
   tc2=0.;
