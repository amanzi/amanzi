 ...
 ****************************************************************** */
void
PDE_DiffusionNLFVwithBndFaces::Init_(Teuchos::ParameterList& plist)
{
  ...
  if (global_op_ == Teuchos::null) {
    ...
    global_op_schema_ =
      OPERATOR_SCHEMA_DOFS_CELL | OPERATOR_SCHEMA_DOFS_BNDFACE;
    ...
    global_op_ =
      Teuchos::rcp(new Operator_CellBndFace(cvs, plist, global_op_schema_));
  ...
  if (jacobian == "none") {
    ...
    Errors::Message msg(
      "PDE_DiffusionNLFVwithBndFaces: \"true Jacobian\" not supported -- maybe "
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFVwithBndFaces::SetScalarCoefficient(
...
PDE_DiffusionNLFVwithBndFaces::InitStencils_()
{
  ...
  for (int i = 0; i < 2 * dim_; ++i) {
    stencil_faces_[i] =
      Teuchos::rcp(new Epetra_IntVector(mesh_->face_map(true)));
    stencil_cells_[i] =
      Teuchos::rcp(new Epetra_IntVector(mesh_->face_map(true)));
  ...
  cvs_tmp.SetMesh(mesh_)->SetGhosted(true)->AddComponent(
    "tensor", AmanziMesh::CELL, dim_ * dim_);
  Teuchos::RCP<CompositeVector> cv_tmp =
    Teuchos::rcp(new CompositeVector(cvs_tmp));
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFVwithBndFaces::UpdateMatrices(
  ...
  const Teuchos::Ptr<const CompositeVector>& u)
{
  ...
  Epetra_MultiVector& weight = *stencil_data_->ViewComponent("weight", true);
  Epetra_MultiVector& flux_data =
    *stencil_data_->ViewComponent("flux_data", true);

  // allocate auxiliary matrix structure
  CompositeVectorSpace cvs;
  cvs.SetMesh(mesh_)->SetGhosted(true)->AddComponent(
    "face", AmanziMesh::FACE, 2);
  ...
  for (int c = 0; c < ncells_owned; ++c) {
    ...
    for (int n = 0; n < nfaces; ++n) {
      ...
      double gamma, g1, g2, gg(-1.0), w1, w2(0.0), tpfa, mu(1.0), ntpfa1,
        ntpfa2;
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFVwithBndFaces::OneSidedFluxCorrections_(
      ...
      for (int i = i0; i < dim_; ++i) {
        ...
        if (c3 >= 0) {
          ...
          int bf = mesh_->exterior_face_map(false).getLocalElement(
            mesh_->face_map(false).getGlobalElement(f1));
...
void
PDE_DiffusionNLFVwithBndFaces::OneSidedNeumannCorrections_(
  const CompositeVector& u, CompositeVector& flux_cv)
{
  ...
  for (int f = 0; f < nfaces_owned; f++) {
    if ((bc_model[f] == OPERATOR_BC_NEUMANN) ||
      ...
      int my_bf_id = mesh_->exterior_face_map(false).getLocalElement(
          ...
          if (f1 >= 0) {
            if (c3 >= 0) {
              mesh_->face_get_cells(
                f1, AmanziMesh::Parallel_type::ALL, &cells_tmp);
              OrderCellsByGlobalId_(cells_tmp, c1, c2);
    ...
    for (int n = 0; n < nfaces; ++n) {
      ...
      if (cells.size() > 1) {
        ...
        int bf = mesh_->exterior_face_map(false).getLocalElement(
          mesh_->face_map(false).getGlobalElement(f));
        flux[0][f] =
          flux_data[0][f] * uc[0][c] - flux_data[dim_][f] * ubnd[0][bf];
      }
    }
  }
 ...
 ****************************************************************** */
void
PDE_DiffusionNLFVwithBndFaces::ApplyBCs(bool primary, bool eliminate,
                                        bool essential_eqn)
{
  const std::vector<int>& bc_model = bcs_trial_[0]->bc_model();
  const std::vector<double>& bc_value = bcs_trial_[0]->bc_value();
  ...
  Epetra_MultiVector& rhs_bnd =
    *global_op_->rhs()->ViewComponent("boundary_face", true);
  Epetra_MultiVector& hap = *stencil_data_->ViewComponent("hap", true);
 ...
 * **************************************************************** */
void
PDE_DiffusionNLFVwithBndFaces::UpdateFlux(
  const Teuchos::Ptr<const CompositeVector>& u,
  const Teuchos::Ptr<CompositeVector>& flux)
{
  ...
  Epetra_MultiVector& wgt_sideflux =
    *wgt_sideflux_cv.ViewComponent("face", true);
 ...
 ****************************************************************** */
int
PDE_DiffusionNLFVwithBndFaces::OrderCellsByGlobalId_(
  const AmanziMesh::Entity_ID_List& cells, int& c1, int& c2)
{
  ...
  c2 = cells[1];
  if (mesh_->cell_map(true).getGlobalElement(c1) >
      mesh_->cell_map(true).getGlobalElement(c2)) {
    int c(c1);
    c1 = c2;
    c2 = c;
    return 1;
 ...
 ****************************************************************** */
int
PDE_DiffusionNLFVwithBndFaces::NLTPFAContributions_(int f, double& tc1,
                                                    double& tc2)
{
  int c, c1, c2, c3, f1;
  AmanziMesh::Entity_ID_List cells, cells_tmp, faces;
