...
class PDE_DiffusionNLFVwithBndFaces : public virtual PDE_Diffusion {
 public:
  ...
  {
    operator_type_ = OPERATOR_DIFFUSION_NLFVFACES;
    Init_(plist);
  }

  PDE_DiffusionNLFVwithBndFaces(
 ...
 protected:
  void InitStencils_();
  void OneSidedFluxCorrections_(int i0, const CompositeVector& u,
                                CompositeVector& sideflux);
  void OneSidedWeightFluxes_(int i0, const CompositeVector& u,
                             CompositeVector& sideflux);
  void OneSidedNeumannCorrections_(const CompositeVector& u,
                                   CompositeVector& sideflux);
  int OrderCellsByGlobalId_(const AmanziMesh::Entity_ID_List& cells, int& c1,
                            int& c2);
