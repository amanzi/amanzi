    ...
    bool primary, bool eliminate, bool essential_eqn)
{
  ...
  //Teuchos::RCP<MultiVector_type> rhs_kind;
  //if (primary) 
  auto rhs_kind = rhs.ViewComponent(schema_row.KindToString(kind), true);
  ...
  rhs.GatherGhostedToMaster(Tpetra::CombineMode::ADD);
}
    ...
    bool primary, bool eliminate, bool essential_eqn)
{
  ...
  // AmanziMesh::Entity_kind kind = bc.kind();
  auto rhs_node = rhs.ViewComponent("node", true);
        ...
        for (int n = 0; n != nents; ++n) {
          ...
          if (bc_model[f] == OPERATOR_BC_DIRICHLET) {
            ...
            for (int k = 0; k < d; ++k) {
              ...
              if (eliminate) {
                for (int m = 0; m < nrows; m++) {
                  rhs_loc(m) = -Acell(m, noff) * bc_value(f,k);
                  Acell(m, noff) = 0.0;
                }
              }
    ...
    const Teuchos::RCP<const AmanziMesh::Mesh>& fracture)
{
  ...
  auto gids = gfmap->getNodeElementList();
    ...
    const Teuchos::RCP<const AmanziMesh::Mesh>& mesh)
{
  ...
  // create meta data
  std::string compname("face");
  auto cs = Teuchos::rcp(new CompositeSpace());
  cs->SetMesh(mesh)->SetGhosted(true);
  cs->AddComponent(compname, AmanziMesh::FACE, mmap, gmap, 1);
