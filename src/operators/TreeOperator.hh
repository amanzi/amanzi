...
Note that these are really intended for preconditioners -- it is unlikely that
these need assembled for the operator itself, and therefore no
ComputeResidual() methods are provided.  It would be difficult to manage a RHS
for these systems.
...
class TreeOperator {
 public:
  ...
  void SymbolicAssembleMatrix(int schema, int nonstandard = 0);
