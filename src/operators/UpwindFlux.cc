...
/* ******************************************************************
* Same as Compute but takes into account the Dirichlet BCs 
* for advection
****************************************************************** */
void
UpwindFlux::Compute_wBC(const CompositeVector& flux,
                    const std::vector<int>& bc_model, const std::vector<double>& bc_value, CompositeVector& field)
{
  AMANZI_ASSERT(field.HasComponent("cell"));
  AMANZI_ASSERT(field.HasComponent(face_comp_));
