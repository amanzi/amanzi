                    ...
                    CompositeVector& field)
{
  ...
  for (int f = 0; f < nfaces_wghost; ++f) {
    ...
    if (ncells == 2 && ndofs == 1) {
      ...
      if (bc_model[f] == OPERATOR_BC_DIRICHLET && flag) {
        ...
        //std::cout<<"field_f on g = "<<g<<"; val = "<<field_bf[0][bf]<<std::endl;
      }
    }
  }
  ...
  /*  
  for (int f = 0; f < nfaces_wghost; ++f) { 
    std::cout<<"Krel value on f = "<<f<<"; val = "<<field_f[0][f]<<std::endl;
  }
  */
...
/* ******************************************************************
* Same as UpwindFlux::Compute but we also take into account the Dirichlet BC values
****************************************************************** */
void UpwindFlux::Compute_wBC(const CompositeVector& flux, const std::vector<int>& bc_model, const std::vector<double>& bc_value, CompositeVector& field)
{
  AMANZI_ASSERT(field.HasComponent("cell"));
  AMANZI_ASSERT(field.HasComponent(face_comp_));
