 ...
 public:
  ...
  void CheckMatrixSPD(bool symmetry = true, bool pos_def = true) {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      if (symmetry) AMANZI_ASSERT(fabs(ahb - bha) < 1e-12 * fabs(ahb));
      if (pos_def) {
        AMANZI_ASSERT(aha > 0.0);
        AMANZI_ASSERT(bhb > 0.0);
      }
    }
  }
  ...
  void CheckPreconditionerSPD(double rtol = 1e-12, bool symmetry = true, bool pos_def = true) {
    ...
    if (symmetry) AMANZI_ASSERT(fabs(ahb - bha) < rtol * fabs(ahb));
    if (pos_def) {
      AMANZI_ASSERT(aha > 0.0);
      AMANZI_ASSERT(bhb > 0.0);
    }
  ...
  void CheckSpectralBounds() {
    ...
    for (auto it = op_->OpBegin(); it != op_->OpEnd(); ++it) {
      ...
      for (int i = 0; i < matrices.size(); i++) {
        ...
        Amanzi::WhetStone::DGEEV_F77(
            "N", "N", &n, Acell.Values(), &n, dmem1.Values(), dmem2, 
