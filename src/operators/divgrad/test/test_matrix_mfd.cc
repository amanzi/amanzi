...
TEST_FIXTURE(mfd, AssembleRandomTwoPointNormed) {
  ...
  CHECK_CLOSE(14.9865519938823, norm_Aff, 1.e-8);
  CHECK_CLOSE(4.2133358376325, norm_schur, 1.e-8);
  ...
  filename_Aff << "test/MatrixMFD_Aff_np" << comm->NumProc() << ".txt";
  EpetraExt::RowMatrixToMatlabFile(filename_Aff.str().c_str(), *A->Aff());

  std::stringstream filename_Sff;
  filename_Sff << "test/MatrixMFD_Sff_np" << comm->NumProc() << ".txt";
  ...
  if (comm->MyPID() == 0) {
    ...
    filename_Aff_ref << "test/MatrixMFD_Aff_ref_np" << comm->NumProc() << ".txt";
    ...
    filename_Sff_ref << "test/MatrixMFD_Sff_ref_np" << comm->NumProc() << ".txt";
    ...
    filename_Aff_test << "test/MatrixMFD_Aff_np" << comm->NumProc() << ".txt";
    ...
    filename_Sff_test << "test/MatrixMFD_Sff_np" << comm->NumProc() << ".txt";
...
TEST_FIXTURE(mfd, ApplyConstantMFD) {
  Teuchos::RCP<CompositeVector> kr = Teuchos::null;
  createMFD("mfd", kr.ptr());
...
TEST_FIXTURE(mfd, ApplyConstantMFDKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyLinearMFDKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyInverseLinearMFDKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ConsistentFaceLinearMFDKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyRandomMFDKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyInverseRandomMFDKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyInverseRandomMFDKrRandom) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, AssembleRandomMFDVsOLDVALUE) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
  ...
  setDirichletOne();
  createMFD("optimized mfd", kr.ptr());
  ...
  A->Schur()->Apply(x_f, *b->ViewComponent("face",false));
  double norm_schur(0.);
  b->ViewComponent("face",false)->Norm2(&norm_schur);
  std::cout << std::setprecision(15) << "norms = " << norm_Aff << ", " << norm_schur << std::endl;
