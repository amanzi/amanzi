...
struct mfd {
  ...
  Teuchos::RCP<Operators::MatrixMFD_TPFA> A;
  ...
  void createMFD(std::string method, Teuchos::Ptr<CompositeVector> kr, bool applyBC) {
    ...
    A = Teuchos::rcp(new Operators::MatrixMFD_TPFA(plist, mesh));
  ...
  void createMFDRef(std::string method, Teuchos::Ptr<CompositeVector> kr, bool applyBC) {
    Teuchos::ParameterList plist;
    plist.set("MFD method", method);
    plist.sublist("preconditioner").set("preconditioner type", "boomer amg");
    plist.sublist("preconditioner").sublist("boomer amg parameters")
...
TEST_FIXTURE(mfd, ApplyConstantTPFA) {
...
TEST_FIXTURE(mfd, ApplyConstantTPFAKr) {
...
TEST_FIXTURE(mfd, ApplyLinearTPFAKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyInverseLinearTPFAKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ConsistentFaceLinearTPFAKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyRandomTPFAKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, ApplyInverseRandomTPFAKr) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
...
TEST_FIXTURE(mfd, AssembleRandomTPFANormed) {
  ...
  filename_Sff << "test/MatrixMFD_TPFA_Scc_np" << comm->NumProc() << ".txt";
  ...
  if (comm->MyPID() == 0) {
    ...
    filename_Sff_ref << "test/MatrixMFD_TPFA_Scc_ref_np" << comm->NumProc() << ".txt";
    ...
    filename_Sff_test << "test/MatrixMFD_TPFA_Scc_np" << comm->NumProc() << ".txt";
...
TEST_FIXTURE(mfd, TPFACompareMFD) {
  CompositeVectorSpace kr_sp;
  kr_sp.SetMesh(mesh)->SetGhosted()->SetComponent("face",AmanziMesh::FACE,1);
