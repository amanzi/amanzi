...
class AnalyticBase {
 public:
  ...
  virtual double
  ScalarDiffusivity(const Amanzi::AmanziGeometry::Point& p, double t)
  {
    return 1.0;
  }
  ...
  // error calculation
  void ComputeCellError(Epetra_MultiVector& p, double t, double& pnorm,
                        double& l2_err, double& inf_err);
  void ComputeFaceError(Epetra_MultiVector& u, double t, double& unorm,
                        double& l2_err, double& inf_err);
  void ComputeNodeError(Epetra_MultiVector& p, double t, double& pnorm,
                        double& l2_err, double& inf_err, double& hnorm,
                        double& h1_err);
  void ComputeEdgeError(Epetra_MultiVector& p, double t, double& pnorm,
                        double& l2_err, double& inf_err, double& hnorm,
                        double& h1_err);
 ...
 ****************************************************************** */
inline void
AnalyticBase::ComputeCellError(Epetra_MultiVector& p, double t, double& pnorm,
                               double& l2_err, double& inf_err)
{
  pnorm = 0.0;
  l2_err = 0.0;
 ...
 ****************************************************************** */
inline void
AnalyticBase::ComputeFaceError(Epetra_MultiVector& u, double t, double& unorm,
                               double& l2_err, double& inf_err)
{
  unorm = 0.0;
  l2_err = 0.0;
  ...
  for (int f = 0; f < nfaces; f++) {
    ...
    // std::cout << f << " xf=" << xf << " u=" << u[0][f] << " u_ex=" << tmp <<
    // std::endl;
  }
#ifdef HAVE_MPI
 ...
 ****************************************************************** */
inline void
AnalyticBase::ComputeNodeError(Epetra_MultiVector& p, double t, double& pnorm,
                               double& l2_err, double& inf_err, double& hnorm,
                               double& h1_err)
{
  pnorm = 0.0;
  ...
  for (int c = 0; c < ncells; c++) {
    ...
    for (int k = 0; k < nnodes; k++) {
      ...
      if (std::abs(tmp - p[0][v]) > .01) {
        ...
        // std::cout << v << " at " << xv << " error: " << tmp << " " << p[0][v]
        // << std::endl;
      }
 ...
 ****************************************************************** */
inline void
AnalyticBase::ComputeEdgeError(Epetra_MultiVector& p, double t, double& pnorm,
                               double& l2_err, double& inf_err, double& hnorm,
                               double& h1_err)
{
  pnorm = 0.0;
  ...
  for (int c = 0; c < ncells; c++) {
    ...
    for (int k = 0; k < nedges; k++) {
      ...
      // std::cout << e << " at " << xe << " error: " << tmp << " " << p[0][e]
      // << std::endl;
    }
  }
#ifdef HAVE_MPI
 ...
 ****************************************************************** */
inline void
AnalyticBase::ComputeEdgeMomentsError(Epetra_MultiVector& p, double t,
                                      int ngauss, double& pnorm, double& l2_err,
                                      double& inf_err)
{
  pnorm = 0.0;
  l2_err = 0.0;
