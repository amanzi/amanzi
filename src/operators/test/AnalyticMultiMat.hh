...
#ifndef AMANZI_OPERATOR_ANALYTIC_MAT_HH_
#define AMANZI_OPERATOR_ANALYTIC_MAT_HH_

#include "AnalyticBase.hh"

class AnalyticMultiMat : public AnalyticBase {
 public:
  AnalyticMultiMat(Teuchos::RCP<const Amanzi::AmanziMesh::Mesh> mesh) :
      AnalyticBase(mesh),
      g_(0.0) {};
  AnalyticMultiMat(Teuchos::RCP<const Amanzi::AmanziMesh::Mesh> mesh, double g) :
      AnalyticBase(mesh),
      g_(g) {};
  ~AnalyticMultiMat() {};
  ...
  double Scalar_Coef(int mat_id) {

    double val(1.);

    if (mat_id==0) val = 1.;
    else if (mat_id==1) val = 100.;
