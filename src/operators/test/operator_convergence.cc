...
TEST(OPERATOR_MIXED_DIFFUSION_SFF) {
  using namespace Amanzi;
  using namespace Amanzi::AmanziMesh;
  using namespace Amanzi::AmanziGeometry;
  using namespace Amanzi::Operators;
  ...
  if (MyPID == 0) std::cout << "Test: 2D steady-state elliptic solver, mixed discretization, Schur to Sff" << std::endl;
  ...
  for (int n = 0; n < 240; n+=50) {
    ...
    // create the local diffusion operator
    Teuchos::ParameterList olist = plist.get<Teuchos::ParameterList>("PK operators")
                                        .get<Teuchos::ParameterList>("mixed diffusion sff");
    OperatorDiffusion op2(olist, mesh);
