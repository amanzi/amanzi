...
RunTestDiffusionDivK2D(std::string diffusion_list, std::string upwind_list)
{
  ...
  Teuchos::ParameterList op_list =
    plist.sublist("PK operator").sublist(diffusion_list);
  ...
  Teuchos::ParameterList& ulist =
    plist.sublist("PK operator").sublist(upwind_list);
  ...
  if (getRank == 0) {
    ...
    ul2_err /= unorm;
    printf(
      "L2(p)=%12.8g  Inf(p)=%12.8g  L2(u)=%12.8g  Inf(u)=%12.8g  itr=%3d\n",
      pl2_err,
      pinf_err,
      ul2_err,
      uinf_err,
...
{
  RunTestDiffusionDivK2D<UpwindFlux<HeatConduction>>("diffusion operator divk",
                                                     "upwind");
}

TEST(OPERATOR_DIFFUSION_DIVK_SECOND_ORDER)
{
  RunTestDiffusionDivK2D<UpwindSecondOrder<HeatConduction>>(
    "diffusion operator second-order", "upwind second-order");
}
...
TEST(OPERATOR_DIFFUSION_DIVK_AVERAGE_3D)
{
  ...
  int getRank = comm->getRank();
  if (getRank == 0)
    std::cout << "\nTest: 3D elliptic solver, divK discretization, average"
              << std::endl;
  ...
  // create diffusion operator
  Teuchos::ParameterList op_list =
    plist.sublist("PK operator").sublist("diffusion operator divk");
  auto op = Teuchos::rcp(new PDE_DiffusionMFD(op_list, mesh));
  op->Init(op_list);
  op->SetBCs(bc, bc);
  const CompositeVectorSpace& cvs = op->global_operator()->DomainMap();
  ...
  upwind.Compute(*flux, *solution, bc_model, *knc->values());
  // upwind.Compute2(*flux, *solution, bc_model, bc_value, *knc->values(),
  // func);
