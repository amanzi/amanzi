...
/* *****************************************************************
* Elasticity model: exactness with advection term
***************************************************************** */
TEST(OPERATOR_ELASTICITY_ADVECTION_EXACTNESS) {
  using namespace Amanzi;
  using namespace Amanzi::AmanziMesh;
  using namespace Amanzi::AmanziGeometry;
  using namespace Amanzi::Operators;
  ...
  Epetra_MpiComm comm(MPI_COMM_WORLD);
  int MyPID = comm.MyPID();
  if (MyPID == 0) std::cout << "\nTest: 2D elasticity: exactness with advection test" << std::endl;
  ...
  //  -- add diffusion
  Teuchos::ParameterList dlist = plist.get<Teuchos::ParameterList>("PK operator").sublist("elasticity operator");
  Teuchos::RCP<Elasticity> op_diff = Teuchos::rcp(new Elasticity(dlist, mesh));
  op_diff->SetBCs(bcf, bcf);
  op_diff->AddBCs(bcv, bcv);

  Teuchos::RCP<Operator> global_op = op_diff->global_operator();
  const CompositeVectorSpace& cvs = global_op->DomainMap();

  // -- add advection
  Teuchos::ParameterList alist = plist.get<Teuchos::ParameterList>("PK operator").sublist("advection operator");
  Teuchos::RCP<AdvectionRiemann> op_adv = Teuchos::rcp(new AdvectionRiemann(alist, global_op));
  ...
  // populate the elasticity operator
  op_diff->SetTensorCoefficient(K);
  op_diff->UpdateMatrices();

  op_adv->UpdateMatrices(solution);

  // get and assmeble the global operator
  op_diff->ApplyBCs(true, true);
  op_adv->ApplyBCs(false, true);
  global_op->SymbolicAssembleMatrix();
  global_op->AssembleMatrix();
