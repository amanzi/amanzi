...
void
MagneticDiffusion2D(double dt, double tend, int nx, int ny, double Xa,
                    double Ya, double Xb, double Yb, const std::string& name)
{
  ...
  if (getRank == 0)
    std::cout << "\nTest: Magnetic diffusion, TM mode, dt=" << dt
              << ", name: " << name << std::endl;
  ...
  int nnodes_owned =
    mesh->num_entities(AmanziMesh::NODE, AmanziMesh::Parallel_type::OWNED);
  int nnodes_wghost =
  ...
  Teuchos::RCP<CompositeVectorSpace> cvs_b =
    Teuchos::rcp(new CompositeVectorSpace());
  cvs_b->SetMesh(mesh)->SetGhosted(true)->AddComponent(
  ...
  while (told + dt / 2 < tend) {
    ...
    ParameterList lop_list =
      plist.sublist("solvers").sublist("silent").sublist("pcg parameters");
    AmanziSolvers::
    ...
    Teuchos::RCP<CompositeVectorSpace> cvs =
      Teuchos::rcp(new CompositeVectorSpace());
    cvs->SetMesh(mesh)->SetGhosted(true)->AddComponent(
      "cell", AmanziMesh::CELL, 2);
    ...
    if (getRank == 0) {
      std::cout << "time: " << told << "  ||r||=" << solver.residual()
                ...
                << "  heat= " << heat << "  avgB=" << avgB / ncells_owned
...
template <class Analytic>
void
MagneticDiffusion3D(double dt, double tend, bool convergence, int nx, int ny,
                    int nz, double Xa, double Ya, double Za, double Xb,
                    double Yb, double Zb, const std::string& name)
{
  using namespace Teuchos;
  using namespace Amanzi;
  using namespace Amanzi::AmanziMesh;
  ...
  if (getRank == 0)
    std::cout << "\nTest: Magnetic diffusion 3D, dt=" << dt
              << ", name: " << name << std::endl;
  ...
  if (name == "structured")
    mesh = meshfactory.create(
      Xa, Ya, Za, Xb, Yb, Zb, nx, ny, nz, request_faces, request_edges);
  else
    mesh = meshfactory.create(name, request_faces, request_edges);
  // mesh = meshfactory.create("test/hex_split_faces5.exo", request_faces,
  // request_edges);
  ...
  Teuchos::ParameterList olist =
    plist.sublist("PK operator").sublist("electromagnetics operator");
  Teuchos::RCP<PDE_MagneticDiffusion> op_mag =
    Teuchos::rcp(new PDE_MagneticDiffusion(olist, mesh));
  op_mag->SetBCs(bc1, bc1);
  ...
  while (told + dt / 2 < tend) {
    ...
    for (int f = 0; f < nfaces_wghost; ++f) {
      ...
      } else if ((fabs(xf[0] - Xa) < 1e-6 && convergence) ||
                 fabs(xf[0] - Xb) < 1e-6 || fabs(xf[1] - Ya) < 1e-6 ||
                 fabs(xf[1] - Yb) < 1e-6 || fabs(xf[2] - Za) < 1e-6 ||
                 fabs(xf[2] - Zb) < 1e-6) {
    ...
    double err_out[2];
    Teuchos::reduceAll(
      *mesh->get_comm(), Teuchos::REDUCE_SUM, 2, err_in, err_out);
...
{
  MagneticDiffusion2D<AnalyticElectromagnetics05>(
    0.01, 0.1, 10, 10, 0.0, 0.0, 1.0, 1.0, "test/random10.exo");
  // MagneticDiffusion2D<AnalyticElectromagnetics05>(0.01 / 2, 0.1, 20,20,
  // 0.0,0.0, 1.0,1.0, "structured");
}

TEST(MAGNETIC_DIFFUSION3D_RELAX)
{
  MagneticDiffusion3D<AnalyticElectromagnetics04>(0.1,
                                                  0.5,
                                                  false,
                                                  10,
                                                  8,
...
{
  // MagneticDiffusion3D<AnalyticElectromagnetics05>(0.01, 0.1, true, 8,8,8,
  // 0.0,0.0,0.0, 1.0,1.0,1.0, "structured");
  MagneticDiffusion3D<AnalyticElectromagnetics05>(0.01,
                                                  0.1,
                                                  true,
                                                  8,
                                                  8,
