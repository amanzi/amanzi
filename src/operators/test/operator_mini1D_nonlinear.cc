...
class NonlinearProblem : public AmanziSolvers::SolverFnBase<DenseVector> {
 public:
  ...
  NonlinearProblem(Teuchos::RCP<Operators::Mini_Diffusion1D> op, double dt,
...
{
  std::cout << "\nTest: 1D nonlinear elliptic problem: constant absolute K"
            << std::endl;
  ...
  for (int loop = 0; loop < 2; ++loop) {
    ...
    auto fn =
      Teuchos::rcp(new NonlinearProblem(op, 0.0, bcl, type_l, bcr, type_r));
    fn->SetICs(rhs, sol);
