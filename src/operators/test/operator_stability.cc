...
TEST(OPERATOR_MIXED_DIFFUSION)
{
  ...
  if (getRank == 0)
    std::cout << "Test: 2D steady-state elliptic solver, mixed discretization"
              << std::endl;
  ...
  for (int n = 0; n < 240; n += 50) {
    ...
    Teuchos::ParameterList olist =
      plist.sublist("PK operators").sublist("mixed diffusion");
    ...
    auto solver = Teuchos::rcp(
      new LinearOperatorPCG<Operator, CompositeVector, CompositeVectorSpace>(
        global_op, global_op));
...
TEST(OPERATOR_NODAL_DIFFUSION)
{
  ...
  int getRank = comm->getRank();
  if (getRank == 0)
    std::cout << "\nTest: 2D steady-state elliptic solver, nodal discretization"
              << std::endl;
  ...
  for (int n = 0; n < 400; n += 110) {
    ...
    // create the local diffusion operator
    Teuchos::ParameterList olist =
      plist.sublist("PK operators").sublist("nodal diffusion");
    PDE_DiffusionMFD op2(olist, mesh);
    op2.Init(olist);
    op2.SetBCs(bc, bc);
