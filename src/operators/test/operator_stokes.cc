...
TEST(OPERATOR_STOKES_EXACTNESS)
{
  ...
  Teuchos::RCP<const Mesh> mesh =
    meshfactory.create(0.0, 0.0, 1.0, 1.0, 16, 20);
  ...
  Point xv(2);
  Teuchos::RCP<BCs> bcv =
    Teuchos::rcp(new BCs(mesh, AmanziMesh::NODE, DOF_Type::POINT));
  ...
  // -- create an elasticity operator using in particular schema data. In the
  // future,
  // -- we could take definition of DOFs data from WhetStone using the provided
  // -- discretization method.
  Teuchos::ParameterList op_list =
    plist.sublist("PK operator").sublist("elasticity operator");
  Teuchos::RCP<PDE_Elasticity> op00 =
    Teuchos::rcp(new PDE_Elasticity(op_list, mesh));
  op00->SetTensorCoefficient(K);
  // -- add boundary conditions to the discrete PDE: Lame equation
  op00->SetBCs(bcf, bcf);
  op00->AddBCs(bcv, bcv);

  // -- create a divergence operator. Note that it uses two different schemas
  // for DOFs for velocity and pressure.
  op_list = plist.sublist("PK operator").sublist("divergence operator");
  Teuchos::RCP<PDE_Abstract> op10 =
    Teuchos::rcp(new PDE_Abstract(op_list, mesh));
  ...
  // create a tree operator for the discrete Stokes PDE. It is a 2x2 block
  // operator composed of Lame operator (blok 00), divergence operator (blok 10)
  // and traspose of the divergence operator (block 01).
  ...
  tvs->PushBack(Teuchos::rcp(new TreeVectorSpace(
    Teuchos::rcpFromRef(op10->global_operator()->RangeMap()))));

  Teuchos::RCP<TreeOperator> op =
    Teuchos::rcp(new Operators::TreeOperator(tvs));
  ...
  AmanziSolvers::LinearOperatorGMRES<TreeOperator, TreeVector, TreeVectorSpace>
    solver(op, pc);
  ...
  *rhs.SubVector(1)->Data() = *global10->rhs();
  ...
  ana.ComputeNodeError(
    *solution.SubVector(0)->Data(), 0.0, unorm, ul2_err, uinf_err);

  // -- compute pressure error
  double pnorm, pl2_err, pinf_err;
  ana.ComputeCellError(
    *solution.SubVector(1)->Data(), 0.0, pnorm, pl2_err, pinf_err);

  if (getRank == 0) {
    ul2_err /= unorm;
    printf(
      "L2(u)=%12.8g  Inf(u)=%12.8g  L2(p)=%12.8g  Inf(p)=%12.8g  itr=%3d\n",
      ul2_err,
      uinf_err,
      pl2_err,
      pinf_err,
  ...
  if (getRank == 0) {
    const Epetra_MultiVector& u =
      *solution.SubVector(0)->Data()->ViewComponent("node");
