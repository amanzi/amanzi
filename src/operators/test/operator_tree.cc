...
TEST(SURFACE_SUBSURFACE_SUPERMAP) {
  ...
  int ncells_subsurf = mesh_subsurf->getNumEntities(AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_type::OWNED);
  auto mesh_surf = meshfactory.create(mesh_subsurf, {"surface"}, AmanziMesh::Entity_kind::FACE, true);
  int ncells_surf = mesh_surf->getNumEntities(AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_type::OWNED);
...
TEST(SURFACE_SUBSURFACE_DIFFUSION) {
  ...
  auto mesh_surf = meshfactory.create(mesh_subsurf, {"surface"}, Amanzi::AmanziMesh::Entity_kind::FACE, true);
  int ncells_surf = mesh_surf->getNumEntities(AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_type::OWNED);
  ...
  Teuchos::RCP<Operators::BCs> bc_subsurf = Teuchos::rcp(new Operators::BCs(mesh_subsurf, AmanziMesh::Entity_kind::FACE, WhetStone::DOF_Type::SCALAR));
  ...
  Teuchos::RCP<Operators::BCs> bc_surf = Teuchos::rcp(new Operators::BCs(mesh_surf, AmanziMesh::Entity_kind::FACE, WhetStone::DOF_Type::SCALAR));
  ...
  std::vector<int> parents(mesh_surf->getNumEntities(AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_type::OWNED));
  ...
  for (int sc=0; sc!=ncells_surf; ++sc) {
    ...
    mesh_subsurf->getFaceCells(f, AmanziMesh::Parallel_type::OWNED, cells);
