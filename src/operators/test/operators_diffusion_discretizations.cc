...
void
test(const Teuchos::RCP<AnalyticBase>& ana,
...
template <class PDE_Diffusion_type>
void
testWGravity(const Teuchos::RCP<AnalyticBase>& ana,
             double gravity,
             const std::string& pc_type,
             const std::string& bc_type,
             const std::string& mesh_type,
             ...
             int niters = 1)
{
  ...
  std::cout << std::endl
            ...
            << "================================================================================"
            << std::endl
            << "DiffusionWithGravity Test (np=" << fix.comm->getSize() << "): " << disc_type << ", "
            << ana->name() << ", " << pc_type << ", " << mesh_type << std::endl
            << "--------------------------------------------------------------------------------"
            << std::endl;
...
SUITE(DIFFUSION)
{
  ...
  TEST(EXACT)
  {
    ...
    CompositeVector& rhs = *fix.global_op->rhs();
    {
      ...
      for (int c = 0; c != fix.mesh->getNumEntities(AmanziMesh::Entity_kind::CELL,
        ...
        rhs_c(c, 0) += ana->source_exact(xc, 0.0) * fix.mesh->getCellVolume(c);
      }
    }
  ...
  {
    auto ana = Teuchos::rcp(new Analytic00(1, 1.0, 1.0, 0.0));
    test<Operators::PDE_DiffusionFV>(ana,
                                     "identity",
                                     "Dirichlet",
                                     "Generate2D",
                                     "fv",
  ...
  TEST(Analytic00_LinearGravity1_FV_Dirichlet_Generate2D_identity)
  {
    ...
    testWGravity<Operators::PDE_DiffusionFVwithGravity>(ana,
                                                        1.1,
                                                        "identity",
                                                        "Dirichlet",
                                                        "Generate2D",
...
#  endif
  ...
  {
    auto ana = Teuchos::rcp(new Analytic00(1, 1.0, 1.0, 1.1));
    testWGravity<Operators::PDE_DiffusionMFDwithGravity>(ana,
                                                         1.1,
                                                         "identity",
                                                         "Dirichlet",
                                                         "Generate2D",
  ...
  {
    auto ana = Teuchos::rcp(new Analytic00(1, 1.0, 1.0, 0.0));
    test<Operators::PDE_DiffusionMFD>(ana,
                                      "diagonal",
                                      "Dirichlet",
                                      "Generate2D",
                                      "mixed",
