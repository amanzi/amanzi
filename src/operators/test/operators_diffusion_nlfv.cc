...
RunTestDiffusionNLFV_DMP(double gravity, bool testing)
{
  ...
  for (int loop = 0; loop < 12; ++loop) {
    ...
    AmanziSolvers::LinearOperatorBelosGMRES<Operator, CompositeVector, CompositeVectorSpace> solver(
      global_op, global_op);
    ...
    if (MyPID == 0) {
      ...
      ul2_err /= unorm;
      printf(
        "L2(p)=%10.4e  Inf(p)=%10.4e  L2(u)=%10.4e  Inf(u)=%10.4e  (itr=%d ||r||=%10.4e code=%d)\n",
        pl2_err,
        pinf_err,
        ul2_err,
        uinf_err,
...
RunTestDiffusionNLFVwithBndFaces_DMP(double gravity, bool testing)
{
  ...
  int MyPID = comm->MyPID();
  if (MyPID == 0)
    std::cout << "\nTest: 2D elliptic solver, NLFVwithBndFaces with DMP, g=" << gravity
              << std::endl;
  ...
  Teuchos::ParameterList op_list =
    plist.get<Teuchos::ParameterList>("PK operator").sublist("diffusion operator nlfv");
