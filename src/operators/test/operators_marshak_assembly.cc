...
void
writeMarshakMatrix(std::string op_list_name, double floor, bool jac)
{
  ...
  auto solution = cvs->Create();
  {
    ...
    for (int c = 0; c < ncells_owned; ++c) {
      const AmanziGeometry::Point& xc = mesh->getCellCentroid_host(c);
  ...
  if (solution->hasComponent("face")) {
    ...
    for (int f = 0; f < nfaces_owned; ++f) {
      const AmanziGeometry::Point& xf = mesh->getFaceCentroid_host(f);
    ...
    for (int f = 0; f != nfaces_owned; ++f) {
      ...
      if (cells.size() == 2) {
        ...
        if (bc_model[f] == Operators::OPERATOR_BC_DIRICHLET) {
          int bf = mesh->exterior_face_map(false)->getLocalElement(
            mesh->face_map(false)->getGlobalElement(f));
