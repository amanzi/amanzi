...
***************************************************************** */
double
MyRemapDG::InitialMass(const CompositeVector& p1, int order)
{
...
***************************************************************** */
void
MyRemapDG::CollectStatistics(double t, const CompositeVector& u)
{
  double tglob = t;
  if (tglob >= tprint_) {
...
***************************************************************** */
void
RemapTestsDualRK(std::string map_name,
                 ...
                 int deform = 1)
{
  ...
  if (MyPID == 0) {
    ...
    std::cout << "      discretization: order=" << order << ", map=" << map_name << std::endl;
  ...
  ana.ComputeCellErrorRemap(
    *dg, p2c, tend, 0, mesh1, pnorm, l2_err, inf_err, l20_err, inf0_err, &pec);
  ...
  for (int c = 0; c < ncells_owned; ++c) {
    const AmanziGeometry::Point& xc0 = mesh0->getCellCentroid(c);
    const AmanziGeometry::Point& xc1 = mesh1->getCellCentroid(c);
  ...
  ana.ComputeCellErrorRemap(*dg, q2c, tend, 1, mesh1, pnorm, l2_err, inf_err, l20_err, inf0_err);
