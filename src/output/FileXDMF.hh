...
class FileXDMF {
 ...
 private:
  ...
  Teuchos::XMLObject GetHeaderLocal_(const std::string name, const double time, const int cycle, bool write_mesh);
  ...
  Teuchos::XMLObject GetField_(const std::string& varname, const std::string& location,
...
template<> struct FileXDMF::DatatypeMap<int> { static const std::string type; };
template<> struct FileXDMF::DatatypeMap<float> { static const std::string type; };
template<> struct FileXDMF::DatatypeMap<double> { static const std::string type; };
                     ...
                     const std::string& dofnum)
{
  ...
  if (location == AmanziMesh::Entity_kind::CELL) {
    node.addChild(GetField_(var_name+"."+dofnum, "Cell", data_type, n_elems_, cycle_));
  } else if (location == AmanziMesh::Entity_kind::NODE) {
    node.addChild(GetField_(var_name+"."+dofnum, "Node", data_type, n_nodes_, cycle_));
  } else {
    Errors::Message message;
    message << "FileXDMF: Cannot write location of type \"" << AmanziMesh::entity_kind_string(location) << "\"";
...
template<typename scalar_type>
void
FileXDMF::WriteFields(const std::string& varname, const std::vector<std::string>& subfield_names,
...
template<typename scalar_type>
void
FileXDMF::WriteFields(const std::string& varname, int n_dofs, const AmanziMesh::Entity_kind& location)
{
