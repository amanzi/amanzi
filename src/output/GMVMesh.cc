...
static inline void write_mesh_to_file_(const AmanziMesh::Mesh &mesh, std::string filename)
{
  ...
  int max_nodes = AmanziMesh::MeshAlgorithms::getMaxCellNumNodes(mesh);
  unsigned int *xh = new unsigned int[max_nodes];

  int max_faces = AmanziMesh::MeshAlgorithms::getMaxCellNumFaces(mesh);
  ...
  for (int i=0; i<num_cells; i++) {
    ...
    if (dim == 3 && dim_cell == 3) {
      if (nnodes == 8) {
        ...
        mesh.getCellFacesAndDirs(i, cfaces, &fdirs);
        int nfaces = cfaces.size();

        for (int j=0, n=0; j<nfaces; j++) {
          mesh.getFaceNodes(cfaces[j], fnodes);
