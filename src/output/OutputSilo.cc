...
OutputSilo::OutputSilo(Teuchos::ParameterList& plist,
                       ...
                       bool is_vis, bool is_dynamic)
  : mesh_(mesh), fid_(NULL), count_(0)
{
  if (mesh_->vis_mesh().get_comm()->getSize() > 1) {
    Errors::Message msg("OutputSilo does not yet support parallel runs.");
    Exceptions::amanzi_throw(msg);
  }
  if (mesh_->vis_mesh().space_dimension() != 3 ||
      mesh_->vis_mesh().manifold_dimension() != 3) {
...
OutputSilo::CreateTimestep(double time, int cycle)
{
  ...
  filename << filenamebase_ << "_" << std::setfill('0') << std::setw(sigfigs_)
           << count_;
  ...
  // -- nodal coordinates
  int nnodes = mesh_->vis_mesh().num_entities(AmanziMesh::NODE,
  ...
  int ncells = mesh_->vis_mesh().num_entities(AmanziMesh::CELL,
                                              AmanziMesh::Parallel_type::OWNED);
  int ierr = DBPutUcdmesh(fid_,
                          "mesh",
  ...
  int nfaces = mesh_->vis_mesh().num_entities(AmanziMesh::FACE,
                                              AmanziMesh::Parallel_type::OWNED);
  ...
  for (int f = 0; f != nfaces; ++f) {
    ...
    mesh_->vis_mesh().face_get_cells(
      f, AmanziMesh::Parallel_type::ALL, &fcells);
...
// write data to file
void
OutputSilo::WriteVector(const Epetra_Vector& vec, const std::string& name) const
{
  if (vec.getLocalLength() ==
      mesh_->vis_mesh().num_entities(AmanziMesh::CELL,
                                     AmanziMesh::Parallel_type::OWNED)) {
    int ierr = DBPutUcdvar1(fid_,
                            ...
                            NULL);
    AMANZI_ASSERT(!ierr);
  } else if (vec.getLocalLength() ==
             mesh_->vis_mesh().num_entities(AmanziMesh::NODE,
                                            AmanziMesh::Parallel_type::OWNED)) {
    int ierr = DBPutUcdvar1(fid_,
                            FixName_(name).c_str(),
                            "mesh",
    ...
    Errors::Message msg(
      "OutputSilo only knows how to write CELL and NODE based quantities.");
    Exceptions::amanzi_throw(msg);
  }
}
...
void
OutputSilo::WriteAttribute(const std::string& val,
