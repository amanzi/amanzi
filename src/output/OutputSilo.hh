...
class OutputSilo : public Output {
 public:
  OutputSilo(Teuchos::ParameterList& plist,
             const Teuchos::RCP<const AmanziMesh::Mesh>& mesh, bool is_vis,
             bool is_dynamic);
  ...
  // write data to file
  virtual void
  WriteVector(const Epetra_Vector& vec, const std::string& name) const;
  ...
  virtual void
  WriteAttribute(const std::string& val, const std::string& name) const;
