...
OutputXDMF::createTimestep(double time, int cycle)
{
  ...
  if (!init_) {
    ...
    h5_data_ =
      std::make_unique<FileHDF5>(mesh_->getComm(), h5_data_filename_, FILE_CREATE);

    // create and set up the FileHDF5Mesh h5
    h5_mesh_filename_ = filenamebase_ + "_mesh.h5";
    h5_mesh_ =
      std::make_unique<FileHDF5>(mesh_->getComm(), h5_mesh_filename_, FILE_CREATE);
...
OutputXDMF::writeDualMesh_(int cycle)
{
  ...
  Teuchos::reduceAll(*vis_mesh.getComm(), Teuchos::REDUCE_SUM, 2, local_conn_ents.data(), global_conn_ents.data());
  ...
  // write the cell map
  auto dual_cell_map = Teuchos::rcp(new Map_type(global_cells_tp, local_conn_ents[1], 0, vis_mesh.getComm()));
...
void
OutputXDMF::write(const Teuchos::ParameterList& attrs, const CompositeVector_<double_type>& vec) const
{
