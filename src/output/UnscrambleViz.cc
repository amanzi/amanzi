...
main(int argc, char* argv[])
{
  ...
  if ((fp = fopen(argv[1], "r")) == NULL) {
    ...
    int elem_types[num_elems][3]; // 0)element type id 1)connectivity length
                                  // 2)offset is element array
    int elem_cnt = 0;
    ...
    if ((fp = fopen(argv[2], "r")) == NULL) {
      ...
      for (int i = 0; i < groupList.size(); ++i) {
        ...
        hid_t group = H5Gcreate(new_file,
        ...
        H5Giterate(
          data_file, group_name.str().c_str(), NULL, dataset_info, NULL);
        for (int j = 0; j < datasetList.size(); j++) {
          ...
          status = unpermute(
            ds_name.str().c_str(), data_file, new_file, nodemap, elemmap);
        }
      }
    }
