...
herr_t
group_info(hid_t loc_id, const char* name, void* empty)
{
  H5G_stat_t buf;
  ...
  if ((fp = fopen(argv[1], "r")) == NULL) {
    ...
    for (int i = 0; i < num_elems; i++) {
      ...
      std::cout << "  E>> id = " << i << " type = " << elem_types[i][0]
                << " conn_len = " << elem_types[i][1] << " offset = " << elem_types[i][2]
                << std::endl;
    }
    ...
    dataset_id =
      H5Dcreate(new_file, "/Mesh/Nodes", dt, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    if ((fp = fopen(argv[2], "r")) == NULL) {
      ...
      for (int i = 0; i < groupList.size(); ++i) {
        ...
        for (int j = 0; j < datasetList.size(); j++) {
          ...
          status =
            unpermute(ds_name.str().c_str(), data_file, new_file, nodemap.data(), elemmap.data());
        }
      }
    }
