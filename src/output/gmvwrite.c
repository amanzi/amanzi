...
void
write_ascii_float(long num, float* val);
void
write_ascii_double(long num, double* val);
void
write_ascii_int(long num, int* val);
void
write_ascii_long(long num, long* val);


/* --------------------------------------------------------- */

void
gmvwrite_openfile(char filenam[])
{
  fp = fopen(filenam, "w");
...
void
gmvwrite_openfile_ascii(char filenam[])
{
  fp = fopen(filenam, "w");
...
void
gmvwrite_openfile_ir(char filenam[], int isize, int rsize)
{
...
void
gmvwrite_openfile_cxir(char filenam[], int isize, int rsize)
{
  /*  Check if this machine can address 64bit integers (isize = 8).  */
  if (isize == 8) {
    if (sizeof(long) < 8) {
...
void
gmvwrite_openfile_ir_ascii(char filenam[], int isize, int rsize)
{
  /*  Check if this machine can address 64bit integers (isize = 8).  */
  if (isize == 8) {
    if (sizeof(long) < 8) {
...
void
gmvwrite_nodes_fromfile(char* filename, long nndes)
{
  char* tmpbuf;
...
void
gmvwrite_node_data_struct(void* nxv, void* nyv, void* nzv, void* x, void* y,
                          void* z)
{
  int alt, count;
...
void
gmvwrite_node_data_lstruct(void* nxv, void* nyv, void* nzv, void* x, void* y,
                           void* z)
{
...
void
gmvwrite_node_data_amr(int nxc, int nyc, int nzc, void* x0, void* y0, void* z0,
...
void
gmvwrite_nodev_fromfile(char* filename, long nndes)
{
  char* tmpbuf;
...
void
gmvwrite_nodev_data(void* nndes, void* x, void* y, void* z)
{
  int tmpnndes, i, j;
  ...
  if (rflag64) {
    if (filetype == IEEE_F) {
      ...
      for (i = 0; i < totnodes; i++) {
        tempxyz64[j] = tempx64[i];
        j++;
        tempxyz64[j] = tempy64[i];
        j++;
        tempxyz64[j] = tempz64[i];
        j++;
      }
...
void
gmvwrite_nodev_data_lstruct(void* nxv, void* nyv, void* nzv, void* x, void* y,
                            void* z)
{
...
gmvwrite_cells_amr(void* numcells, void* numtop, void* daughters)
{
  ...
  if (iflag64) {
    ...
    for (i = 0; i < tmpnumcells; i++) tmpdaughters[i] = *((int*)daughters + i);
    n_cells = tmpnumtop;
  }
...
void
gmvwrite_cells_fromfile(char* filename, long nclls)
{
  char* tmpbuf;
...
gmvwrite_cell_header(void* ncells)
{
  ...
  if (filetype == IEEE_F) fwrite(tmpname, sizeof(char), 8, fp);
  if (iflag64) {
    if (filetype == IEEE_F)
...
gmvwrite_cell_type(char cell_type[], int nverts, void* nodes)
{
  ...
  if (iflag64)
    ...
    for (count = 0; count < nverts; count++) tempnodes[count] = nodesptr[count];
  ...
  if (iflag64) {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < nverts; i++) fprintf(fp, " %ld", tempnodes64[i]);
      fprintf(fp, "\n");
    }
    free(tempnodes64);
  } else {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < nverts; i++) fprintf(fp, " %d", tempnodes[i]);
      fprintf(fp, "\n");
    }
    free(tempnodes);
  }
...
void
gmvwrite_general_cell_type(char cell_type[], int nverts[], int nfaces,
                           void* nodeids)
{
  ...
  for (count = 0; count < nfaces; count++) tempnverts[count] = nverts[count];

  for (count = 0; count < nfaces; count++) sumverts += nverts[count];
  ...
  if (filetype == IEEE_F) {
    ...
    fprintf(fp, "%s %d\n", cell_type, tmpnfaces);
    for (i = 0; i < nfaces; i++) fprintf(fp, " %d", tempnverts[i]);
    fprintf(fp, "\n");
  }
  if (iflag64) {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < sumverts; i++) fprintf(fp, " %ld", tempnodeids64[i]);
      fprintf(fp, "\n");
    }
    free(tempnodeids64);
  } else {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < sumverts; i++) fprintf(fp, " %d", tempnodeids[i]);
      fprintf(fp, "\n");
    }
    free(tempnodeids);
  }
...
void
gmvwrite_faces_fromfile(char* filename, long nclls)
{
  char* tmpbuf;
...
gmvwrite_face_data(int nverts, void* nodeids, void* cellid1, void* cellid2)
{
  ...
  if (iflag64) {
    if (filetype == IEEE_F) {
      ...
      for (i = 0; i < nverts; i++) fprintf(fp, " %ld", tempnodeids64[i]);
    ...
    if (filetype == IEEE_F) {
      ...
      for (i = 0; i < nverts; i++) fprintf(fp, " %d", tempnodeids[i]);
...
void
gmvwrite_vfaces_fromfile(char* filename, long nclls)
{
  char* tmpbuf;
...
void
gmvwrite_vface_header(void* nfaces)
{
  int tmpnfaces;
  long tmpnfaces64;
...
void
gmvwrite_vface_data(int nverts, int facepe, void* oppface, int oppfacepe,
                    void* cellid, void* nodeids)
{
...
void
gmvwrite_xfaces_fromfile(char* filename, long nfces, long nclls)
{
  char* tmpbuf;
...
void
gmvwrite_xface_header(void* nfaces)
{
  int tmpnfaces;
  long tmpnfaces64;
...
void
gmvwrite_xface_data(long totverts, void* nverts, void* nodeids, void* cellid,
                    void* oppface, void* facepe, void* oppfacepe)
{
  int *tempface = NULL, *tempnodeids = NULL, *tempptr = NULL;
  long *tempface64 = NULL, *tempnodeids64 = NULL, *tempptr64 = NULL, nc,
       tempnverts;
  int count;
  ...
  if (iflag64) {
    ...
    tempptr = nverts;
    for (count = 0; count < n_faces; count++) tempface[count] = tempptr[count];
    tempptr = nodeids;
    for (count = 0; count < totverts; count++)
...
gmvwrite_material_ids(int matids[], int data_type)
{
  ...
  for (count = 0; count < npts; count++) { tempmatids[count] = matids[count]; }
...
void
gmvwrite_variable_endvars(void)
{
  if (filetype == IEEE_F) {
...
gmvwrite_flag_data(int data_type, int flag_data[])
{
  ...
  for (i = 0; i < numthings; i++) tmpdata[i] = flag_data[i];
...
void
gmvwrite_polygons_data(int nverts, int matnum, void* x, void* y, void* z)
{
  ...
  if (rflag64) {
    if (filetype == IEEE_F) {
      ...
      for (i = 0; i < nverts; i++) fprintf(fp, "%lg ", tx64[i]);
      fprintf(fp, "\n");
      for (i = 0; i < nverts; i++) fprintf(fp, "%lg ", ty64[i]);
      fprintf(fp, "\n");
      for (i = 0; i < nverts; i++) fprintf(fp, "%lg ", tz64[i]);
      fprintf(fp, "\n");
    }
  } else {
    if (filetype == IEEE_F) {
      ...
      for (i = 0; i < nverts; i++) fprintf(fp, "%g ", tx[i]);
      fprintf(fp, "\n");
      for (i = 0; i < nverts; i++) fprintf(fp, "%g ", ty[i]);
      fprintf(fp, "\n");
      for (i = 0; i < nverts; i++) fprintf(fp, "%g ", tz[i]);
      fprintf(fp, "\n");
    }
  }
}
...
void
gmvwrite_polygons_endpoly(void)
{
...
void
gmvwrite_tracers_endtrace(void)
{
...
gmvwrite_nodeids(void* nodeids)
{
  ...
  if (iflag64) {
    ...
    for (i = 0; i < n_nodes; i++) tmpnodeids[i] = *((int*)nodeids + i);
  }
...
void
gmvwrite_nodeids_fromfile(char* filename)
{
  char* tmpbuf;
...
gmvwrite_cellids(void* cellids)
{
  ...
  if (iflag64) {
    ...
    for (i = 0; i < n_cells; i++) tmpcellids[i] = *((int*)cellids + i);
  }
...
void
gmvwrite_cellids_fromfile(char* filename)
{
  char* tmpbuf;
...
void
gmvwrite_surface_fromfile(char* filename, long nsrf)
{
  char* tmpbuf;
...
void
gmvwrite_surfvars_endsvar(void)
{
  if (filetype == IEEE_F) {
...
gmvwrite_surfflag_data(int flag_data[])
{
  ...
  for (i = 0; i < n_surf; i++) tmpdata[i] = flag_data[i];
...
void
gmvwrite_surfflag_endflag(void)
{
  if (filetype == IEEE_F) {
...
void
gmvwrite_units_fromfile(char* filename)
{
  char* tmpbuf;
...
void
gmvwrite_units_typehdr(int data_type, int numtypes)
{
  int tmpdata_type, tmpnumtypes;
...
void
gmvwrite_units_name(char fldname[], char unitname[])
{
  if (filetype == IEEE_F) {
...
void
gmvwrite_vinfo_name_data(int nelem, int nlines, char varname[], void* vids)
{
...
void
gmvwrite_traceids(int ntracers, void* traceids)
{
  ...
  if (iflag64) {
    ...
    for (i = 0; i < ntracers; i++) tmptraceids[i] = *((int*)traceids + i);
  }
...
void
gmvwrite_traceids_fromfile(char* filename)
{
  char* tmpbuf;
...
gmvwrite_faceids(void* faceids)
{
  ...
  if (iflag64) {
    ...
    for (i = 0; i < n_faces; i++) tempfaceids[i] = *((int*)faceids + i);
  }
...
void
gmvwrite_faceids_fromfile(char* filename)
{
  char* tmpbuf;
...
void
gmvwrite_group_fromfile(char* filename)
{
  char* tmpbuf;
                    ...
                    void* group_data)
{
  ...
  if (iflag64) {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < tmpnumgrp; i++) fprintf(fp, "%ld ", tmpdata64[i]);
      fprintf(fp, "\n");
    }
    free(tmpdata64);
  } else {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < tmpnumgrp; i++) fprintf(fp, "%d ", tmpdata[i]);
      fprintf(fp, "\n");
    }
    free(tmpdata);
  }
...
gmvwrite_surfids(void* surfids)
{
  ...
  if (iflag64) {
    ...
    for (i = 0; i < n_surf; i++) tmpsurfids[i] = *((int*)surfids + i);
  }
...
void
gmvwrite_subvars_name_data(int data_type, int numelem, char varname[],
...
void
gmvwrite_ghosts(int data_type, int numghst, void* ghost_data)
{
  ...
  if (iflag64) {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < tmpnumghst; i++) fprintf(fp, "%ld ", tmpdata64[i]);
      fprintf(fp, "\n");
    }
    free(tmpdata64);
  } else {
    if (filetype == IEEE_F)
      ...
      for (i = 0; i < tmpnumghst; i++) fprintf(fp, "%d ", tmpdata[i]);
      fprintf(fp, "\n");
    }
    free(tmpdata);
  }
