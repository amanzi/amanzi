...
TEST(HDF5_MPI) {
  ...
  auto Mesh = fac.create(0.0, 0.0, 0.0, 1.0, 1.0, 1.0, 8, 1, 1);
  ...
  */
  ...
  read_quantity = Teuchos::rcp(new Epetra_Vector(Mesh->getMap(AmanziMesh::Entity_kind::CELL, false)));
  std::cout << std::endl;
  restart_input->readData(*read_quantity, "cell_quantity");
  
  std::cout << "E>> read back:" << std::endl << *read_quantity;
  std::cout << "E>> cell map:" << std::endl << Mesh->getMap(AmanziMesh::Entity_kind::CELL, false);
