...
TEST(SILO_STRUCTURED)
{
  ...
  Teuchos::RCP<Amanzi::AmanziMesh::Mesh_MSTK> Mesh =
    Teuchos::rcp(new Amanzi::AmanziMesh::Mesh_MSTK(
      0.0, 0.0, 0.0, 1.0, 1.0, 1.0, 8, 1, 1, comm));

  unsigned int num_nodes = Mesh->num_entities(
    Amanzi::AmanziMesh::NODE, Amanzi::AmanziMesh::Parallel_type::OWNED);
  unsigned int num_cells = Mesh->num_entities(
...
TEST(SILO_POLYGONAL)
{
  ...
  // Write a file which contains both mesh and data.
  Teuchos::ParameterList plist;
  plist.set("file name base", "polygonal_vis");
  Amanzi::OutputSilo io(plist, Mesh, true, false);
