...
#ifndef AMANZI_PK_DOMAIN_FUNCTION_FIRSTORDER_EXCHANGE_DEPTH_HH_
#define AMANZI_PK_DOMAIN_FUNCTION_FIRSTORDER_EXCHANGE_DEPTH_HH_
...
class PK_DomainFunctionFirstOrderExchangeDepth : public FunctionBase,
                                            public Functions::UniqueMeshFunction {
 public:
  PK_DomainFunctionFirstOrderExchangeDepth(const Teuchos::RCP<const AmanziMesh::Mesh>& mesh,
                                      const Teuchos::ParameterList& plist,
                                      AmanziMesh::Entity_kind kind)
    : FunctionBase(plist),
      UniqueMeshFunction(mesh, AmanziMesh::Parallel_kind::OWNED),
      kind_(kind){};
  virtual ~PK_DomainFunctionFirstOrderExchangeDepth() = default;
...
template <class FunctionBase>
void
PK_DomainFunctionFirstOrderExchangeDepth<FunctionBase>::Init(const Teuchos::ParameterList& plist,
...
/* ******************************************************************
* Compute and distribute the result by FirstOrderExchangeDepth.
****************************************************************** */
template <class FunctionBase>
void
PK_DomainFunctionFirstOrderExchangeDepth<FunctionBase>::Compute(double t0, double t1)
{
  ...
  for (UniqueSpecList::const_iterator uspec = unique_specs_.at(kind_)->begin();
    ...
    for (auto c = ids->begin(); c != ids->end(); ++c) {
      ...
      for (int i = 0; i < nfun; ++i) {
        val_vec[i] = -(*(*uspec)->first->second)(args)[i] * tcc[i][*c] * tmp * ws[0][*c];
      }
      value_[*c] = val_vec;
    }
  }
