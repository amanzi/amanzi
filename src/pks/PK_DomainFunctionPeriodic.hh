...
/*
  Periodic BCs for transport PKs
  (supports 1D regular mesh only)

*/

#ifndef AMANZI_PK_DOMAIN_FUNCTION_PERIODIC_HH_
#define AMANZI_PK_DOMAIN_FUNCTION_PERIODIC_HH_
...
class PK_DomainFunctionPeriodic : public FunctionBase {
 public:
  PK_DomainFunctionPeriodic(const Teuchos::RCP<const AmanziMesh::Mesh>& mesh) : mesh_(mesh){};
  virtual ~PK_DomainFunctionPeriodic() = default;
  ...
  virtual DomainFunction_kind getType() const override { return DomainFunction_kind::PERIODIC; }
 ...
 private:
  AmanziMesh::Entity_ID entity_lid_;        // 1st cell
  AmanziMesh::Entity_ID entity_gid_out_;    // last cell
...
template <class FunctionBase>
void
PK_DomainFunctionPeriodic<FunctionBase>::Init(const Teuchos::ParameterList& plist,
                                             const std::string& keyword,
                                             AmanziMesh::Entity_kind region_kind)
{
  ...
  if (regions.size() != 1) {
    Errors::Message m;
    m << "Invalid \"regions\" list in processing periodic bc spec (must be of length 1).";
    Exceptions::amanzi_throw(m);
  }
...
template <class FunctionBase>
void
PK_DomainFunctionPeriodic<FunctionBase>::Compute(double t0, double t1)
{
