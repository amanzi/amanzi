...
template <class Base_t, class PK_Contained_t>
void
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::Functional(double t_old, double t_new,
        Teuchos::RCP<TreeVector> u_old, Teuchos::RCP<TreeVector> u_new,
        Teuchos::RCP<TreeVector> f)
{
  int i = 0;
  for (auto& pk : sub_pks_) {
    pk->Functional(t_old, t_new, u_old->SubVector(i), u_new->SubVector(i), f->SubVector(i));
    ++i;
  }
}
...
template <class Base_t, class PK_Contained_t>
int
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::ApplyPreconditioner(Teuchos::RCP<const TreeVector> u,
...
template <class Base_t, class PK_Contained_t>
double
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::ErrorNorm(Teuchos::RCP<const TreeVector> u,
...
template <class Base_t, class PK_Contained_t>
void
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::UpdatePreconditioner(double t,
...
template <class Base_t, class PK_Contained_t>
bool
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::IsAdmissible(Teuchos::RCP<const TreeVector> up)
{
  int i = 0;
  for (auto& pk : sub_pks_) {
...
template <class Base_t, class PK_Contained_t>
bool
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::ModifyPredictor(double h,
        Teuchos::RCP<const TreeVector> u0, Teuchos::RCP<TreeVector> u)
{
  bool modified = false;
  int i = 0;
  for (auto& pk : sub_pks_) {
...
template <class Base_t, class PK_Contained_t>
AmanziSolvers::FnBaseDefs::ModifyCorrectionResult 
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::ModifyCorrection(double h,
        ...
        Teuchos::RCP<TreeVector> du)
{
  ...
  int i = 0;
  for (auto& pk : sub_pks_) {
    auto local_modified = pk->ModifyCorrection(h, res->SubVector(i),
            u->SubVector(i), du->SubVector(i));
    ++i;
...
template <class Base_t, class PK_Contained_t>
void
PK_MixinMPCImplicit<Base_t,PK_Contained_t>::UpdateContinuationParameter(double lambda)
{
