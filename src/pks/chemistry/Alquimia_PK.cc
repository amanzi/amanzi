...
Alquimia_PK::Initialize()
{
  ...
  if (!aux_names_.empty()) {
    ...
    aux_output_ = Teuchos::rcp(new Epetra_MultiVector(mesh_->getMap(AmanziMesh::Entity_kind::CELL,false), n_total));
  } else {
                               ...
                               int cell)
{
  ...
  if (alq_mat_props_.saturation > saturation_tolerance_) {
    ...
    if (not success) {
      if (vo_->os_OK(Teuchos::VERB_MEDIUM)) {
        Teuchos::OSTab tab = vo_->getOSTab();
        *vo_->os() << "no convergence in cell: " << mesh_->getMap(AmanziMesh::Entity_kind::CELL,false).GID(cell) << std::endl;
      }
      return -1;
    }
  }
