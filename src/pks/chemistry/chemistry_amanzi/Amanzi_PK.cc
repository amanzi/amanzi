...
void Amanzi_PK::Initialize(const Teuchos::Ptr<State>& S)
{
  ...
  } catch (AmanziChemistry::ChemistryException& geochem_err) {
    ierr = 1;
  ...
  if (cp_list_->isSublist("Thermodynamic Database")) {
    ...
    if (tdb_list_.isParameter("format")) {
      ...
      if (database_format == "simple") {
        chem_ = new AmanziChemistry::SimpleThermoDatabase();
      } else {
...
void Amanzi_PK::SetupAuxiliaryOutput()
{
  ...
  for (unsigned int i = 0; i < nvars; i++) {
    ...
    if (index == -1) {
      ...
      Exceptions::amanzi_throw(AmanziChemistry::ChemistryInvalidInput(message.str()));
    } else {
