...
ThreePhaseEnergyEvaluator::ThreePhaseEnergyEvaluator(const ThreePhaseEnergyEvaluator& other) :
    ...
    bpor_key_(other.bpor_key_),
    sl_key_(other.sl_key_),
    mdl_key_(other.mdl_key_),
    iel_key_(other.iel_key_),
    sg_key_(other.sg_key_),
    mdg_key_(other.mdg_key_),
    ieg_key_(other.ieg_key_),
    si_key_(other.si_key_),
    mdi_key_(other.mdi_key_),
    iei_key_(other.iei_key_),
    dr_key_(other.dr_key_),
    ier_key_(other.ier_key_),
...
void ThreePhaseEnergyEvaluator::EvaluateField_(const Teuchos::Ptr<State>& S,
  ...
  const Epetra_MultiVector& n_l = *S->GetFieldData(mdl_key_)
    ->ViewComponent("cell",false);
  const Epetra_MultiVector& u_l = *S->GetFieldData(iel_key_)
    ->ViewComponent("cell",false);
  ...
  const Epetra_MultiVector& n_g = *S->GetFieldData(mdg_key_)
    ->ViewComponent("cell",false);
  const Epetra_MultiVector& u_g = *S->GetFieldData(ieg_key_)
    ->ViewComponent("cell",false);
  ...
  const Epetra_MultiVector& n_i = *S->GetFieldData(mdi_key_)
    ->ViewComponent("cell",false);
  const Epetra_MultiVector& u_i = *S->GetFieldData(iei_key_)
    ->ViewComponent("cell",false);

  const Epetra_MultiVector& phi = *S->GetFieldData(por_key_)
    ->ViewComponent("cell",false);
  const Epetra_MultiVector& phib = *S->GetFieldData(bpor_key_)
    ->ViewComponent("cell",false);
  const Epetra_MultiVector& u_rock = *S->GetFieldData(ier_key_)
    ->ViewComponent("cell",false);
  const Epetra_MultiVector& rho_rock = *S->GetFieldData(dr_key_)
    ->ViewComponent("cell",false);
