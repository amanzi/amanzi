...
double VaporInGas_Diffusion::Diffusion(double T, double p)
{
  return dref_ * std::pow(T / Tref_, 1.8);
}


double VaporInGas_Diffusion::DDiffusionDT(double T, double p)
{
  return dref_ * (1.8 / Tref_) * std::pow(T / Tref_, 0.8);
}


double VaporInGas_Diffusion::DDiffusionDp(double T, double p) {
  return 0.0;
}
