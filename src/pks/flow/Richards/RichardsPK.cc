...
namespace Amanzi {
  ...
  RichardsPK::RichardsPK(Teuchos::ParameterList &flow_plist, Teuchos::RCP<State> &S,
    ...
    S->require_field("pressure_dot", FIELD_LOCATION_CELL, "flow");

    // -- p-lambda vector on the super_map
    S->require_field("pressure_lambda", FIELD_LOCATION_CELL_AND_FACE, "flow");
    Teuchos::RCP<Epetra_MultiVector> plambda_ptr = S->get_field("pressure_lambda", "flow");
    solution_->PushBack(plambda_ptr);
    S->require_field("pressure_lambda_dot", FIELD_LOCATION_CELL_AND_FACE, "flow");
    ...
    // -- saturation
    S->require_field("saturation", FIELD_LOCATION_CELL, "flow");
    S->get_field_record("saturation")->set_io_vis(true);

    // -- parameters
    S->require_field("permeability", FIELD_LOCATION_CELL);
    S->require_field("relative_permeability", FIELD_LOCATION_CELL);
    S->require_field("porosity", FIELD_LOCATION_CELL);
  ...
  void RichardsPK::initialize(Teuchos::RCP<State> &S) {
    ...
    // -- IC for pressure
    Teuchos::RCP<Epetra_MultiVector> plambda = S->get_field("pressure_lambda", "flow");
    problem_->InitializePressure(plambda);
    S->get_field_record("pressure_lambda")->set_initialized();
    Teuchos::RPC<Epetra_MultiVector> pressure = GetCellView(plambda);
    S->set_field_pointer("pressure", "flow", pressure);
  ...
  void RichardsPK::state_to_solution(Teuchos::RCP<State>& S,
    ...
    (*soln)[0] = S->get_field("pressure_lambda", "flow");
  };
  ...
  void RichardsPK::state_to_solution(Teuchos::RCP<State>& S,
    ...
    (*soln)[0] = S->get_field("pressure_lambda", "flow");
    (*soln_dot)[0] = S->get_field("pressure_lambda_dot", "flow");
  };
  ...
  void RichardsPK::solution_to_state(Teuchos::RCP<TreeVector>& soln,
          Teuchos::RCP<State>& S) {
    S->set_field_pointer("pressure_lambda", "flow", (*soln)[0]);
    S->set_field_pointer("pressure", "flow", GetCellView((*soln)[0]));
  };
  ...
  void RichardsPK::solution_to_state(Teuchos::RCP<TreeVector>& soln,
    ...
    S->set_field_pointer("pressure_lambda", "flow", (*soln)[0]);
    S->set_field_pointer("pressure", "flow", GetCellView((*soln)[0]));
    S->set_field_pointer("pressure_lambda_dot", "flow", (*soln_dot)[0]);
    S->set_field_pointer("pressure_dot", "flow", GetCellView((*soln_dot)[0]));
  };
  ...
  void RichardsPK::fun(double t, Teuchos::RCP<const TreeVector> soln,
    ...
    // flux
    problem_->DerivePreconditionerMFD(soln, matrix_, T, 0.0, false); // stiffness matrix
    matrix_->computeResidual(soln, f);

    // accumulation
    Teuchos::RPC<Epetra_MultiVector> pressure = GetCellView((*soln)[0]);
    Teuchos::RPC<Epetra_MultiVector> pressure_dot = GetCellView((*soln_dot)[0]);
    Teuchos::RPC<Epetra_MultiVector> f_cell = GetCellView((*f)[0]);
    problem_->DeriveAccumulation(*pressure, *pressure_dot,
            *S_next_->get_field('porosity'), f_cell);
    f->Scale(-1.0);
  };
  ...
  void RichardsPK::precon(Teuchos::RCP<const TreeVector> u, Teuchos::RCP<TreeVector> Pu) {
    preconditioner_->ApplyInverse(*(*u)[0], *(*y)[0]);
  }

  /* ******************************************************************
   * computes a norm on u-du and returns the result
   *********************************************************************/
  double RichardsPK::enorm(Teuchos::RCP<const TreeVector> u,
                           Teuchos::RCP<const TreeVector> du) {
    double error_norm = 0.0;
    for (int n=0; n<u.MyLength(); n++) {
      double tmp = abs(du[n])/(atol_ + rtol_*abs(u[n]));
   ...
   *********************************************************************/
  void RichardsPK::update_precon(double t, Teuchos::RCP<const TreeVector> up,
          double h, int* errc) {
    problem_->DerivePreconditionerMFD(up, preconditioner_, t, h, true);
    errc = 0;
  }
