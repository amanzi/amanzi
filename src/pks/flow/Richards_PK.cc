  ...
  if (multiscale_model == "dual continuum discontinuous matrix") {
    ...
    if (nnodes > 1) {
      S_->Require<CV_t, CVS_t>("water_storage_ms_aux", Tags::DEFAULT, passwd_)
        .SetMesh(mesh_)
        ->SetGhosted(false)
        ->SetComponent("cell", AmanziMesh::Entity_kind::CELL, nnodes - 1);
...
Richards_PK::Initialize()
{
  ...
  op_acc_ = Teuchos::rcp(new Operators::PDE_Accumulation(AmanziMesh::Entity_kind::CELL, op_preconditioner_));
