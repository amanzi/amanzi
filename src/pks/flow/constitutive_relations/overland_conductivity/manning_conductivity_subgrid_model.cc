...
#include "manning_conductivity_subgrid_model.hh"

namespace Amanzi {
namespace Flow {

ManningConductivitySubgridModel::ManningConductivitySubgridModel(Teuchos::ParameterList& plist) :
    plist_(plist) {
...
double ManningConductivitySubgridModel::Conductivity(double depth, double slope, double coef) {
  if (depth <= 0.) return 0.;
...
double ManningConductivitySubgridModel::Conductivity(double depth, double slope, double coef, double pdd, double frac_cond, double beta) {
  if (pdd <= 0.) return 0.;
...
double ManningConductivitySubgridModel::DConductivityDDepth(double depth, double slope, double coef) {
  if (depth <= 0.) return 0.;
  double exponent = manning_exp_ + 1.0;
  double scaling = coef * std::sqrt(std::max(slope, slope_regularization_));
...
double ManningConductivitySubgridModel::DConductivityDDepth(double depth, double slope, double coef, double pd_depth, double frac_cond, double beta) {  
