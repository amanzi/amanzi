...
UnfrozenFractionModel::UnfrozenFractionModel(Teuchos::ParameterList& plist) :
    plist_(plist) {
  width_ = plist_.get<double>("transition width", 0.1);
...
double UnfrozenFractionModel::UnfrozenFraction(double temp) {
  ...
  if (adj_temp > 0.) {
    uf = 1.;
  } else if (adj_temp < -2*width_) {
    uf = 0.;
  } else {
    uf = (std::sin(pi_/2. * (adj_temp + width_)/width_) + 1.)/2.;
  }
  return uf;
}
...
double UnfrozenFractionModel::DUnfrozenFractionDT(double temp) {
  ...
  if (adj_temp > 0.) {
    ...
    duf = std::cos(pi_/2. * (adj_temp + width_)/width_)/2. * pi_/2. / width_;
  }
  return duf;
}
