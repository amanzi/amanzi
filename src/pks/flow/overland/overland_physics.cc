...
void Richards::AddAccumulation_(const Teuchos::RCP<CompositeVector>& g) {
  ...
  for (int c=0; c!=c_owned; ++c) {
    ...
    double wc1 = wc_liq1 * (*poro1)(c) * (*cell_volume1)(c);
    ...
    double wc0 = wc_liq0 * (*poro0)(c) * (*cell_volume0)(c);
...
void Richards::UpdateSecondaryVariables_(const Teuchos::RCP<State>& S) {
  ...
  Teuchos::RCP<const CompositeVector> mol_dens_liq = S->GetFieldData("molar_density_liquid");
  Teuchos::RCP<const CompositeVector> visc_liq = S->GetFieldData("viscosity_liquid");
