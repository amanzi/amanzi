...
void SnowDistribution::SetupSnowDistribution_(const Teuchos::Ptr<State>& S) {
  ...
  mfd_pc_plist.set("nonlinear coefficient", "upwind: face");
...
void SnowDistribution::SetupPhysicalEvaluators_(const Teuchos::Ptr<State>& S) {
  ...
  S->RequireField(Keys::getKey(domain_,"snow_skin_potential"))->SetMesh(mesh_)->SetGhosted()
  ...
  if (update_perm) {
    if (upwind_method_ == Operators::UPWIND_METHOD_TOTAL_FLUX) {
      ...
      Teuchos::RCP<const CompositeVector> potential =
          S->GetFieldData(Keys::getKey(domain_,"snow_skin_potential"));
      face_matrix_diff_->UpdateFlux(*potential, *flux_dir);
    }
