...
// -----------------------------------------------------------------------------
void
Lake_Thermo_PK::ApplyBoundaryConditions_(const Teuchos::Ptr<CompositeVector>& u) {
...
double Lake_Thermo_PK::BoundaryValue(const Teuchos::RCP<const Amanzi::CompositeVector>& solution, int face_id){
  double value=0.;
...
void Lake_Thermo_PK::set_states(const Teuchos::RCP<State>& S,
                                        const Teuchos::RCP<State>& S_inter,
                                        const Teuchos::RCP<State>& S_next) {
  Lake_Thermo_PK::set_states(S, S_inter, S_next);
}


int Lake_Thermo_PK::BoundaryDirection(int face_id) {
  AmanziMesh::Entity_ID_List cells;
  mesh_->face_get_cells(face_id, AmanziMesh::Parallel_type::ALL, &cells);
  AMANZI_ASSERT(cells.size() == 1);
...
// state.
// -----------------------------------------------------------------------------
void Lake_Thermo_PK::ChangedSolution(const Teuchos::Ptr<State>& S) {
  if (S == Teuchos::null) {
...
// -----------------------------------------------------------------------------
void Lake_Thermo_PK::ChangedSolution() {
  solution_evaluator_->SetFieldAsChanged(S_next_.ptr());
};
