...
MPCLake1D::MPCLake1D(Teuchos::ParameterList& pk_tree,
...
MPCLake1D::Setup(const Teuchos::Ptr<State>& S) {
  ...
  if (pk0_method != pk1_method) {
    Errors::Message msg("MPC_Lake1D: for Lake1D problems, due to issues in Jacobians, the flow and energy discretization methods must be the same.");
...
int MPCLake1D::ApplyPreconditioner(Teuchos::RCP<const TreeVector> r,
...
MPCLake1D::ModifyCorrection(double h, Teuchos::RCP<const TreeVector> r,
