...
void 
WeakMPCSemiCoupledDeform::generalize_inputspec(const Teuchos::Ptr<State>& S){
 ...
 //  Teuchos::Array<std::string> pks_order = plist_->get<Teuchos::Array<std::string> >("PKs order");
  ...
  Teuchos::Array<std::string> pk_order = top_pk_mpc.get<Teuchos::Array<std::string> >("PKs order"); // sublist of PK that corresponds to each processor
 ...
 // Generalizing PKs
  ...
  for(int i=2; i<len;i++){ 
    ...
    Teuchos::ParameterList& pss_list_cyc = temp0.sublist("PSS"+d_num);
    ...
    Teuchos::ParameterList subsid_list = pks_list_main.sublist(pk2_order[0]); // subsidence PK
    subsid_list.setName(subsid_pk);
    subsid_list.set("primary variable", getKey(domain_ss,"base_porosity"));
    subsid_list.set("domain name",domain_ss);
    ...
    //------------------- PSS
    Teuchos::ParameterList pss_list = pks_list_main.sublist(pk3_order[1]); //PSS list
    ...
    Teuchos::Array<std::string> names = pks_list_main
      .sublist(pk3_order[1]).get<Teuchos::Array<std::string> >("PKs order"); // get the PK order in PSS={SSF,SSE,SF,SE}
