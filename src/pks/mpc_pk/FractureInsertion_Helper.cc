...
/* *******************************************************************
* Populate advective coupling fluxes for T-based formulation
******************************************************************* */
void
UpdateEnthalpyCouplingFluxes_T(
...
******************************************************************* */
void
UpdateEnthalpyCouplingFluxes_H(
  const Teuchos::RCP<State>& S,
  const Teuchos::RCP<const AmanziMesh::Mesh>& mesh_matrix,
  const Teuchos::RCP<const AmanziMesh::Mesh>& mesh_fracture,
  const std::vector<Teuchos::RCP<Operators::PDE_CouplingFlux>>& adv_coupling_ops,
...
/* *******************************************************************
* Populate thermo coupling fluxes for H-based formulation
******************************************************************* */
void
UpdateThermoCouplingFluxes_H(
  const Teuchos::RCP<State>& S,
  const Teuchos::RCP<const AmanziMesh::Mesh>& mesh_matrix,
  const Teuchos::RCP<const AmanziMesh::Mesh>& mesh_fracture,
  const std::vector<Teuchos::RCP<Operators::PDE_CouplingFlux>>& thermo_coupling_ops,
  bool flag)
{
  Key temperature_key_m("temperature"), temperature_key_f("fracture-temperature");
  Key enthalpy_key_m("enthalpy"), enthalpy_key_f("fracture-enthalpy");
  Key heat_diffusion_to_matrix_key("fracture-heat_diffusion_to_matrix");
  ...
  const auto& kn = *S->Get<CompositeVector>(heat_diffusion_to_matrix_key).ViewComponent("cell");

  Teuchos::RCP<const Epetra_MultiVector> dTdH_m, dTdH_f;
  if (!flag) {
    const auto& tmp1 = S->GetDerivative<CompositeVector>(
      temperature_key_m, Tags::DEFAULT, enthalpy_key_m, Tags::DEFAULT);
    tmp1.ScatterMasterToGhosted("cell");
    dTdH_m = tmp1.ViewComponent("cell");

    const auto& tmp2 = S->GetDerivative<CompositeVector>(
      temperature_key_f, Tags::DEFAULT, enthalpy_key_f, Tags::DEFAULT);
    tmp2.ScatterMasterToGhosted("cell");
    dTdH_f = tmp2.ViewComponent("cell");
  }

  // update coupling terms for thermoection
  int ncells_owned_f =
    mesh_fracture->getNumEntities(AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_kind::OWNED);
  auto values1 = std::make_shared<std::vector<double>>(2 * ncells_owned_f, 0.0);
  auto values2 = std::make_shared<std::vector<double>>(2 * ncells_owned_f, 0.0);
  ...
  for (int c = 0; c < ncells_owned_f; ++c) {
    ...
    for (int k = 0; k < ncells; ++k) {
      ...
      // since we multiply by temperature, the model for the flux is
      // k(T_m/H_m) H_m - k(T_f/H_f) H_f for both matrix and preconditioner
      int c1 = cells[k];
      factor = (flag) ? T_m[0][c1] / H_m[0][c1] : (*dTdH_m)[0][c1];
      (*values1)[np] = tmp * factor;

      
      factor = (flag) ? T_f[0][c] / H_f[0][c] : (*dTdH_f)[0][c];
      (*values2)[np] = tmp * factor;
  ...
  // setup coupling operators with new data
  thermo_coupling_ops[0]->Setup(values1, 1.0);
  thermo_coupling_ops[0]->UpdateMatrices(Teuchos::null, Teuchos::null);

  thermo_coupling_ops[1]->Setup(values2, -1.0);
  thermo_coupling_ops[1]->UpdateMatrices(Teuchos::null, Teuchos::null);

  thermo_coupling_ops[2]->Setup(values1, -1.0);
  thermo_coupling_ops[2]->UpdateMatrices(Teuchos::null, Teuchos::null);
