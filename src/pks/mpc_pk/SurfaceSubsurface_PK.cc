...
// Constructor
// -----------------------------------------------------------------------------
SurfaceSubsurface_PK::SurfaceSubsurface_PK(Teuchos::ParameterList& pk_tree,
...
{
  int nsurf_nodes = mesh_surface_->getNumEntities(AmanziMesh::Entity_kind::NODE, AmanziMesh::Parallel_type::OWNED);
  int nsurf_cells = mesh_surface_->getNumEntities(AmanziMesh::Entity_kind::CELL, AmanziMesh::Parallel_type::OWNED);
...
// -----------------------------------------------------------------------------
void
SurfaceSubsurface_PK::CommitStep(double t_old, double t_new, const Tag& tag)
{
...
// -----------------------------------------------------------------------------
bool
SurfaceSubsurface_PK::AdvanceStep(double t_old, double t_new, bool reinit)
{
  bool fail = false;
  ...
  if (vo_->getVerbLevel() >= Teuchos::VERB_HIGH) {
    ...
    *vo_->os() << "shallow water PK made " << nsubcycles
               << " subcycles,  water exchange=" << water_exchange << " m^3" << std::endl;
  }
