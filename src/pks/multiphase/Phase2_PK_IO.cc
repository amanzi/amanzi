/*
  ...
  This routine implements various I/O components of Phase2_PK which
  are common to Phase2_PK and Saturation_PK.
...
* on each processor.                                                     
****************************************************************** */
void Phase2_PK::ProcessParameterList(Teuchos::ParameterList& plist)
{
 ...
 // Create the BC objects.
  if (!plist.isSublist("boundary conditions")) {
    Errors::Message msg;
    msg << "Phase1_PK: problem does not have <boundary conditions> list\n";
  ...
  //bc_flux_phase1_ = bc_factory.CreateMassFlux(bc_submodel_p_, 1);
  bc_flux_phase2_ = bc_factory.CreateMassFlux(bc_submodel_s_, 2);
  bc_pressure_ = bc_factory.CreatePressure(bc_submodel_p_);
  bc_saturation_ = bc_factory.CreateSaturation(bc_submodel_s_); 
  
  // Create the source object if any
  /*
  if (!plist.isSublist("source terms")) {
    Errors::Message msg;
    msg << "Phase2_PK: problem does not have <source terms> list\n";
    Exceptions::amanzi_throw(msg);
  } else {
...
**************************************************************** */
/*
void Phase2_PK::ProcessSublistTimeInterval(
...
**************************************************************** */
/*
void Phase2_PK::ProcessSublistTimeIntegration(
    Teuchos::ParameterList& list, const std::string name, TI_Specs& ti_specs)
{
  ...
  if (list.isSublist(name)) {
    ...
    msg << "\nMultiphase PK: specified time integration sublist does not exist.";
    Exceptions::amanzi_throw(msg);
  }
}
*/
...
**************************************************************** */
/*
void Phase2_PK::ProcessStringTimeIntegration(const std::string name, int* method)
{
  Errors::Message msg;
  ...
  if (name == "Picard") {
    ...
    *method = FLOW_TIME_INTEGRATION_BDF2;
    ...
    msg << "Multiphase PK: time integration method \"" << name.c_str() << "\" is not known.";
    Exceptions::amanzi_throw(msg);
  }
}
*/
...
**************************************************************** */
/*
std::string Phase2_PK::FindStringPreconditioner(const Teuchos::ParameterList& list)
{   
  Errors::Message msg;
  std::string name; 
  ...
  if (list.isParameter("preconditioner")) {
    ...
    msg << "Multiphase PK: parameter <preconditioner> is missing either in TI or LS list.";
    Exceptions::amanzi_throw(msg);
  }

  if (! preconditioner_list_.isSublist(name)) {
    msg << "Multiphase PK: preconditioner \"" << name.c_str() << "\" does not exist.";
    Exceptions::amanzi_throw(msg);
  }
  return name;
}
...
**************************************************************** */
/*
std::string Phase2_PK::FindStringLinearSolver(const Teuchos::ParameterList& list)
{   
  Errors::Message msg;
  std::string name; 
  ...
  if (list.isParameter("linear solver")) {
    ...
    msg << "Multiphase PK: time integrator does not define <linear solver>.";
    Exceptions::amanzi_throw(msg);
  }

  if (! linear_operator_list_.isSublist(name)) {
    msg << "Multiphase PK: linear solver \"" << name.c_str() << "\" does not exist.";
    Exceptions::amanzi_throw(msg);
  }
  return name;
}
...
**************************************************************** */
/*
void Phase2_PK::ProcessStringErrorOptions(Teuchos::ParameterList& list, int* control)
{
  *control = 0;
    ...
    for (int i=0; i < options.size(); i++) {
      if (options[i] == "pressure") {
        ...
        *control += FLOW_TI_ERROR_CONTROL_RESIDUAL;
        ...
        Errors::Message msg;
        msg << "Multiphase PK: unknown error control option has been specified.";
        Exceptions::amanzi_throw(msg);
      }
    }
  }
...
**************************************************************** */
void Phase2_PK::ProcessStringSourceDistribution(const std::string name, int* method)
{
  if (name != "none") {
    Errors::Message msg;
...
**************************************************************** */
/*
void Phase2_PK::OutputTimeHistory(
    const Teuchos::ParameterList& plist, std::vector<dt_tuple>& dT_history)
{
  if (plist.isParameter("plot time history") && 
    ...
    *vo_->os() << "saving time history in file flow_dt_history.txt..." << std::endl;
    ...
    char file_name[30];
    sprintf(file_name, "flow_dt_history_%d.txt", ti_phase_counter_++);
