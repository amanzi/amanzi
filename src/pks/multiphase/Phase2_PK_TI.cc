...
****************************************************************** */
void Phase2_PK::Functional(double t_old, double t_new, 
              ...
              Teuchos::RCP<TreeVector> f)
{
  ...
  //std::cout << "op_matrix phase2: \n";
  //std::cout << *op_matrix_->global_operator()->A();
  //std::cout << "rhs: \n";
  //op_matrix_->global_operator()->rhs()->Print(std::cout);
  //op_matrix_->global_operator()->A()->Apply(*pressure_phase2_->ViewComponent("cell"), *f->Data()->ViewComponent("cell"));
  //f->Data()->Update(-1.0, *op_matrix_->global_operator()->rhs(), 1.0);
  op_matrix_->global_operator()->ComputeNegativeResidual(*pressure_phase2_, *f->Data());
  //std::cout << "Phase2_PK residual before adding accumulation term: " << *f->Data()->ViewComponent("cell") << "\n";
  ...
  //std::cout << "Phase2_PK residual: " << f_cell << "\n";
}

/* ******************************************************************
* Apply preconditioner inv(B) * X.                                                 
****************************************************************** */
int Phase2_PK::ApplyPreconditioner(Teuchos::RCP<const TreeVector> u, 
...
****************************************************************** */
void Phase2_PK::UpdatePreconditioner(double Tp, Teuchos::RCP<const TreeVector> u, double dTp)
{
  // Get the new pressure and saturation from the solution tree vector
  ...
  */
  ...
  rel_perm_n_->dKdS()->Scale(-rho_/mu_); // Must scale derivative by -1
  ...
  if (include_capillary_) {
    ...
    upwind_pc_->Compute(*upwind_velocity_, *upwind_velocity_, bc_model_s_, bc_value_s_, 
  ...
  //std::cout << "Phase2_PK, prec matrix1: " << *op1_preconditioner_->global_operator()->A() << "\n";
  //std::cout << "Phase2_PK, prec matrix2: " << *op2_preconditioner_->global_operator()->A() << "\n";
  //op_sum_->global_operator()->SymbolicAssembleMatrix();
  //op_sum_->global_operator()->AssembleMatrix();
  //std::cout << "Phase2_PK, prec sum matrix: " << *op_sum_->global_operator()->A() << "\n";
} // End UpdatePreconditioner


void Phase2_PK::NumericalJacobian(double t_old, double t_new, 
