...
/*
2 component (Hydrogen, Water), 2 phase with assumptions as in [Gharbia, Jaffre' 14]. 2D physical scenario with advection BCs.
*/
...
run_test(const std::string& domain, const std::string& filename)
{
  ...
  if (MyPID == 0)
    std::cout << "Test: multiphase pk, 2D physical scenario with advection BCs" << std::endl;
...
{
  run_test("2D", "test/multiphase_advection.xml");
}
