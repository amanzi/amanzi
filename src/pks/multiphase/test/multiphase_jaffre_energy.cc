...
/*
  MoMas benchmark with energy: 2 component Hydrogen (H) and water (W) to show gas 
  phase appearance/disappearance with assumptions/model as in [Gharbia, Jaffre' 14].
  The system is further coupled with the energy equation. 
  Primary variables are pressure liquid, saturation liquid, molar density 
  of hydrogen in liquid phase, and temperature.
*/
...
run_test(const std::string& domain, const std::string& filename)
{
  ...
  while (t < tend && iloop < 100000) {
    ...
    if (iloop % 2 == 0) {
      ...
      const auto& u5 = *S->Get<CompositeVector>("temperature").ViewComponent("cell");
      ...
      io->WriteVector(*u5(0), "temperature", AmanziMesh::Entity_kind::CELL);
      io->FinalizeCycle();
