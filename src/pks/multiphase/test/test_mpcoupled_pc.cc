...
TEST(MULTIPHASE_MPCOUPLED) {
  ...
  std::string xmlFileName = "test/test_mpcoupled_pc.xml";
  ...
  //double dT = 1728000;
  double t_sim = 0.0;
  //double t_final = 25920000;
  double t_final = time_list->get<double>("end time", 0.0);
  double dT = time_list->get<double>("time step", 0.0);
  ...
  if (MyPID == 0) {
    GMV::open_data_file(*mesh, (std::string)"test_mpcoupled_pc.gmv");
    GMV::start_data();
