...
TEST(MULTIPHASE_PHASE1) {
  ...
  if (MyPID == 0) std::cout << "Test: multiphase phase1, uniform rectangular mesh" << std::endl;

  /* read parameter list */
  //std::string xmlFileName = "test/test_pressure_pk.xml";
  std::string xmlFileName = "test/test_phase1_pk.xml";
  ...
  for (int c = 0; c < ncells; c++) {
    ...
    if (xc[0] < 0.35) {
      saturation_cell[0][c] = 0.2;
    }
    else {
      saturation_cell[0][c] = 0.8;
    }
  ...
  */
  ...
  // Create pressure PK
  Multiphase::Phase1_PK* PPK = new Multiphase::Phase1_PK(*pk_tree_list, parameter_list, S, u_new);
  S->Setup();
  S->InitializeFields();

  Teuchos::RCP<CompositeVector> pressure = Teuchos::rcp(new CompositeVector(*S->GetFieldData("phase1_pressure")));
  Teuchos::RCP<CompositeVector> saturation = Teuchos::rcp(new CompositeVector(*S->GetFieldData("phase1_saturation")));
  Teuchos::RCP<CompositeVector> pressure_old = Teuchos::rcp(new CompositeVector(*S->GetFieldData("phase1_pressure")));
  Teuchos::RCP<CompositeVector> saturation_old = Teuchos::rcp(new CompositeVector(*S->GetFieldData("phase1_saturation")));
  saturation->Scale(-1.0);
  saturation->Shift(1.0);
  saturation_old->Scale(-1.0);
  saturation_old->Shift(1.0);
  ...
  PPK->Initialize();
  PPK->CommitStep(0.0, 0.1);
  /*
  Epetra_MultiVector& phase_velocity = *S->GetFieldData("phase1_velocity", passwd)->ViewComponent("face", false);
  Point vel(1.0, 0.0);
