...
TEST(MULTIPHASE_SATURATION) {
  ...
  /* read parameter list */
  std::string xmlFileName = "test/test_seq_q5.xml";
  ...
  Amanzi::timer_manager.add("Pressure Solve", Amanzi::Timer::ACCUMULATE);
  bool failed = true;
  while (!((t_sim > t_final) || abs(t_sim - t_final) < 1.0e-8)) {
    Amanzi::timer_manager.start("Pressure Solve");
    PPK->AdvanceStep(t_sim, t_sim + dT, false);
    Amanzi::timer_manager.stop("Pressure Solve");
  ...
  Epetra_MultiVector& pres = *S->GetFieldData("pressure", passwd)->ViewComponent("cell", false);
  if (MyPID == 0) {
    GMV::open_data_file(*mesh, (std::string)"test_seq_q5.gmv");
    GMV::start_data();
    GMV::write_cell_data(ws_c, 0, "water_saturation");
    GMV::write_cell_data(pres, 0, "pressure");
    GMV::close_data_file();
  }

  ifstream file_in;
  file_in.open("matlab_mrst_q5.txt");
  int nc = 0;
  while (!file_in.eof()) {
    if (nc > ws_c.MyLength()) break;
    double p,s;
    file_in >> p >> s;
    //printf("%f %f %f %f %f %f \n", p,s,xwg,xag,xwl,xal);
    pres[0][nc] = p;
  ...
  if (MyPID == 0) {
    GMV::open_data_file(*mesh, (std::string)"test_seq_q5_mrst.gmv");
    GMV::start_data();
    GMV::write_cell_data(ws_c, 0, "water_saturation");
    GMV::write_cell_data(pres, 0, "pressure");
    GMV::close_data_file();
  ...
  file_in.open("matlab_mrst_q5_pc.txt");
  nc = 0;
  while (!file_in.eof()) {
    if (nc > ws_c.MyLength()) break;
    double p,s;
  ...
  if (MyPID == 0) {
    GMV::open_data_file(*mesh, (std::string)"test_seq_q5_mrst_pc.gmv");
    GMV::start_data();
    GMV::write_cell_data(ws_c, 0, "water_saturation");
    GMV::write_cell_data(pres, 0, "pressure");
    GMV::close_data_file();
