...
// Inital Conditions
// ---- ---- ---- ---- ---- ---- ---- ----
void dry_bed_1D_setIC(Teuchos::RCP<const Amanzi::AmanziMesh::Mesh> mesh, Teuchos::RCP<Amanzi::State>& S, int icase)
{
  double pi = M_PI;
...
void RunTest(int icase)
{
  ...
  if (MyPID == 0) {
    std::cout<<"Test: 1D Shallow water: Flow over dry bed (with B = 0, B > 0 cases) "<<std::endl;
  }
    
  // Read parameter list
  std::string xmlFilename;
  if (icase == 1) {
    xmlFilename = "test/shallow_water_1D_dry_bed.xml";
  }
