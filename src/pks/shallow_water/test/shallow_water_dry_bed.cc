...
// Inital Conditions
// ---- ---- ---- ---- ---- ---- ---- ----
void dry_bed_setIC(Teuchos::RCP<const Amanzi::AmanziMesh::Mesh> mesh, Teuchos::RCP<Amanzi::State>& S, int icase)
{
  double pi = M_PI;
  ...
  for (int c = 0; c < ncells_wghost; ++c) {
    ...
    if (icase == 1) {
      ...
      if ( (xc[0] - 5.0)*(xc[0] - 5.0) + (xc[1] - 5.0)*(xc[1] - 5.0) < 0.5*0.5 + 1.e-12) {
...
void RunTest(int icase)
{
  ...
  if (MyPID == 0) {
    std::cout<<"Test: Shallow water: Flow over dry bed (with B = 0, B > 0 cases) "<<std::endl;
  }
  ...
  if (icase == 1) {
    // Rectangular mesh
    mesh = meshfactory.create(0.0, 0.0, 10.0, 1.0, 80, 1, request_faces, request_edges);
  } else if (icase == 2) {
