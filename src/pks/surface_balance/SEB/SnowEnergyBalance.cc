...
void SurfaceEnergyBalance::SnowPackDensity (LocalData& row) {
    ...
    double deniminator=row.st_energy.Ps+row.st_energy.ht_snow+row.st_energy.CiL;
row.st_energy.density_snow=((row.st_energy.Ps*row.st_energy.density_freshsnow)+(row.st_energy.ht_snow*row.st_energy.density_snow)+(row.st_energy.CiL*row.st_energy.density_frost))/(deniminator);
...
// Caculate Vapor pressure and dewpoint temperature from Air
    SurfaceEnergyBalance::VaporCalc(seb.vp_air);
//FILL IN DATA STRUCTURE NEEDED FOR PARAMETERS IN ENERGY BALANCE   

    
// Find Albedo
    SurfaceEnergyBalance::AlbedoCalc(seb); // Calculating Surface Albedo
    seb.st_energy.fQswIn=(1-seb.st_energy.albedo_value)*seb.st_energy.QswIn;
    
//Find Thermal Conductivity of snow
    SurfaceEnergyBalance::ThermalConductSnow (seb); //Calculating themeral conductifity of snow
 
//Convert snow leath Ps (m) in SWE lenght
    SurfaceEnergyBalance::SWE(seb);
   
if (seb.st_energy.ht_snow>0) {// ### IF THERE IS SNOW CHECK ### IF THERE IS SNOW CHECK ### IF THERE IS SNOW CHECK ##################
    SurfaceEnergyBalance::BisectionEnergyCalc(seb); //Solving for snowsurface temperature and Energy balance equation
    ...
    if (seb.st_energy.Ts>273.15) {// If snowsurface is warmer then ice
        ...
        SurfaceEnergyBalance::MeltEnergyCalc(seb); // Recaculating Energy Balance when melt in occuring
        seb.st_energy.funcall="MELT";
    }//End if snowsurface is warmer then ice
    
    SurfaceEnergyBalance::MeltSublRateCalc(seb); //Calculating melt, sublimation, and condisitation rate
    SurfaceEnergyBalance::DeltaSnowPack(seb);  //CALCULATE THE CHANGE IN SNOWPACK DEPTH
...
// Make sure proper mass of snowpack water gets delivered to ATS
    if (seb.st_energy.ht_snow < ((seb.st_energy.Ps - seb.st_energy.TotwLoss)*-1)) {
        SurfaceEnergyBalance::WaterMassCorr(seb);
...
// CHECK FOR SMALL MELTING SNOWPACK, WHICH CAN CAUSE SUPPER HIGH QC Qc = -Ks*(Ts-Tb)/Zs;  --> blows up
    if (seb.st_energy.ht_snow<=0.009) { 
        if (seb.st_energy.Ts==273.15) {
            SurfaceEnergyBalance::TeenyTinySnowPack(seb);
         }
...
if (seb.st_energy.ht_snow<=0) {// ### IF THERE IS NO SNOW CHECK  ###  ### IF THERE IS NO SNOW CHECK  ###  #################
     SurfaceEnergyBalance::GroundEnergyCalc(seb);
     seb.st_energy.funcall="BARE";
     SurfaceEnergyBalance::EvapCalc(seb);
...
SurfaceEnergyBalance::SnowPackDensity(seb);

// UDATE SNOWPACK HEIGHT
seb.st_energy.HoldZs=seb.st_energy.ht_snow;
SurfaceEnergyBalance::SnowPackCalc(seb);
