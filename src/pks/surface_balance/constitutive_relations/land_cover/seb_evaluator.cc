    ...
    plist_(plist)
{
  ...
  sg_albedo_key_ = Keys::readKey(plist, domain_, "ground albedos", "albedos");
  dependencies_.insert(sg_albedo_key_);
  sg_emissivity_key_ = Keys::readKey(plist, domain_, "ground emissivities", "emissivities");
  dependencies_.insert(sg_emissivity_key_);
  area_frac_key_ = Keys::readKey(plist, domain_snow_, "snow area fraction", "area_fraction");
                             ...
                             const std::vector<Teuchos::Ptr<CompositeVector> >& results)
{
  ...
  for (unsigned int c=0; c!=ncells; ++c) {
    ...
    if (area_fracs[0][c] < 1) {
      ...
      mass_source[0][c] += (1-area_fracs[0][c]) * flux.M_surf;
      energy_source[0][c] += (1-area_fracs[0][c]) * flux.E_surf * 1.e-6; // convert to MW/m^2
      ...
      ss_mass_source[0][cells[0]] += (1-area_fracs[0][c]) * ss_mass_source_l;
      ...
      snow_source[0][c] += (1-area_fracs[0][c]) * flux.M_snow;
      new_snow[0][c] += (1-area_fracs[0][c]) * met.Ps;
      ...
      // diagnostics
      if (diagnostics_) {
        (*evap_rate)[0][c] -= (1-area_fracs[0][c]) * mb.Me;
        (*qE_sh)[0][c] += (1-area_fracs[0][c]) * eb.fQh;
        (*qE_lh)[0][c] += (1-area_fracs[0][c]) * eb.fQe;
        (*qE_lw_out)[0][c] += (1-area_fracs[0][c]) * eb.fQlwOut;
        (*qE_cond)[0][c] += (1-area_fracs[0][c]) * eb.fQc;
        (*albedo)[0][c] += (1-area_fracs[0][c]) * surf.albedo;
    ...
    if (area_fracs[0][c] > 0.) {
      ...
      met.Ps = Psnow[0][c] / area_fracs[0][c];

      SEBPhysics::SnowProperties snow;
      snow.height = snow_depth[0][c] / area_fracs[0][c]; // all snow on this patch
      ...
      energy_source[0][c] += area_fracs[0][c] * flux.E_surf * 1.e-6; // convert to MW/m^2 from W/m^2
      snow_source[0][c] += area_fracs[0][c] * flux.M_snow;

      new_snow[0][c] += std::max(met.Ps + mb.Me, 0.) * area_fracs[0][c];
...
SEBEvaluator::EnsureCompatibility(const Teuchos::Ptr<State>& S)
{
  ...
  for (auto dep_key : dependencies_) {
    ...
    } else if (dep_key == sg_albedo_key_ || dep_key == sg_emissivity_key_) {
