...
createRun(const std::string &mpc_A_name, const std::string &pk_A_name,
  ...
  mpc_a->SetChildren(std::vector<Teuchos::RCP<MPC_A_Children_t>>{pk_a, mpc_b});
...
SUITE(PKS_MPC_THREE) {
  ...
  TEST(SEQUENTIAL_ABC) {
    using PK_A_t = PK_Explicit_Adaptor<PK_ODE_Explicit<
        PK_MixinExplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorA>>;
    using PK_B_t = PK_Explicit_Adaptor<PK_ODE_Explicit<
        PK_MixinExplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorB>>;
    using PK_C_t = PK_Explicit_Adaptor<PK_ODE_Explicit<
        PK_MixinExplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorC>>;
    ...
    auto run = createRun<MPC_t, PK, PK_A_t, MPC_t, PK, PK_B_t, PK_C_t>(
        "ABC weak forward euler", "A, forward euler", "BC weak forward euler",
        "B, forward euler", "C, forward euler");
  ...
  TEST(SEQUENTIAL_A_IMPLICIT_BC) {
    using PK_A_t = PK_Explicit_Adaptor<PK_ODE_Explicit<
        PK_MixinExplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorA>>;
    using PK_B_t = PK_Implicit_Adaptor<PK_ODE_Implicit<
        PK_MixinImplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorB>>;
    using PK_C_t = PK_Implicit_Adaptor<PK_ODE_Implicit<
        PK_MixinImplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorC>>;
    ...
    auto run =
        createRun<MPC_A_t, PK, PK_A_t, MPC_B_t, PK_Implicit<TreeVector>, PK_B_t,
                  PK_C_t>("ABC weak forward euler", "A, forward euler",
  ...
  TEST(IMPLICIT_ABC) {
    using PK_A_t = PK_Implicit_Adaptor<PK_ODE_Implicit<
        PK_MixinImplicit<PK_MixinLeafCompositeVector<PK_Default>>,
        DudtEvaluatorA>>;
    using PK_B_t = PK_Implicit_Adaptor<PK_ODE_Implicit<
        PK_MixinImplicit<PK_MixinLeafCompositeVector<PK_Default>>,
    ...
    auto run = createRun<MPC_t, PK_Implicit<TreeVector>, PK_A_t, MPC_t,
                         PK_Implicit<TreeVector>, PK_B_t, PK_C_t>(
        "BC global implicit variable", "A, backward euler",
        "BC global implicit variable", "B, backward euler",
        "C, backward euler");
    auto nsteps = run_test(run->S, run->pk);
