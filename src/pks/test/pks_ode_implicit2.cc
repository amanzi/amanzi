...
create(const Teuchos::RCP<State>& S) {
  ...
  auto ti_c_list = Teuchos::sublist(ti_list, "timestep controller standard parameters");
  ...
  return Teuchos::rcp(new PK_Adaptor<PK_ODE_Implicit<PK_MixinImplicit<PK_MixinLeaf<PK_Default> > > >(pk_tree, global_list, S, soln));
}
...
SUITE(PK_ODE_IMPLICIT_WITH_FAIL) {
  ...
  TEST(Advance2) {
    ...
    CHECK_CLOSE(4.0, (*S->Get<CompositeVector>("primary", "next").ViewComponent("cell",false))[0][0], 1.e-10);
  }
