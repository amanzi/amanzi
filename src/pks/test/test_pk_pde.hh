...
public:
  ...
  void FunctionalTimeDerivative(double t, const TreeVector &u, TreeVector &f) {
    ...
    const auto& dudt = this->S_->template Get<CompositeVector>(dudt_key_, tag_inter_);

    // evaluate cell volume
    this->S_->GetEvaluator("cell_volume", tag_inter_)
        .Update(*this->S_, this->name());
    const auto& cv = this->S_->template Get<CompositeVector>("cell_volume", tag_inter_);
...
public:
  PK_PDE_Implicit(const Teuchos::RCP<Teuchos::ParameterList> &pk_tree,
    ...
    conserved_key_ = this->plist_->template get<std::string>("conserved quantity", "u");
  }
  ...
  void Setup() {
    ...
    auto rhss = Teuchos::Array<std::string>(std::vector<std::string>{"source_cv", "accumulation"});
    ...
    re_list.sublist("preconditioner").sublist("boomer amg parameters").set("cycle iterations", 2);
    re_list.sublist("preconditioner").sublist("boomer amg parameters").set("smoother sweeps", 5);
    re_list.sublist("preconditioner").sublist("boomer amg parameters").set("tolerance", 0.0);
    re_list.sublist("preconditioner").sublist("boomer amg parameters").set("verbosity", 0);
  ...
  void Initialize() {
    Base_t::Initialize();
    auto& ic_cv = this->S_->template GetW<CompositeVector>(this->key_, "", this->key_);
    ic_cv.PutScalar(0.);
    this->S_->GetRecordW(this->key_, "", this->key_).set_initialized();
  }
  ...
  int ApplyPreconditioner(Teuchos::RCP<const TreeVector> u,
    ...
    const auto& lin_op = this->S_->template GetDerivative<Operators::Operator>(res_key_, tag_new_, this->key_, tag_new_);
