...
******************************************************************* */
bool
TransportExplicit_PK::AdvanceStep(double t_old, double t_new, bool reinit)
{
  bool failed = false;
...
******************************************************************* */
void
TransportExplicit_PK::AdvanceDonorUpwind(double dt_cycle)
{
  dt_ = dt_cycle; // overwrite the maximum stable transport step
  mass_solutes_source_.assign(num_aqueous + num_gaseous, 0.0);
...
******************************************************************* */
void
TransportExplicit_PK::AdvanceDonorUpwindManifold(double dt_cycle)
{
  dt_ = dt_cycle; // overwrite the maximum stable transport step
  mass_solutes_source_.assign(num_aqueous + num_gaseous, 0.0);
...
****************************************************************** */
void
TransportExplicit_PK::DudtOld_(double t, const Epetra_Vector& component, Epetra_Vector& f_component)
{
  auto flowrate = S_->Get<CompositeVector>(vol_flowrate_key_).ViewComponent("face", true);
  ...
  // process external sources
  if (srcs_.size() != 0) {
    ComputeSources_(t, 1.0, f_component, *component_rcp, current_component_, current_component_);
  }
