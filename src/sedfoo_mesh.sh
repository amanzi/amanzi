function go_mesh () {
    ...
    # for node, face, cell
    gsed -i 's|\([^ ]*\)node_get_coordinates(\([^,]*\),[ ]*&\(.*\));|\3 = \1getNodeCoordinate(\2);|g' "$1"
    gsed -i 's|\([^ ]*\)cell_get_coordinates(\([^,]*\),[ ]*&\(.*\));|\3 = \1getCellCoordinates(\2);|g' "$1"
    gsed -i 's|\([^ ]*\)face_get_coordinates(\([^,]*\),[ ]*&\(.*\));|\3 = \1getFaceCoordinates(\2);|g' "$1"
    ...
    gsed -i 's@cell_get_faces_and_dirs(\([^&]*\)&\(.*\),[ ]*\(true\|false\)[ ]*)@getCellFacesAndDirs(\1\2)@g' "$1"
    gsed -i 's@cell_get_faces_and_dirs(\([^&]*\)&\(.*\))@getCellFacesAndDirs(\1\2)@g' "$1"

    gsed -i 's@cell_get_faces_and_bisectors(\([^&]*\)&\(.*\),[ ]*\(true\|false\)[ ]*)@getCellFacesAndBisectors(\1\2)@g' "$1"
    gsed -i 's@cell_get_faces_and_bisectors(\([^&]*\)&\(.*\))@getCellFacesAndBisectors(\1\2)@g' "$1"
    
    gsed -i 's@face_get_edges_and_dirs(\([^&]*\)&\(.*\),[ ]*\(true\|false\)[ ]*)@getFaceEdgesAndDirs(\1\2)@g' "$1"
    gsed -i 's@face_get_edges_and_dirs(\([^&]*\)&\(.*\))@getFaceEdgesAndDirs(\1\2)@g' "$1"

    # old style cell_get_faces
    gsed -i 's|cell_get_faces(\([^&]*\)&\(.*\))|getCellFaces(\1\2)|g' "$1"
    # new style cell get faces
    gsed -i 's|=\(.*\)cell_get_faces|=\1getCellFaces|g' "$1"
    
    gsed -i 's|cell_get_edges(\([^&]*\)&\(.*\))|getCellEdges(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)cell_get_edges|=\1getCellEdges|g' "$1"

    gsed -i 's|cell_get_nodes(\([^&]*\)&\(.*\))|getCellNodes(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)cell_get_nodes|=\1getCellNodes|g' "$1"

    gsed -i 's|face_get_edges(\([^&]*\)&\(.*\))|getFaceEdges(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)face_get_edges|=\1getFaceEdges|g' "$1"
    gsed -i 's|face_get_nodes(\([^&]*\)&\(.*\))|getFaceNodes(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)face_get_nodes|=\1getFaceNodes|g' "$1"
    gsed -i 's|edge_get_nodes(\([^&]*\)&\(.*\))|getEdgeNodes(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)edge_get_nodes|=\1getEdgeNodes|g' "$1"

    gsed -i 's|face_get_cells(\([^&]*\)&\(.*\))|getFaceCells(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)face_get_cells|=\1getFaceCells|g' "$1"
    gsed -i 's|edge_get_cells(\([^&]*\)&\(.*\))|getEdgeCells(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)edge_get_cells|=\1getEdgeCells|g' "$1"
    gsed -i 's|node_get_cells(\([^&]*\)&\(.*\))|getNodeCells(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)node_get_cells|=\1getNodeCells|g' "$1"
    gsed -i 's|edge_get_faces(\([^&]*\)&\(.*\))|getEdgeFaces(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)edge_get_faces|=\1getEdgeFaces|g' "$1"
    gsed -i 's|node_get_faces(\([^&]*\)&\(.*\))|getNodeFaces(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)node_get_faces|=\1getNodeFaces|g' "$1"
    gsed -i 's|node_get_edges(\([^&]*\)&\(.*\))|getNodeEdges(\1\2)|g' "$1"
    gsed -i 's|=\(.*\)node_get_edges|=\1getNodeEdges|g' "$1"
    ...
    gsed -i 's^->cell_map(^->getMap(AmanziMesh::Entity_kind::CELL,^g' "$1"
    gsed -i 's^->face_map(^->getMap(AmanziMesh::Entity_kind::FACE,^g' "$1"
    gsed -i 's^->edge_map(^->getMap(AmanziMesh::Entity_kind::EDGE,^g' "$1"
    gsed -i 's^->node_map(^->getMap(AmanziMesh::Entity_kind::NODE,^g' "$1"
    gsed -i 's^\.cell_map(^.getMap(AmanziMesh::Entity_kind::CELL,^g' "$1"
    gsed -i 's^\.face_map(^.getMap(AmanziMesh::Entity_kind::FACE,^g' "$1"
    gsed -i 's^\.edge_map(^.getMap(AmanziMesh::Entity_kind::EDGE,^g' "$1"
    gsed -i 's^\.node_map(^.getMap(AmanziMesh::Entity_kind::NODE,^g' "$1"
