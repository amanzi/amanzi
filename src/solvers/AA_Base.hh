 ...
 ***************************************************************** */
template <class Vector, class VectorSpace>
AA_Base<Vector, VectorSpace>::AA_Base(int mvec, double vtol, double beta,
...
AA_Base<Vector, VectorSpace>::QRdelete()
{
  ...
  for (int i = 0; i < m - 1; i++) {
    ...
    double temp =
      sqrt(R_[loc_id] * R_[loc_id] + R_[loc_id - 1] * R_[loc_id - 1]);
...
template <class Vector, class VectorSpace>
void
AA_Base<Vector, VectorSpace>::Correction(
  const Vector& f, Vector& dir, const Teuchos::Ptr<const Vector>& u_old)
{
