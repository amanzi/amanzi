 ...
 public:
  ...
  NOX::Abstract::Group& operator=(const NOX::Abstract::Group& source) override
  {
    const AmanziGroup<VectorClass>& sourceGroup =
      dynamic_cast<const AmanziGroup<VectorClass>&>(source);

    fn_ = sourceGroup.fn_;
    if (Teuchos::nonnull(sourceGroup.x_))
      x_ = Teuchos::rcp_dynamic_cast<NoxVector<VectorClass>>(
        sourceGroup.x_->clone());
    if (Teuchos::nonnull(sourceGroup.f_))
      f_ = Teuchos::rcp_dynamic_cast<NoxVector<VectorClass>>(
        sourceGroup.f_->clone());
    if (Teuchos::nonnull(sourceGroup.dx_))
      dx_ = Teuchos::rcp_dynamic_cast<NoxVector<VectorClass>>(
        sourceGroup.dx_->clone());
                       ...
                       NOX::Abstract::Vector& result) const override
  {
    ...
    int ierr =
      fn_->ApplyPreconditioner(input_a->get_vector(), result_a->get_vector());
    dx_->scale(-1.);
    if (ierr > 0) {
      return NOX::Abstract::Group::Ok;
    } else {
  ...
  clone(NOX::CopyType type = NOX::DeepCopy) const override
  {
    Teuchos::RCP<AmanziGroup<VectorClass>> cloneGroup =
      Teuchos::rcp(new AmanziGroup<VectorClass>(fn_));
    cloneGroup->fn_ = fn_;
    if (Teuchos::nonnull(x_))
      cloneGroup->x_ =
        Teuchos::rcp_dynamic_cast<NoxVector<VectorClass>>(x_->clone());
    if (Teuchos::nonnull(f_))
      cloneGroup->f_ =
        Teuchos::rcp_dynamic_cast<NoxVector<VectorClass>>(f_->clone());
    if (Teuchos::nonnull(dx_))
      cloneGroup->dx_ =
        Teuchos::rcp_dynamic_cast<NoxVector<VectorClass>>(dx_->clone());
  ...
  virtual Teuchos::RCP<const NOX::Abstract::Vector> getXPtr() const override
  {
    return x_;
  }
  virtual Teuchos::RCP<const NOX::Abstract::Vector> getFPtr() const override
  {
    return f_;
  }
