  ...
  Dump_(false)
{
  ...
  // X in AX = Y
  AMANZI_CHK_ERRV(HYPRE_IJVectorCreate(comm, ilower, iupper, &XHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorSetObjectType(XHypre_, HYPRE_PARCSR));
  AMANZI_CHK_ERRV(HYPRE_IJVectorInitialize(XHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorAssemble(XHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorGetObject(XHypre_, (void**) &ParX_));
  XVec_ = Teuchos::rcp((hypre_ParVector *) hypre_IJVectorObject(((hypre_IJVector *) XHypre_)),false);

  // Y in AX = Y
  AMANZI_CHK_ERRV(HYPRE_IJVectorCreate(comm, ilower, iupper, &YHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorSetObjectType(YHypre_, HYPRE_PARCSR));
  AMANZI_CHK_ERRV(HYPRE_IJVectorInitialize(YHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorAssemble(YHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorGetObject(YHypre_, (void**) &ParY_));
...
void Hypre<MatrixType>::Destroy(){
  if(isInitialized()){
    AMANZI_CHK_ERRV(HYPRE_IJMatrixDestroy(HypreA_));
  }
  AMANZI_CHK_ERRV(HYPRE_IJVectorDestroy(XHypre_));
  AMANZI_CHK_ERRV(HYPRE_IJVectorDestroy(YHypre_));
  if(IsSolverCreated_){
    AMANZI_CHK_ERRV(SolverDestroyPtr_(Solver_));
  }
  if(IsPrecondCreated_){
    AMANZI_CHK_ERRV(PrecondDestroyPtr_(Preconditioner_));
  }

  // Maxwell
  if(HypreG_) {
    AMANZI_CHK_ERRV(HYPRE_IJMatrixDestroy(HypreG_));
  }
  if(xHypre_) {
    AMANZI_CHK_ERRV(HYPRE_IJVectorDestroy(xHypre_));
  }
  ...
  if(zHypre_) {
    AMANZI_CHK_ERRV(HYPRE_IJVectorDestroy(zHypre_));
  }
} //Destroy()
...
void Hypre<MatrixType>::initialize(){
  const std::string timerName ("Amanzi::Hypre::initialize");
...
void Hypre<MatrixType>::setParameters(const Teuchos::ParameterList& list){
  ...
  AMANZI_CHK_ERR(SetParameter(SetPrecond));
  ...
  if(NumFunctions > 0){
    ...
    for(int i = 0; i < NumFunctions; i++){
      AMANZI_CHK_ERR(AddFunToList(params[i]));
    }
  }
  ...
  if (list.isSublist("hypre: Solver functions")) {
    ...
    for (auto it = solverList.begin(); it != solverList.end(); ++it) {
      ...
      if (it->second.isType<HYPRE_Int>()) {
        AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Solver, funct_name , Teuchos::getValue<HYPRE_Int>(it->second)))));
      } else if (!std::is_same<HYPRE_Int,int>::value && it->second.isType<int>()) {
        AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Solver, funct_name , Teuchos::as<global_ordinal_type>(Teuchos::getValue<int>(it->second))))));
      } else if (it->second.isType<double>()) {
        AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Solver, funct_name , Teuchos::getValue<double>(it->second)))));
      } else {
        AMANZI_CHK_ERR(-1);
      }
    }
  ...
  if (list.isSublist("hypre: Preconditioner functions")) {
    ...
    for (auto it = precList.begin(); it != precList.end(); ++it) {
      std::string funct_name = it->first;
      if (it->second.isType<HYPRE_Int>()) {
        AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Preconditioner, funct_name , Teuchos::getValue<HYPRE_Int>(it->second)))));
      } else if (!std::is_same<HYPRE_Int,int>::value && it->second.isType<int>()) {
        AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Preconditioner, funct_name ,  Teuchos::as<global_ordinal_type>(Teuchos::getValue<int>(it->second)))))); 
      } else if (it->second.isType<double>()) {
        AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Preconditioner, funct_name , Teuchos::getValue<double>(it->second)))));
        ...
        if (FunctionParameter::isFuncIntInt(funct_name)) {
          ...
          AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Preconditioner, funct_name , arg0, arg1))));
          ...
          AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Preconditioner, funct_name , arg0, arg1, arg2, arg3))));
          ...
          AMANZI_CHK_ERR(AddFunToList(rcp(new FunctionParameter(Hypre_Is_Preconditioner, funct_name , arg0, arg1, arg2, arg3, arg4, arg5))));
        } else {
          AMANZI_CHK_ERR(-1);
        }
      }
...
int Hypre<MatrixType>::SetParameter(Hypre_Chooser chooser, global_ordinal_type (*pt2Func)(HYPRE_Solver, global_ordinal_type), global_ordinal_type parameter){
  ...
  AMANZI_CHK_ERR(AddFunToList(temp));
  return 0;
...
int Hypre<MatrixType>::SetDiscreteGradient(Teuchos::RCP<const crs_matrix_type> G){
  ...
  AMANZI_CHK_ERR(HYPRE_IJMatrixCreate(comm, ilower, iupper, jlower, jupper, &HypreG_));
  AMANZI_CHK_ERR(HYPRE_IJMatrixSetObjectType(HypreG_, HYPRE_PARCSR));
  AMANZI_CHK_ERR(HYPRE_IJMatrixInitialize(HypreG_));
  ...
  for(LO i = 0; i < (LO)G->getNodeNumRows(); i++){
    ...
    AMANZI_CHK_ERR(HYPRE_IJMatrixSetValues(HypreG_, 1, &numEntries, GlobalRow, new_indices.data(), values.getRawPtr()));
  }
  AMANZI_CHK_ERR(HYPRE_IJMatrixAssemble(HypreG_));
  AMANZI_CHK_ERR(HYPRE_IJMatrixGetObject(HypreG_, (void**)&ParMatrixG_));
...
int Hypre<MatrixType>::SetCoordinates(Teuchos::RCP<multivector_type> coords) {
  ...
  if( NumEntries != iupper-ilower+1) {
    std::cout<<"Amanzi::Hypre::SetCoordinates(): Error on rank "<<A_->getRowMap()->getComm()->getRank()<<": MyLength = "<<coords->getLocalLength()<<" GID range = ["<<ilower<<","<<iupper<<"]"<<std::endl;
    throw std::runtime_error("Hypre<MatrixType>: SetCoordinates: Length mismatch");
  }

  AMANZI_CHK_ERR(HYPRE_IJVectorCreate(comm, ilower, iupper, &xHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorSetObjectType(xHypre_, HYPRE_PARCSR));
  AMANZI_CHK_ERR(HYPRE_IJVectorInitialize(xHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorSetValues(xHypre_,NumEntries,indices,xPtr));
  AMANZI_CHK_ERR(HYPRE_IJVectorAssemble(xHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorGetObject(xHypre_, (void**) &xPar_));

  AMANZI_CHK_ERR(HYPRE_IJVectorCreate(comm, ilower, iupper, &yHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorSetObjectType(yHypre_, HYPRE_PARCSR));
  AMANZI_CHK_ERR(HYPRE_IJVectorInitialize(yHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorSetValues(yHypre_,NumEntries,indices,yPtr));
  AMANZI_CHK_ERR(HYPRE_IJVectorAssemble(yHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorGetObject(yHypre_, (void**) &yPar_));

  AMANZI_CHK_ERR(HYPRE_IJVectorCreate(comm, ilower, iupper, &zHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorSetObjectType(zHypre_, HYPRE_PARCSR));
  AMANZI_CHK_ERR(HYPRE_IJVectorInitialize(zHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorSetValues(zHypre_,NumEntries,indices,zPtr));
  AMANZI_CHK_ERR(HYPRE_IJVectorAssemble(zHypre_));
  AMANZI_CHK_ERR(HYPRE_IJVectorGetObject(zHypre_, (void**) &zPar_));
...
void Hypre<MatrixType>::compute(){
  const std::string timerName ("Amanzi::Hypre::compute");
  Teuchos::RCP<Teuchos::Time> timer = Teuchos::TimeMonitor::lookupCounter (timerName);
  if (timer.is_null ()) timer = Teuchos::TimeMonitor::getNewCounter (timerName);
  double startTime = timer->wallTime();
  // Start timing here.
  {
    ...
    global_ordinal_type ilower = GloballyContiguousRowMap_->getMinGlobalIndex();
    global_ordinal_type iupper = GloballyContiguousRowMap_->getMaxGlobalIndex();
    AMANZI_CHK_ERR(HYPRE_IJMatrixCreate(comm, ilower, iupper, ilower, iupper, &HypreA_));
    ...
    if(SolveOrPrec_ == Hypre_Is_Solver) {
      ...
      if (SolverPrecondPtr_ != NULL && UsePreconditioner_) {
        ...
        AMANZI_CHK_ERR(SetPrecondType(PrecondType_));
        CallFunctions();
        AMANZI_CHK_ERR(SolverPrecondPtr_(Solver_, PrecondSolvePtr_, PrecondSetupPtr_, Preconditioner_));
      } else {
        CallFunctions();
      }
    } else {
    ...
    if(SolveOrPrec_ == Hypre_Is_Solver){
      ...
      AMANZI_CHK_ERR(PrecondSetupPtr_(Preconditioner_, ParMatrix_, ParX_, ParY_));
    }
...
int Hypre<MatrixType>::CallFunctions() const{
  for(int i = 0; i < NumFunsToCall_; i++){
    AMANZI_CHK_ERR(FunsToCall_[i]->CallFunction(Solver_, Preconditioner_));
  }
  return 0;
} //CallFunctions()
...
void Hypre<MatrixType>::apply (const Tpetra::MultiVector<scalar_type,local_ordinal_type,global_ordinal_type,node_type>& X,
  ...
  using LO = local_ordinal_type;
  using SC = scalar_type;
  const std::string timerName ("Amanzi::Hypre::apply");
  Teuchos::RCP<Teuchos::Time> timer = Teuchos::TimeMonitor::lookupCounter (timerName);
  if (timer.is_null ()) timer = Teuchos::TimeMonitor::getNewCounter (timerName);
  double startTime = timer->wallTime();
  // Start timing here.
  {
    ...
    if (NumVectors != Y.getNumVectors()) AMANZI_CHK_ERR(-1);  // X and Y must have same number of vectors
    ...
    for(int VecNum = 0; VecNum < (int) NumVectors; VecNum++) {
      ...
      AMANZI_CHK_ERR(HYPRE_ParVectorSetConstantValues(ParY_, 0.0));
      if(SolveOrPrec_ == Hypre_Is_Solver){
        // Use the solver methods
        AMANZI_CHK_ERR(SolverSolvePtr_(Solver_, ParMatrix_, ParX_, ParY_));
      } else {
        // Apply the preconditioner
        AMANZI_CHK_ERR(PrecondSolvePtr_(Preconditioner_, ParMatrix_, ParX_, ParY_));
      }
...
getDomainMap () const
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
    A.is_null (), std::runtime_error, "Amanzi::Hypre::getDomainMap: The "
...
getRangeMap () const
{
  Teuchos::RCP<const row_matrix_type> A = getMatrix();
  TEUCHOS_TEST_FOR_EXCEPTION(
    A.is_null (), std::runtime_error, "Amanzi::Hypre::getRangeMap: The "
    "input matrix A is null.  Please call setMatrix() with a nonnull input "
    "matrix before calling this method.");
...
#define AMANZI_HYPRE_INSTANT(S,LO,GO,N)                            \
