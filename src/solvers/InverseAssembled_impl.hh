...
template <class Vector>
void
copyFromSuperVector(const Teuchos::RCP<const SuperMap>& smap, const Vector_type& sv, Vector& v)
{
  AMANZI_ASSERT(smap.get());
...
typename std::enable_if<
  is_assembling<Operator>::value,
  std::tuple<Teuchos::RCP<const SuperMap>, Teuchos::RCP<Vector_type>, Teuchos::RCP<Vector_type>>>::
  type
...
template <class Operator>
typename std::enable_if<
  is_assembled<Operator>::value,
  std::tuple<Teuchos::RCP<SuperMap>, Teuchos::RCP<Vector_type>, Teuchos::RCP<Vector_type>>>::type
getSuperMap(Operator& m)
{
