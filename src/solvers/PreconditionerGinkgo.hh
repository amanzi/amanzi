...
class PreconditionerGinkgo : public Preconditioner {
 public:
  ...
  void copyTpetraToGinkgo()
  {
    ...
    Kokkos::parallel_for(
      nrows, KOKKOS_LAMBDA(const int i) {
        if (i != 0) colsperrow(i) = colsperrow(i) + colsperrow(i - 1);
      });
    Kokkos::parallel_for(
      nrows + 1, KOKKOS_LAMBDA(const int i) {
        if (i != nrows)
    ...
    Kokkos::View<LO*, Kokkos::DefaultExecutionSpace> colindices =
      h_->getCrsGraph()->getLocalGraph().entries;
    ...
    matrix_ =
      mtx::create(exec_,
                  ...
                  gko::Array<Matrix_type::local_ordinal_type>::view(
                    exec_, colindices.size(), colindices.data()),
                  gko::Array<Matrix_type::local_ordinal_type>::view(
                    exec_, colsperrow.size(), colsperrow.data()));
...
#  endif
  ...
  virtual int applyInverse(const Vector_type& v, Vector_type& hv) const override
  {
    ...
    auto lo_hv =
      vec::create(exec_, gko::dim<2>(hv.getData().size(), 1)); //,hv.getData().getRawPtr());
    // Copy data
