...
template <class Vector, class VectorSpace>
void
SolverNKA_LS<Vector, VectorSpace>::Init(
  ...
  do {
    ...
    *u_prev = *u;
    ...
    if (fn_->IsAdmissible(u)) {
      ...
      if (vo_->os_OK(Teuchos::VERB_LOW)) {
        *vo_->os() << num_itrs_ << ": error(res) = " << error << std::endl
                   << num_itrs_ << ": L2 error(res) = " << l2_error
                   << std::endl;
      }
      ...
      if (bt_monitor_ == BT_MONITOR_ENORM || bt_monitor_ == BT_MONITOR_EITHER) {
        good_iterate |=
          error < previous_error * (1. + backtrack_rtol_) + backtrack_atol_;
      }
      if (bt_monitor_ == BT_MONITOR_L2 || bt_monitor_ == BT_MONITOR_EITHER) {
        good_iterate |= l2_error < previous_l2_error * (1. + backtrack_rtol_) +
                                     backtrack_atol_;
      }
    }
    ...
    if ((!good_iterate) &&
      ...
      // find an admissible endpoint alpha, starting from ten times the full
      // correction
      alpha = max_alpha_;
      *u = *u_prev;
      ...
      l2_error = r->norm2();
      if (vo_->os_OK(Teuchos::VERB_LOW)) {
        *vo_->os() << num_itrs_ << ": post-LS error(res) = " << error
                   << std::endl
                   << num_itrs_ << ": post-LS L2 error(res) = " << l2_error
                   << std::endl;
      }
    ...
    if (!admissible_iterate) {
      ...
      if (vo_->os_OK(Teuchos::VERB_LOW)) {
        *vo_->os() << "Solution iterate search direction not downhill, FAIL."
                   << std::endl;
      }
