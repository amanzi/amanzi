...
class NonlinearProblem : public Amanzi::AmanziSolvers::SolverFnBase<Vector_type> {
 public:
  ...
  void Residual(const Teuchos::RCP<Vector_type>& u, const Teuchos::RCP<Vector_type>& f)
  {
    ...
    assert(uv != fv);
    Kokkos::parallel_for(
      "solver_fnbase1::Residual", fv.extent(0), KOKKOS_LAMBDA(const int c) {
        double x = uv(c, 0);
        fv(c, 0) = x * (x * x + 1.0);
      });
  }
  ...
  void UpdatePreconditioner(const Teuchos::RCP<const Vector_type>& up)
  {
    ...
    if (exact_jacobian_) {
      ...
      Kokkos::parallel_for(
        "solver_fnbase1::UpdatePreconditioner loop 1", hv.extent(0), KOKKOS_LAMBDA(const int c) {
          double x = upv(c, 0);
          hv(c, 0) = 3 * x * x + 1.0;
        });
    } else {
      ...
      auto hv = h_->getLocalViewDevice(Tpetra::Access::ReadWrite);
      Kokkos::parallel_for(
        "solver_fnbase1::UpdatePreconditioner loop 2", hv.extent(0), KOKKOS_LAMBDA(const int c) {
          double x = upv(c, 0);
          hv(c, 0) = x * x + 2.5;
        });
    }
