...
class NonlinearProblem6 : public Amanzi::AmanziSolvers::SolverFnBase<Vector_type> {
 public:
  ...
  void Residual(const Teuchos::RCP<Vector_type>& u, const Teuchos::RCP<Vector_type>& f)
  {
    auto uv = u->getLocalViewDevice(Tpetra::Access::ReadOnly);
    auto fv = f->getLocalViewDevice(Tpetra::Access::ReadWrite);
    Kokkos::parallel_for(
      "solver_fnbase6::Residual", fv.extent(0), KOKKOS_LAMBDA(const int c) {
        double x = uv(c, 0);
  ...
  int
  ApplyPreconditioner(const Teuchos::RCP<const Vector_type>& u, const Teuchos::RCP<Vector_type>& hu)
  {
  ...
  double
  ErrorNorm(const Teuchos::RCP<const Vector_type>& u, const Teuchos::RCP<const Vector_type>& du)
  {
    double norm_du;
    norm_du = du->normInf();
  ...
  void UpdatePreconditioner(const Teuchos::RCP<const Vector_type>& up)
  {
    ...
    if (exact_jacobian_) {
      ...
      Kokkos::parallel_for(
        "solver_fnbase6::UpdatePreconditioner loop 1", hv.extent(0), KOKKOS_LAMBDA(const int c) {
          double x = upv(c, 0);
      ...
      auto hv = h_->getLocalViewDevice(Tpetra::Access::ReadWrite);
      Kokkos::parallel_for(
        "solver_fnbase6::UpdatePreconditioner loop 2", hv.extent(0), KOKKOS_LAMBDA(const int c) {
          double x = upv(c, 0);
