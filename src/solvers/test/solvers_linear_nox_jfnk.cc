...
SUITE(SOLVERS)
{
  class LinearFn : public AmanziSolvers::SolverFnBase<Epetra_Vector> {
   public:
    ...
    Teuchos::RCP<LinearFn> Clone() const
    {
      return Teuchos::rcp(new LinearFn(*this));
    }
    ...
    virtual void
    UpdatePreconditioner(const Teuchos::RCP<const Epetra_Vector>& u)
    {}
  ...
  TEST(NOX_JFNK_SOLVER_LINEAR)
  {
    std::cout << "Checking Nox JFNK solver on linear problem..." << std::endl;
    ...
    jfnk_list.sublist("JF matrix parameters")
      .set("finite difference epsilon", 1e-7);
    ...
    Teuchos::RCP<AmanziSolvers::SolverNox<Epetra_Vector, Epetra_Map>> jfnk =
      Teuchos::rcp(
        new AmanziSolvers::SolverNox<Epetra_Vector, Epetra_Map>(jfnk_list));
    jfnk->Init(fn, *map);
