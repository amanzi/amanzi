...
{ 
  ...
  if (comm->getSize() > 1) { 
    int ndigits = (int)floor(log10(comm->getSize())) + 1;
    std::string fmt = boost::str(boost::format("%%s.%%d.%%0%dd") % ndigits);
    file_name = boost::str(boost::format(fmt) % file_name % comm->getSize() % comm->getRank());
  } 
  ...
  if (comm->getRank() == 0 || distributed_data) {  
    ...
    for (int k = 0; k < nvectors; ++k) {
      ...
      for (int i = 0; i < num_elem_blk; i++) { 
        ...
        free(var_values); 
        printf("getRank=%d  ierr=%d  id=%d  ncells=%d\n", comm->getRank(), ierr, ids[i], num_elem_this_blk); 
