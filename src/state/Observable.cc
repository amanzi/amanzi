...
Observable::Update(const Teuchos::Ptr<State>& S, std::vector<double>& data, int start_loc)
{
            ...
            if (direction_.get()) {
              ...
              if (c < 0) {
                ...
                msg << "Observeable on face region \"" << region_
                    ...
                    << "\" but face " << vec.getMesh()->getMap(AmanziMesh::Entity_kind::FACE,true)->getGlobalElement(id)
  ...
  if (reduction_ == "point" || reduction_ == "integral" || reduction_ == "average" ||
    ...
    Teuchos::reduceAll<int>(*comm_,Teuchos::REDUCE_SUM, value.size(),value.data(), global_value.data());
    ...
    std::vector<double> global_value(value);
    Teuchos::reduceAll<int>(*comm_,Teuchos::REDUCE_MIN, value.size() - 1,value.data(), global_value.data());
    value = global_value;
  } else if (reduction_ == "maximum") {
    std::vector<double> global_value(value);
    Teuchos::reduceAll<int>(*comm_,Teuchos::REDUCE_MAX, value.size() - 1,value.data(), global_value.data());
    value = global_value;
  } else {
    AMANZI_ASSERT(false);
  }
