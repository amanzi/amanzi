...
/* *******************************************************************/
State_Old::State_Old(int number_of_components_,
...
State_Old::State_Old(Teuchos::RCP<Amanzi::AmanziMesh::Mesh> mesh_maps_)
    : mesh_maps(mesh_maps_),
      number_of_ion_exchange_sites_(0),
      number_of_sorption_sites_(0),
      using_sorption_(false),
...
State_Old::State_Old(Teuchos::ParameterList &parameter_list_,
             Teuchos::RCP<Amanzi::AmanziMesh::Mesh> mesh_maps_)
    : mesh_maps(mesh_maps_),
...
/* *******************************************************************/
void State_Old::init_verbosity (Teuchos::ParameterList &parameter_list_) {
...
/* *******************************************************************/
void State_Old::create_default_compnames(int n)
{
  compnames.resize(n);
  for (int i=0; i<n; i++)
  {
...
void State_Old::SetupSoluteNames(void) {
...
void State_Old::initialize_from_parameter_list()
{
  using Teuchos::OSTab;
  Teuchos::EVerbosityLevel verbLevel = this->getVerbLevel();
  Teuchos::RCP<Teuchos::FancyOStream> out = this->getOStream();
...
void State_Old::CreateStoragePrimarySpecies(void) {
...
void State_Old::CreateStorageSecondaryActivityCoeff(const int size) {
...
void State_Old::CreateStorageMinerals() {
...
void State_Old::CreateStorageTotalSorbed(void) {
  if (using_sorption()) {
...
void State_Old::CreateStorageIonExchange(void) {
...
void State_Old::CreateStorageSurfaceComplexation(void) {
...
void State_Old::CreateStorageSorptionIsotherms(void) {
...
void State_Old::set_time ( double new_time ) {
  last_time = new_time;
  time = new_time;
}


void State_Old::set_cycle(int new_cycle) {
  cycle = new_cycle;
}


void State_Old::set_number_of_components(const int n)
{
  number_of_components = n;
}


void State_Old::update_total_component_concentration(Teuchos::RCP<Epetra_MultiVector> new_tcc)
{
  *total_component_concentration = *new_tcc;
}


void State_Old::update_total_component_concentration(const Epetra_MultiVector& new_tcc)
{
  *total_component_concentration = new_tcc;
}
...
/* *******************************************************************/
void State_Old::update_darcy_flux(const Epetra_Vector &new_darcy_flux)
{
  *darcy_flux = new_darcy_flux;
}


void State_Old::update_pressure(const Epetra_Vector &new_pressure)
{
...
/* *******************************************************************/
void State_Old::advance_time(double dT)
{
  last_time = time;
  time = time + dT;
}
...
/* *******************************************************************/
void State_Old::set_cell_value_in_region(const double& value, Epetra_Vector& v,
...
/* *******************************************************************/
void State_Old::set_cell_value_in_region(const Epetra_Vector& x, Epetra_Vector& v,
                                     const std::string& region) {
...
/* *******************************************************************/
void State_Old::set_cell_value_in_region(const Amanzi::Function& fun, Epetra_Vector& v,
                                     const std::string& region)
{
  if (!mesh_maps->valid_set_name(region, Amanzi::AmanziMesh::CELL)) {
    std::stringstream tempstr;
...
/* *******************************************************************/
void State_Old::set_cell_value_in_mesh_block(double value, Epetra_Vector &v,
                                         int mesh_block_id)
{
...
/* *******************************************************************/
void State_Old::set_darcy_flux(const Amanzi::AmanziGeometry::Point& u, const int mesh_block_id)
{
...
/* *******************************************************************/
void State_Old::set_darcy_flux(const Amanzi::AmanziGeometry::Point& u, const std::string region)
{
  if (!mesh_maps->valid_set_name(region,Amanzi::AmanziMesh::CELL)) {
    std::stringstream tempstr;
    tempstr << "\n" << "Region " << region << " required by State does not define a valid cell set" << std::endl;
...
/* *******************************************************************/
void State_Old::set_water_density(const double wd)
{
...
void State_Old::set_water_saturation(const double ws)
{
...
/* *******************************************************************/
void State_Old::set_porosity(const double phi)
{
  porosity->PutScalar(phi);
}


void State_Old::set_porosity(const double phi, const int mesh_block_id)
{
  set_cell_value_in_mesh_block(phi, *porosity, mesh_block_id);
}


void State_Old::set_porosity( const double phi, const std::string region )
{
 ...
 *********************************************************************/
void State_Old::set_specific_storage(const double ss, const std::string region)
{
  set_cell_value_in_region(ss, *specific_storage, region);
}


void State_Old::set_specific_yield(const double sy, const std::string region)
{
...
/* *******************************************************************/
void State_Old::set_zero_total_component_concentration()
{
  if (number_of_components > 0) 
...
void State_Old::set_total_component_concentration(const double* conc, const int mesh_block_id)
{
  for (int nc=0; nc<number_of_components; nc++) {
...
void State_Old::set_total_component_concentration(const double* conc, const std::string region)
{
  for (int nc=0; nc<number_of_components; nc++) {
...
void State_Old::set_free_ion_concentrations( const double* conc, const std::string region )
{
  for (int nc=0; nc<number_of_components; nc++) {
...
/* *******************************************************************/
void State_Old::set_permeability(const double kappa)
{
...
void State_Old::set_permeability(const double kappa, const int mesh_block_id)
{
...
void State_Old::set_permeability(const double kappa, const std::string region)
{
...
void State_Old::set_vertical_permeability(const double kappa)
{
  vertical_permeability->PutScalar(kappa);
}


void State_Old::set_vertical_permeability(const double kappa, const int mesh_block_id)
{
  set_cell_value_in_mesh_block(kappa, *vertical_permeability, mesh_block_id);
}


void State_Old::set_vertical_permeability(const double kappa, const std::string region)
{
  set_cell_value_in_region(kappa, *vertical_permeability, region);
}


void State_Old::set_horizontal_permeability(const double kappa)
{
  horizontal_permeability->PutScalar(kappa);
}


void State_Old::set_horizontal_permeability(const double kappa, const int mesh_block_id)
{
  set_cell_value_in_mesh_block(kappa,*horizontal_permeability,mesh_block_id);
}


void State_Old::set_horizontal_permeability( const double kappa, const std::string region)
{
  set_cell_value_in_region(kappa,*horizontal_permeability,region);
}
...
/* *******************************************************************/
void State_Old::set_viscosity(const double mu)
{
  *viscosity = mu;
}
...
/* *******************************************************************/
void State_Old::set_gravity(const double *g)
{
  (*gravity)[0] = g[0];
  (*gravity)[1] = g[1];
  (*gravity)[2] = g[2];
...
/* *******************************************************************/
double State_Old::point_value(const std::string& point_region, const std::string& name)
{
  ...
  if (comp_no.size() > 0) { 
    ...
    std::stringstream ss;
    ss << "State_Old::point_value: cannot make an observation for variable " << name;
...
/* *******************************************************************/
void State_Old::set_darcy_flux(const Epetra_Vector& darcy_flux_)
{
  *darcy_flux = darcy_flux_;
}


void State_Old::set_water_saturation(const Epetra_Vector& water_saturation_)
{
  *water_saturation = water_saturation_;
};


void State_Old::set_prev_water_saturation(const Epetra_Vector& prev_water_saturation_)
{
  *prev_water_saturation = prev_water_saturation_;
};


void State_Old::set_porosity(const Epetra_Vector& porosity_)
{
  *porosity = porosity_;
};

void State_Old::set_particle_density(const Epetra_Vector& particle_density_)
{
...
/* *******************************************************************/
void State_Old::set_permeability(const Epetra_Vector& permeability_)
{
...
void State_Old::set_vertical_permeability(const Epetra_Vector& permeability_)
{
  *vertical_permeability = permeability_;
};


void State_Old::set_horizontal_permeability( const Epetra_Vector& permeability_)
{
  *horizontal_permeability = permeability_;
};
...
/* *******************************************************************/
void State_Old::set_pressure(const Epetra_Vector& pressure_)
{
  *pressure = pressure_;
};
...
/* *******************************************************************/
void State_Old::set_lambda(const Epetra_Vector& lambda_)
{
  *lambda = lambda_;
};
...
/* *******************************************************************/
void State_Old::set_water_density(const Epetra_Vector& water_density_)
{
  *water_density = water_density_;
};


void State_Old::set_darcy_velocity(const Epetra_MultiVector& darcy_velocity_)
{
...
/* *******************************************************************/
void State_Old::set_total_component_concentration(const Epetra_MultiVector& total_component_concentration_)
{
...
/* *******************************************************************/
void State_Old::set_material_ids(const Epetra_Vector& material_ids_)
{
...
/* *******************************************************************/
void State_Old::set_uniform_pressure(const Teuchos::ParameterList& unif_p_list, const std::string& region)
{
...
void State_Old::set_linear_pressure(const Teuchos::ParameterList& lin_p_list, const std::string& region)
{
...
void State_Old::set_file_pressure ( const Teuchos::ParameterList& file_p_list, const std::string& region )
{
  // get parameters from parameter list
...
/* *******************************************************************/
void State_Old::set_uniform_saturation(const Teuchos::ParameterList& unif_s_list, const std::string& region)
{
  // get value from paramter list
...
void State_Old::set_linear_saturation(const Teuchos::ParameterList& lin_s_list, const std::string& region)
{
  // get parameters from parameter list
...
int State_Old::get_component_number(const std::string component_name) {
...
std::string State_Old::get_component_name(const int component_number) {
...
/* *******************************************************************/
void State_Old::write_vis_(Amanzi::Vis& vis, bool chemistry_enabled, bool force) {
  using Teuchos::OSTab;
  Teuchos::EVerbosityLevel verbLevel = this->getVerbLevel();
  Teuchos::RCP<Teuchos::FancyOStream> out = this->getOStream();
  OSTab tab = this->getOSTab(); // This sets the line prefix and adds one tab
...
/* *******************************************************************/
void State_Old::write_vis_(Amanzi::Vis& vis, 
...
/* *******************************************************************/
void State_Old::write_vis(Amanzi::Vis& vis, bool chemistry_enabled, bool force) {
  if ( !vis.is_disabled() ) {
...
/* *******************************************************************/
void State_Old::write_vis(Amanzi::Vis& vis, 
		      Teuchos::RCP<Epetra_MultiVector> auxdata, 
		      const std::vector<std::string>& auxnames, 
		      bool chemistry_enabled, bool force)  {  
  if ( !vis.is_disabled() ) {
...
/* *******************************************************************/
void State_Old::set_compnames(std::vector<std::string>& compnames_)
{
  compnames = compnames_;
}

void State_Old::set_compnames(Teuchos::Array<std::string>& compnames_)
{
  compnames.clear();
...
void State_Old::ExtractVolumeFromMesh(void) {
  int ncell = mesh_maps->cell_map(false).NumMyElements();
  if (ncell != volume()->MyLength()) {
    Exceptions::amanzi_throw(Errors::Message("State_Old::ExtractVolumeFromMesh() size error."));
  }
...
void State_Old::VerifyMaterialChemistry(void) {
  /*
...
void State_Old::SetupMineralNames() {
...
void State_Old::SetupSorptionSiteNames() {
...
void State_Old::VerifyMineralogy(const std::string& region_name,
  ...
  for (Teuchos::ParameterList::ConstIterator mineral_iter = minerals_list.begin(); 
    ...
    if (!mineral_name_id_map_.count(mineral_name)) {
      ...
      message << "Error: State_Old::VerifyMineralogy(): " << mineral_name
...
void State_Old::VerifySorptionIsotherms(const std::string& region_name,
  ...
  for (Teuchos::ParameterList::ConstIterator species_iter = isotherms_list.begin(); 
    ...
    if (!comp_no.count(species_name)) {
      std::stringstream message;
      message << "Error: State_Old::VerifySorptionIsotherms(): region: "
...
void State_Old::VerifySorptionSites(const std::string& region_name,
  ...
  for (Teuchos::ParameterList::ConstIterator site_iter = sorption_site_list.begin(); 
    ...
    if (!sorption_site_name_id_map_.count(site_name)) {
      std::stringstream message;
      message << "Error: State_Old::VerifySorptionSites(): " << site_name
...
void State_Old::SetRegionMaterialChemistry(const std::string& region_name,
...
void State_Old::SetRegionMineralogy(const std::string& region_name,
...
void State_Old::SetRegionSorptionIsotherms(const std::string& region_name,
...
void State_Old::SetRegionSorptionSites(const std::string& region_name,
 ...
 **
 ******************************************************************************/
void State_Old::WriteChemistryToVis(Amanzi::Vis* vis) {
...
void State_Old::WriteFreeIonsToVis(Amanzi::Vis* vis) {
  std::string name;
...
void State_Old::WriteTotalSorbedToVis(Amanzi::Vis* vis) {
  if (using_sorption()) {
    std::string name;
    for (int i = 0; i < get_number_of_components(); ++i) {
      name = compnames.at(i);
...
void State_Old::WriteMineralsToVis(Amanzi::Vis* vis) {
  std::string name;
...
void State_Old::WriteIsothermsToVis(Amanzi::Vis* vis) {
  if (use_sorption_isotherms()) {
    std::string name;
    for (int i = 0; i < get_number_of_components(); ++i) {
      name = compnames.at(i);
...
void State_Old::WriteSorptionSitesToVis(Amanzi::Vis* vis) {
  std::string name;
...
void State_Old::WriteIonExchangeSitesToVis(Amanzi::Vis* vis) {
  if (number_of_ion_exchange_sites() > 0) {
...
void State_Old::DeriveDarcyVelocity() {
