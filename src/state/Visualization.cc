...
Visualization::Visualization(Teuchos::ParameterList& plist,
                             ...
                             bool include_io_set)
  : IOEvent(plist), time_unit_written_(false), count_(0), mesh_(mesh)
{
  readParameters_();
...
// -----------------------------------------------------------------------------
Visualization::Visualization() : IOEvent(), my_units_("y"), time_unit_written_(false), count_(0) {}
...
Visualization::write(const State& S)
{
  if (!is_disabled()) {
    ...
    for (auto r = S.data_begin(); r != S.data_end(); ++r) {
      if (writesDomain(Keys::getDomain(r->first)) || r->first == "time" || r->first == "cycle") {
        ...
        if (r->second->HasRecord(tag)) {
          ...
          for (const auto& time_record : S.GetRecordSet("time")) {
            if (r->second->HasRecord(time_record.first) && S.get_time(time_record.first) == time) {
