        ...
        const MultiVector_type& vec) const
{
  ...
  if (!lifted_vectors_.count(vis_name)) {
    ...
    auto lifted_vec =
      Teuchos::rcp(new MultiVector_type(mesh_->getMap(AmanziMesh::Entity_kind::CELL, false), vec.getNumVectors()));
        ...
        const Vector_type& vec) const
{
  ...
  if (!lifted_vectors_.count(vis_name)) {
    // create a lifted vector if we don't currently have one
    auto lifted_vec = Teuchos::rcp(new MultiVector_type(mesh_->getMap(AmanziMesh::Entity_kind::CELL, false), 1));
