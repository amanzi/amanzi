...
// ---------------------------------------------------------------------------
template <>
void EvaluatorSecondaryMonotype<double>::UpdateDerivative_(State &S, const Key &wrt_key,
                                                   const Key &wrt_tag) {
...
void EvaluatorSecondaryMonotype<CompositeVector,
  ...
  for (auto keytag : my_keys_) {
    const auto& fac = S.Require<CompositeVector,CompositeVectorSpace>(keytag.first,
            keytag.second, keytag.first);
  }
