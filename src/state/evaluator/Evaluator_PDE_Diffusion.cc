...
Evaluator_PDE_Diffusion::EnsureCompatibility(State& S)
{
  ...
  if (mesh.get()) {
    // we have a mesh for the RHS, so we can create a diffusion op and get a
    // schema
    Operators::PDE_DiffusionFactory diff_fac;
    // -- Newton correction is controlled by the Operator Apply -- does it want
    ...
    // -- instantiate the PDE_Diffusion object
    if (gravity_) {
    ...
    // Now require a local Op on the schema needed by the PDE
    ...
    // Use the schema to set up the RHS
    CompositeVectorSpace cvs = Operators::cvsFromSchema(schema, mesh, true);
    rhs_fac.Update(cvs);

    // Require scalar coefficient on the space required by the operator (and
    // the options provided by the user to the operator).
    if (!scalar_coef_key_.empty()) {
      S.Require<CompositeVector, CompositeVectorSpace>(scalar_coef_key_, my_tag_)
          .Update(pde_->scalar_coefficient_space());

      // check for derivatives -- derivatives on the Op (the Jacobian term)
      ...
      if (S.HasDerivativeSet(local_op_key_, my_tag_)) {
        for (const auto& deriv : S.GetDerivativeSet(local_op_key_, my_tag_)) {
          ...
          if (S.RequireEvaluator(scalar_coef_key_, my_tag_)
            ...
            // If the derivative is with respect to u, then the Jacobian is
            // generated by the same PDE as the main diffusion operator.  If
            // not, e.g. d k_rel_perm / dT, then this is a contribution to an
            // off-diagional operator, and we need a different PDE.
            //
            // Note -- this branch of the code is not yet tested --etc
            if (!wrt_is_u) {
              ...
              // require a local Op for the Jacobian of the schema needed by the PDE
              ...
              jac_op_fac.set_schema(Operators::Schema(pde_derivs_[deriv.first]->schema_jacobian()));
            } else {
      ...
      // now we can call EnsureCompatibility on the coefficient as all derivs
      // are set.
      S.RequireEvaluator(scalar_coef_key_, my_tag_).EnsureCompatibility(S);
    }

    // require bcs for the PDE
    ...
    // require tensor coefficient on cells
    if (!tensor_coef_key_.empty()) {
      auto& K_fac =
    ...
    // require density and gravity for gravitational terms
    if (gravity_) {
      S.Require<AmanziGeometry::Point>("gravity", "");

      auto& rho_fac = S.Require<CompositeVector,CompositeVectorSpace>(rho_key_, my_tag_);
...
Evaluator_PDE_Diffusion::Update(State& S, const Key& request)
{
  ...
  bool update = false;
  if (requests_.empty()) {
    // first time Update has been called -- give the PDE its data
    update = true;
    ...
    // Note that the below checks will also always be true in the first time
    // through, so the rest of the data is provided to the PDE below.
  }
  ...
  if (!tensor_coef_key_.empty()) {
    if (S.GetEvaluator(tensor_coef_key_, my_tag_).Update(S, my_request)) {
      update = true;
      // Note that by calling SetCoefficients, we tell the PDE that we have
      // changed this data and it will throw out/recalculate things like
      // transmissibility (for FV) and mass matrices (for MFD).
  ...
  if (!scalar_coef_key_.empty()) {
    // note that if we intend to use the Jacobian term, it is best to set the
    // derivative here too.  While we don't use it for the main operator, if
    // we are going to call the derivative later, it would flag the PDE that
    // we had changed k in addition to dkdp, so would recalculate the main
    // operator.  So setting it here saves us work later (if we use the
    // Jacobian).
    if (S.GetEvaluator(scalar_coef_key_, my_tag_).Update(S, my_request)) {
      update = true;
      auto k = S.GetPtr<CompositeVector>(scalar_coef_key_, my_tag_);
      ...
      pde_->SetScalarCoefficient(k, dkdp);
    }
  }
...
Evaluator_PDE_Diffusion::Update_(State& S)
{
  ...
  // these need to become flags/options in the PList for use on off-diagonals
