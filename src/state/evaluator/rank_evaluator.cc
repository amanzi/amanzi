...
Teuchos::RCP<Evaluator> RankEvaluator::Clone() const {
...
// ---------------------------------------------------------------------------
bool RankEvaluator::Update(const Teuchos::Ptr<State> &S, Key request) {
  if (!computed_once_) {
...
// ---------------------------------------------------------------------------
inline bool RankEvaluator::UpdateDerivative(const Teuchos::Ptr<State> &S,
                                            Key request, Key wrt_key) {
  return false; // no derivatives, though this should never be called
}

inline bool RankEvaluator::IsDependency(const Teuchos::Ptr<State> &S,
                                        Key key) const {
  return false;
}

inline bool RankEvaluator::ProvidesKey(Key key) const { return key == my_key_; }
...
std::string RankEvaluator::WriteToString() const {
