...
class EvaluatorModelLauncher {
 public:
  EvaluatorModelLauncher(const std::string& name,
                         const KeyTag& wrt,
                         const KeyTagSet& dependencies,
  ...
  void launch(const AmanziMesh::MeshCache::cEntity_ID_View& material_ids)
  {
    if (wrt_ == dependencies_[I]) {
      ...
      const Model_type& model = model_;
      Kokkos::parallel_for(
        name_, range, KOKKOS_LAMBDA(const int i) { model(Deriv<I>(), material_ids(i)); });
    } else {
      EvaluatorModelLauncher<I - 1, Model_type, Device_type> launcher(
        name_, wrt_, dependencies_, model_);
 ...
 public:
  ...
  void launch(const int extent)
  {
    ...
    msg << "EvaluatorModel (" << name_ << "): requested derivative with respect to ( \""
        << Keys::getKey(wrt_) << "\" ) is not provided.";
    throw(msg);
  }
