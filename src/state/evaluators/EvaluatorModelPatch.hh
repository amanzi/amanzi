...
{
  return Teuchos::rcp(
    new EvaluatorModelPatch<Model, Device_type>(*this));
}
...
EvaluatorModelPatch<Model, Device_type>::Update_(State& S)
{
  ...
  MultiPatch<double>& result = S.GetW<MultiPatch<double>>(my_key.first, my_key.second, my_key.first);
...
EvaluatorModelPatch<Model,Device_type>::EnsureCompatibility(State& S)
{
  ...
  auto& my_fac = S.Require<MultiPatch<double>, MultiPatchSpace>(my_keytag.first, my_keytag.second, my_keytag.first);
