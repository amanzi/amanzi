...
class make_state {
 public:
  ...
  requireSecondary(const std::string& name, const std::vector<std::string>& derivs)
  {
    ...
    auto f_eval = Teuchos::rcp(new EvaluatorModelCV<Model>(es_list));
