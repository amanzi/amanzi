...
class AModel {
 public:
  AModel(double alpha) : alpha_(alpha) {}
 ...
 public:
                         ...
                         const std::vector<CompositeVector*>& results) override
  {
    ...
    for (auto& model : models_) {
      ...
      mesh.get_set_entities(model.first,
    ...
    const std::vector<CompositeVector*>& results) override
  {
    ...
    for (auto& model : models_) {
      ...
      ExecuteModel("A",
                   model.second,
                   AModel<AmanziDefaultDevice>::dAdB(),
                   region_entities);
    }
  }
...
class make_state {
 public:
  make_state()
  {
    ...
    Teuchos::RCP<AmanziGeometry::Region> r1 = Teuchos::rcp(
      new AmanziGeometry::RegionBox("region1",
                                    0,
                                    AmanziGeometry::Point(0.0, 0.0, 0.0),
                                    AmanziGeometry::Point(2.0, 4.0, 4.0)));
    Teuchos::RCP<AmanziGeometry::Region> r2 = Teuchos::rcp(
      new AmanziGeometry::RegionBox("region2",
                                    0,
                                    AmanziGeometry::Point(2.0, 0.0, 0.0),
                                    AmanziGeometry::Point(4.0, 4.0, 4.0)));
    gm->AddRegion(r1);
    gm->AddRegion(r2);
...
SUITE(DAG)
{
  TEST_FIXTURE(make_state, DAG_TWO_FIELDS)
  {
    ...
    CHECK_CLOSE(2.0,
                (S.GetDerivative<CompositeVector>("fa", "", "fb", "")
                   .ViewComponent<AmanziDefaultHost>("cell", false))(1, 0),
                1e-12);
    CHECK_CLOSE(4.0,
                (S.GetDerivative<CompositeVector>("fa", "", "fb", "")
                   .ViewComponent<AmanziDefaultHost>("cell", false))(6, 0),
                1e-12);
    CHECK_CLOSE(4.0,
                (S.GetDerivative<CompositeVector>("fa", "", "fb", "")
                   .ViewComponent<AmanziDefaultHost>("cell", false))(7, 0),
                1e-12);
  }
}
