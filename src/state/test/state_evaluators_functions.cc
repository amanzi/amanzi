...
struct tester {
  tester() {
    ...
    auto gm = Teuchos::rcp(new AmanziGeometry::GeometricModel(2, plist.sublist("regions"), *comm));
  ...
  void require(const std::string& field) {
    S->Require<CompositeVector, CompositeVectorSpace>(field)
        .SetMesh(S->GetMesh())->AddComponent("cell",AmanziMesh::CELL,1);
  ...
  void run_test(const std::string& fname, const std::vector<std::array<double,2> >& results) {
    std::cout << std::endl << std::endl << "Running test: " << fname << std::endl
              << "-----------------------------------" << std::endl;
    require(fname);
    setup();
    ...
    for (int i=0; i!=5; ++i) {
      ...
      auto fv = S->Get<CompositeVector>(fname)
...
TEST_FIXTURE(tester, INDEPENDENT_FROM_FUNCTION) {
  std::vector<std::array<double,2>> result = {{1,3},
                                             {1,3},
                                             {1,3},
                                             {1,3},
                                             {1,3}};
...
TEST_FIXTURE(tester, INDEPENDENT_FROM_FILE) {
  std::vector<std::array<double,2>> result = {{2,4},
                                             {2,4},
                                             {3,5},
                                             {4,6},
                                             {4,6}};
...
TEST_FIXTURE(tester, SECONDARY_ADDITIVE) {
  std::vector<std::array<double,2>> result = {{4,10},
                                             {4,10},
                                             {5,11},
                                             {6,12},
                                             {6,12}};
...
TEST_FIXTURE(tester, SECONDARY_MULTIPLICATIVE) {
  std::vector<std::array<double,2>> result = {{2,12},
                                             {2,12},
                                             {3,15},
                                             {4,18},
                                             {4,18}};
...
TEST_FIXTURE(tester, SECONDARY_FUNCTION) {
  std::vector<std::array<double,2>> result = {{8,18},
                                             {8,18},
                                             {11,21},
                                             {14,24},
                                             {14,24}};
