...
SUITE(EVALS) {
  TEST(EVALS_INDPENDENT_FROMFUNCTION) {
    ...
    S->RequireField("fa", "fa")
        ->SetMesh(mesh)
        ->SetGhosted(true)
        ->SetComponent("cell", AmanziMesh::CELL, 1);
    auto fa_eval =
        Teuchos::rcp(new IndependentVariableFieldEvaluatorFromFunction(A_list));
  ...
  TEST(EVALS_SECONDARY_FROMFUNCTION) {
    ...
    S->RequireField("fb", "fb")
        ->SetMesh(mesh)
        ->SetGhosted(true)
        ->SetComponent("cell", AmanziMesh::CELL, 1);
    auto fb_eval =
        Teuchos::rcp(new SecondaryVariableFieldEvaluatorFromFunction(B_list));
