...
SUITE(EVALS) {
  TEST(EVALS_INDPENDENT_FROMFUNCTION) {
    ...
    auto mesh = meshfactory.create(0.0, 0.0, 0.0, 4.0, 4.0, 4.0, 2, 2, 2, gm);
    ...
    A_list.print(std::cout);
    
    
    S->RequireField("fa", "fa")->SetMesh(mesh)->SetGhosted(true)->SetComponent("cell", AmanziMesh::CELL, 1);
    auto fa_eval = Teuchos::rcp(new IndependentVariableFieldEvaluatorFromFunction(A_list));
    ...
    // {  1.1, 1, 1, 1 }
    // making the value
    // (  { 1.1, 1, 1, 1 } - {1.0, 2.0, 3.0, 4.0} ) *  {0.5, 1.0, 2.0, 2.0}  + 1 = (.05 -1 -4  -6 + 1) = -9.95
  ...
  TEST(EVALS_SECONDARY_FROMFUNCTION) {
    ...
    // Secondary variable evaluator which is a function of A
    ...
    B_list.set<Teuchos::Array<std::string> >("evaluator dependencies", deps);
    ...
    B_list.print(std::cout);

    S->RequireField("fb", "fb")->SetMesh(mesh)->SetGhosted(true)->SetComponent("cell", AmanziMesh::CELL, 1);
    auto fb_eval = Teuchos::rcp(new SecondaryVariableFieldEvaluatorFromFunction(B_list));
    ...
    const Epetra_MultiVector& fb = *S->GetFieldData("fb")->ViewComponent("cell");

    // eval point of 0th cell is:
    // {  1.1, 1, 1, 1 }
    // making the value for A
    // (  { 1.1, 1, 1, 1 } - {1.0, 2.0, 3.0, 4.0} ) *  {0.5, 1.0, 2.0, 2.0}  + 1 = (.05 -1 -4  -6 + 1) = -9.95
    // and then the value for B = (-9.95 - -1) * 2.0 + 1.0 = -16.9
