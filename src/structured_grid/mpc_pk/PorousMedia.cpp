...
int  PorousMedia::num_state_type = 4;

// Options for parallelization of chemistry solver 
static std::string solid    = "Solid";
static std::string absorbed = "Absorbed";
static std::string ctotal="Total";
	  ...
	  while (err > 1.e-8 && k < 80000)
	    {
	      ...
	      tmp.mult(-1.0);
	      richard_scalar_update(dt,curr_nwt_iter);
				     ...
				     Real dt)
{
  ...
  MultiFab::Copy(S_new,*pcnp1_cc,0,ncomps+ntracers+1,1,1);
}
			      ...
			      Real dt)
{
  ...
  if (curr_nwt_iter <= richard_iter || curr_nwt_iter < 10)
    dt_nwt = dt_nwt*1.2;
  else if (curr_nwt_iter > richard_iter )
    dt_nwt = dt_nwt*0.9;
  ...
  for (FillPatchIterator S_fpi(*this,S,1,time,State_Type,0,ncomps);
       ...
       ++S_fpi)
    {
      ...
      FORT_UPHASE_P (updat,ARLIM(uplo),ARLIM(uphi),
		     ...
		     lbddat,ARLIM(lbdlo),ARLIM(lbdhi),
		     pdat,ARLIM(plo),ARLIM(phi),
		     lo,hi,domain_lo,domain_hi,dx,s_bc.dataPtr(),
		     rinflow_bc_lo.dataPtr(),rinflow_bc_hi.dataPtr(),
		     rinflow_vel_lo.dataPtr(),rinflow_vel_hi.dataPtr());
#endif
    }
...

{
  ...
  calcCapillary(pcTime);
  calcLambda(pcTime);
  calc_richard_coef(cmp_pcp1,lambdap1_cc,0);
  calc_richard_jac (cmp_pcp1_dp,lambdap1_cc,pcTime,0);
  while ((itr_nwt < max_itr_nwt) && (err_nwt > max_err_nwt)) 
    {
      diffusion->richard_iter_eqb(nc,gravity,density,res_fix,
				  cmp_pcp1,cmp_pcp1_dp,pcnp1_cc,&err_nwt);      
      if (verbose > 1 && ParallelDescriptor::IOProcessor())
	std::cout << "Newton iteration " << itr_nwt 
	          << " : Error = "       << err_nwt << "\n"; 
      scalar_adjust_constraint(0,ncomps-1);
...
PorousMedia::post_timestep (int crse_iteration)
{
  ...
  if (level == 0)
    {      
	  ...
	  std::cout << "OBS " << "i " << observation_array[i].vals[0]  << std::endl;
        }
    }
  ...
  for (MFIter mfi(*lbd_cc); mfi.isValid(); ++mfi)
    {
      ...
      FORT_RICHARD_COEF(lbddat, ARLIM(lbd_lo), ARLIM(lbd_hi),
			...
			lo,hi,domlo,domhi,bc.dataPtr(),
			rinflow_bc_lo.dataPtr(),rinflow_bc_hi.dataPtr(), 
			rinflow_vel_lo.dataPtr(),rinflow_vel_hi.dataPtr(),&ncp1);
			       ...
			       const int       nc)
{
 ...
 //
  ...
  for (int dir = 0; dir < BL_SPACEDIM; dir++)
    {
      const int ndiff = diffusivity[dir]->nComp();
      for (MFIter ecMfi(*diffusivity[dir]); ecMfi.isValid(); ++ecMfi)
        {
	  for (int i=0; i<ndiff;i++)
	  (*diffusivity[dir])[ecMfi].mult(kpedge[dir][ecMfi],0,i,1);
        }
      (*diffusivity[dir]).FillBoundary();
    }
}
