...
class RK {
 ...
 public:
  ...
  RK(fnBase<Vector>& fn, Teuchos::ParameterList& plist,
...
template <class Vector>
RK<Vector>::RK(fnBase<Vector>& fn, const method_t method,
               const Vector& initvector)
  : fn_(fn)
{
  InitMethod_(method);
...
template <class Vector>
RK<Vector>::RK(fnBase<Vector>& fn, Teuchos::ParameterList& plist,
               const Vector& initvector)
  : fn_(fn), plist_(plist)
{
  std::string methodstring =
    plist_.get<std::string>("RK method", "forward euler");
  method_t method;
