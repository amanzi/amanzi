...
* `"timestep controller type`" ``[string]`` Set the type.  One of the below
types.
* `"timestep controller X parameters`" ``[list]`` List of parameters for a
timestep controller of type X.

Available types include:

- TimestepControllerFixed_  (type `"fixed`"), a constant timestep
- TimestepControllerStandard_ (type `'standard`"), an adaptive timestep based
upon nonlinear iterations
- TimestepControllerSmarter_ (type `'smarter`"), an adaptive timestep based upon
nonlinear iterations with more control
- TimestepControllerAdaptive_ (type `"adaptive`"), an adaptive timestep based
upon error control.
- TimestepControllerFromFile_ (type `"from file`"), uses a timestep history
loaded from an HDF5 file.  (Usually only used for regression testing.)
...
struct TimestepControllerFactory {
 public:
  ...
  Create(const Teuchos::ParameterList& prec_list, Teuchos::RCP<Vector> udot,
  ...
  if (slist.isParameter("timestep controller type")) {
    ...
    if (type == "fixed") {
      if (!slist.isSublist("timestep controller fixed parameters")) {
        Errors::Message msg("TimestepControllerFactory: missing sublist "
                            "\"timestep controller fixed parameters\"");
        Exceptions::amanzi_throw(msg);
      }
      Teuchos::ParameterList tslist =
        slist.sublist("timestep controller fixed parameters");
      ...
      Teuchos::ParameterList tslist =
        slist.sublist("timestep controller standard parameters");
      ...
      Teuchos::ParameterList tslist =
        slist.sublist("timestep controller smarter parameters");
      return Teuchos::rcp(new TimestepControllerSmarter(tslist));

    } else if (type == "adaptive") {
      // if (!slist.isSublist("timestep controller adaptive parameters")) {
      //   Errors::Message msg("TimestepControllerFactory: missing sublist
      //   \"timestep controller adaptive parameters\"");
      //   Exceptions::amanzi_throw(msg);
      // }
      // Teuchos::ParameterList tslist = slist.sublist("timestep controller
      // adaptive parameters"); return Teuchos::rcp(new
      // TimestepControllerAdaptive<Vector>(tslist, udot, udot_prev));
      Errors::Message msg(
        "TimestepControllerFactory: TimestepControllerAdaptive not yet fixed "
        "for Tpetra.");
      Exceptions::amanzi_throw(msg);
      ...
      if (!slist.isSublist("timestep controller from file parameters")) {
        Errors::Message msg("TimestepControllerFactory: missing sublist "
                            "\"timestep controller from file parameters\"");
        Exceptions::amanzi_throw(msg);
      }
      Teuchos::ParameterList tslist =
        slist.sublist("timestep controller from file parameters");
      return Teuchos::rcp(new TimestepControllerFromFile(tslist));

    } else {
      Errors::Message msg("TimestepControllerFactory: invalid value of "
                          "parameter `\"timestep controller type`\"");
      Exceptions::amanzi_throw(msg);
    }
  } else {
    Errors::Message msg("TimestepControllerFactory: parameter `\"timestep "
                        "controller type`\" is missing");
    Exceptions::amanzi_throw(msg);
  }
  return Teuchos::null;
}
