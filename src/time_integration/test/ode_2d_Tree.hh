...
namespace Amanzi
{
  ...
  class ode_2d_Tree : public Interface
  {
    // using TreeVector = Epetra_TreeVector;
  ...
  public:
      ode_2d_Tree(Teuchos::RCP< Matrix> A_fast, Teuchos::RCP< Matrix> A_slow, Epetra_MpiComm* comm);
      void FunctionalTimeDerivativeFull(const double t, const Teuchos::RCP<TreeVector> u, Teuchos::RCP<TreeVector> u_eval) override;
      void FunctionalTimeDerivativeSlow(const double t, const Teuchos::RCP<TreeVector> u, Teuchos::RCP<TreeVector> f) override;
      void FunctionalTimeDerivativeFast(const double t, const Teuchos::RCP<TreeVector> u, Teuchos::RCP<TreeVector> f) override;
      
      //Need to be different for access individual entries
      void exact_rhs(double t, Teuchos::RCP<TreeVector> u_eval);
  };

  
  template<class Interface>
  ode_2d_Tree<Interface>::ode_2d_Tree(Teuchos::RCP< Matrix> A_fast, Teuchos::RCP< Matrix> A_slow, Epetra_MpiComm* comm):
 ...
 */
  template<class Interface>
  void ode_2d_Tree<Interface>::FunctionalTimeDerivativeFull(const double t, const Teuchos::RCP<TreeVector> u, Teuchos::RCP<TreeVector> u_eval) {
    A_->Multiply(false, *u->Data()->ViewComponent("cell"), *u_eval->Data()->ViewComponent("cell"));
  }
 ...
 */
  template<class Interface>
  void ode_2d_Tree<Interface>::FunctionalTimeDerivativeSlow(const double t, const Teuchos::RCP<TreeVector> u, Teuchos::RCP<TreeVector> u_eval){
    A_slow_->Multiply(false, *u->Data()->ViewComponent("cell"), *u_eval->Data()->ViewComponent("cell"));
  }

  template<class Interface>
  void ode_2d_Tree<Interface>::FunctionalTimeDerivativeFast(const double t, const Teuchos::RCP<TreeVector> u, Teuchos::RCP<TreeVector> u_eval){
    A_fast_->Multiply(false, *u->Data()->ViewComponent("cell"), *u_eval->Data()->ViewComponent("cell"));
  }
 ...
 */
  template<class Interface>
  void ode_2d_Tree<Interface>::exact_rhs(double t, Teuchos::RCP<TreeVector> u_eval){
    Teuchos::RCP<Epetra_MultiVector> u_eval_view_ = u_eval->Data()->ViewComponent("cell");
    if ((*A_)[0][1] == 0 && (*A_)[1][0] == 0)
    {
      (*u_eval_view_)[0][0] = exp((*A_)[0][0] * t);
      (*u_eval_view_)[0][1] = exp((*A_)[1][1] * t);
    ...
    (*u_eval_view_)[0][0] = temp_cosh + temp_sinh * ((*A_)[0][0] + 2.0*(*A_)[0][1] - (*A_)[1][1]);
    (*u_eval_view_)[0][1] = temp_cosh + temp_sinh * (-(*A_)[0][0] + 2.0*(*A_)[1][0] + (*A_)[1][1]);
  }
