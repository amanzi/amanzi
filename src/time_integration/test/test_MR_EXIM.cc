...
SUITE(MR_GARK_EXIM_Tests) {
  // data structures for testing
  struct test_data {
    Epetra_MpiComm *comm;

    Teuchos::RCP<Epetra_Vector> init;
    Teuchos::RCP<Epetra_Vector> init_full;
    ...
    test_data() {
      ...
      init_full = Teuchos::rcp(new Epetra_Vector( map_full));
  ...
  //   // create the time stepper
  //   Teuchos::RCP<Amanzi::MRG_IMIM_TI<Epetra_Vector, Epetra_BlockMap> > TS =
  //       Teuchos::rcp(new MRG_IMIM_TI<Epetra_Vector, Epetra_BlockMap>(ToyProblem, MrGark_IMIM_2, plist, plist, init, init_full));
