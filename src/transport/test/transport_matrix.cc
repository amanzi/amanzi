...
cout << "Test: bi-quartic solution with TPFA (using factory)" << endl;
#ifdef HAVE_MPI
  Epetra_MpiComm* comm = new Epetra_MpiComm(MPI_COMM_WORLD);
#else
  Epetra_SerialComm* comm = new Epetra_SerialComm();
  ...
  Teuchos::RCP<Dispersion> matrix = factory.Create("tpfa", plist);
  matrix->Init(&(TPK.dispersion_models()), mesh, TS);
  matrix->Init();
  matrix->InitPreconditioner("Hypre AMG", plist.sublist("Preconditioners"));
  matrix->SymbolicAssembleGlobalMatrix();
  matrix->CalculateDispersionTensor(flux, phi, ws);
