...
// even for empty tags.
Key getKey(const Key& var, const Tag& tag, bool force_delimiter = false);
Key getKey(const KeyTag& var_tag, bool force_delimiter = false);
...
Key getDerivKey(const Key& var, const Tag& tag, const Key& wrt, const Tag& wrt_tag);
...
Key cleanPListName(const std::string& name, const std::string& default_name = "");
...
// Read a domain name in a standard way, potentially with a dtype
Key readDomain(Teuchos::ParameterList& plist, const Key& dtype, const Key& default_name);
...
// value.
Key readDomainHint(Teuchos::ParameterList& plist, const Key& hint, Key hint_dtype, Key dtype);
...
Key readSuffix(Teuchos::ParameterList& list, const Key& basename, const Key& default_name = "");
...
KeyTag readKeyTag(Teuchos::ParameterList& list,
                  const Key& domain,
                  const Key& basename,
                  const Key& default_name = "ERROR_NO_DEFAULT_PROVIDED",
...
// spec.
Teuchos::Array<Key> readKeys(Teuchos::ParameterList& list,
                             const Key& domain,
                             const Key& basename,
                             Teuchos::Array<Key> const* const default_names = nullptr);

// Read a tag from a parameter list
Tag readTag(Teuchos::ParameterList& list,
            const std::string& param = "",
            const Tag& default_tag = Tag{});
