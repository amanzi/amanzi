...
/**
 * @brief RaggedArray data structure
 * @param T Type used for the entries
...
public:
  ...
  RaggedArray(const int& row_map_size, const int& entries_size)
  {
  ...
  RaggedArray(const int& row_map_size)
  {
    row_map_.realloc(row_map_size+1);
    sizes_.realloc(row_map_size, dim);
  }
  ...
  RaggedArray(const RaggedArray<T2,D,MemSpace>& other) :
   ...
   * The RaggedArray is not usable before this operation
   */
  void prefix_sum(){
  ...
  */
   ...
   */
  void assign(const RaggedArray& csr){
...
KOKKOS_INLINE_FUNCTION CT<DeviceOnlyMemorySpace> getFromRaggedArray(const RaggedArray<double, 1, DeviceOnlyMemorySpace>& csr,const int& i) {
    return std::move(CT<DeviceOnlyMemorySpace>(csr.at(i), csr.size(i,0)));
}
template<template<class> class CT>
CT<Kokkos::HostSpace> getFromRaggedArray_host(const RaggedArray<double, 1, DeviceOnlyMemorySpace>& csr,const int& i) {
    return std::move(CT<Kokkos::HostSpace>(csr.at_host(i), csr.size_host(i,0)));
}

// 2d
template<template<class> class CT>
KOKKOS_INLINE_FUNCTION CT<DeviceOnlyMemorySpace> getFromRaggedArray(const RaggedArray<double, 2, DeviceOnlyMemorySpace>& csr,const int& i) {
    return std::move(CT<DeviceOnlyMemorySpace>(csr.at(i), csr.size(i,0),csr.size(i,1)));
}
template<template<class> class CT>
CT<Kokkos::HostSpace> getFromRaggedArray_host(const RaggedArray<double, 2, DeviceOnlyMemorySpace>& csr,const int& i) {
    return std::move(CT<Kokkos::HostSpace>(csr.at_host(i), csr.size_host(i,0),csr.size_host(i,1)));
}

// 3d
template<template<class> class CT>
KOKKOS_INLINE_FUNCTION CT<DeviceOnlyMemorySpace> getFromRaggedArray(const RaggedArray<double, 3, DeviceOnlyMemorySpace>& csr,const int& i) {
    return std::move(CT<DeviceOnlyMemorySpace>(csr.at(i), csr.size(i,0),csr.size(i,1),csr.size(i,2)));
}
template<template<class> class CT>
CT<Kokkos::HostSpace> getFromRaggedArray_host(const RaggedArray<double, 3, DeviceOnlyMemorySpace>& csr,const int& i) {
