...
class ReaderNetCDF : public Reader {
 public:
  ...
  virtual void
  read(const std::string& varname, Teuchos::Array<double>& vec, int index = -1) const override
  {
    read_(varname, vec, index);
  }
  virtual void
  read(const std::string& varname, Teuchos::Array<int>& vec, int index = -1) const override
  {
    read_(varname, vec, index);
  }
