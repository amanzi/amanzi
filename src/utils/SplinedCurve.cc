...
SplinedCurve::SplinedCurve(std::vector<double> x, std::vector<double> y,
...
void
SplinedCurve::Setup(std::vector<double> x, std::vector<double> y,
                    std::vector<double> dydx, SplineEndpoints_t endpoints,
                    bool enforce_monotonicity)
{
  x_ = x;
  y_ = y;
...
SplinedCurve::Setup_()
{
  ...
  for (int i = 0; i != dx_.size(); ++i) {
    dx_[i] = x_[i + 1] - x_[i];
    if (dx_[i] <= 0.) {
      Errors::Message msg("SplinedCurve independent variable x must be "
  ...
  if (mono_) {
    ...
    if (mean_slope > 0.) {
      ...
      if (min_slope < 0.) {
        Errors::Message msg(
          "SplinedCurve requested monotonicity_preserving with postive mean "
      ...
      if (max_slope > 0.) {
        Errors::Message msg(
          "SplinedCurve requested monotonicity_preserving with negative mean "
      ...
      if (min_slope != 0.0 || max_slope != 0.0) {
        Errors::Message msg(
          "SplinedCurve requested monotonicity_preserving with zero mean slope "
