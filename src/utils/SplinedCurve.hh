...
class SplinedCurve {
 public:
  ...
  typedef std::pair<SplineExtrapolation_t, SplineExtrapolation_t>
    SplineEndpoints_t;

  class Amanzi_exception_SplineOutOfRange
    : public Exceptions::Amanzi_exception {};

  SplinedCurve() {}

  SplinedCurve(std::vector<double> x, std::vector<double> y,
               std::vector<double> dydx,
               SplineEndpoints_t endpoints = SplineEndpoints_t(
                 SplineExtrapolation_t::THROW, SplineExtrapolation_t::THROW),
               bool enforce_monotonicity = false);

  void
  Setup(std::vector<double> x, std::vector<double> y, std::vector<double> dydx,
        SplineEndpoints_t endpoints = SplineEndpoints_t(
          SplineExtrapolation_t::THROW, SplineExtrapolation_t::THROW),
        bool enforce_monotonicity = false);
...
SplinedCurve::Value(double x)
{
  int i = interval_(x);
  if (i == -1) {
    // first make sure it isn't the left end, which returns -1 thanks to
    // lower_bound
    if (x == x_[0]) {
      i = 0;
    } else {
...
SplinedCurve::Derivative(double x)
{
  ...
  double t_sq = t * t;
  double dydt =
    (6 * t_sq - 6 * t) * y_[i] + (3 * t_sq - 4 * t + 1) * dx_[i] * dy_[i] +
