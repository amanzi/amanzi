...
void
TimeStepManager::RegisterTimeEvent(double start, double period, double stop,
                                   bool phys)
{
...
TimeStepManager::RegisterTimeEvent(std::vector<double> times, bool phys)
{
  ...
  loc_times.erase(std::unique(loc_times.begin(), loc_times.end(), near_equal),
                  loc_times.end());
...
TimeStepManager::TimeStep(double T, double dT, bool after_failure)
{
  ...
  if ((dt_stable_storage > 0) && (!after_failure)) {
    if (vo_ != Teuchos::null) {
      if (vo_->getVerbLevel() >= Teuchos::VERB_MEDIUM) {
        Teuchos::OSTab tab = vo_->getOSTab();
        *vo_->os() << "PK proposed dT=" << dT << " [sec], changing it to "
                   << dt_stable_storage << " [sec]" << std::endl;
      }
    }
  ...
  for (std::list<TimeEvent>::const_iterator i = timeEvents_.begin();
       i != timeEvents_.end();
       ++i) {
    ...
    case TimeEvent::SPS: {
      if (T < i->start_ && !Amanzi::near_equal(T, i->start_)) {
        next_T_this_event = i->start_;
                 ...
                 (T <= i->stop_ || Amanzi::near_equal(T, i->stop_))) {
        double n_periods = floor((T - i->start_) / i->period_);
        if (Amanzi::near_equal(n_periods + 1.0, (T - i->start_) / i->period_))
          n_periods += 1.0;
    ...
    case TimeEvent::TIMES: {
      ...
      for (std::vector<double>::const_iterator j = i->times_.begin();
           j != i->times_.end();
           ++j) {
        if (*j > T) {
          next_T_this_event = *j;
    ...
    if (vo_ != Teuchos::null) {
      if (vo_->getVerbLevel() >= Teuchos::VERB_MEDIUM) {
        ...
        *vo_->os() << "Proposed dT=" << units.OutputTime(dT)
                   << ". CD limits it to " << units.OutputTime(time_remaining)
                   << std::endl;
      }
    }
...
TimeStepManager::print(std::ostream& os, double start, double end) const
{
  ...
  print_times.erase(
    std::unique(print_times.begin(), print_times.end(), near_equal),
    print_times.end());
  for (std::vector<double>::const_iterator i = print_times.begin();
       i != print_times.end();
       ++i) {
    os << *i << " ";
  }
