...
struct Func2 {
  KOKKOS_INLINE_FUNCTION
  double operator()(double x) const { return Kokkos::sin(x) - x / 2; }
};

struct Func3 {
  KOKKOS_INLINE_FUNCTION
  double operator()(double x) const { return 2 * (x * Kokkos::exp(-20.0) - Kokkos::exp(-20 * x)) + 1.0; }
};


struct Func4 {
  KOKKOS_INLINE_FUNCTION
  double operator()(double x) const { return x ? (x / 1.5 + std::sin(x) - 1) / 2 : -0.5; }
};


struct Func5 {
  KOKKOS_INLINE_FUNCTION
  double operator()(double x) const { return (20 * x - 1) / (19 * x); }
};


struct Func6 {
  KOKKOS_INLINE_FUNCTION
  double operator()(double x) const { return std::pow(x, 12) - 0.2; }
};
...
TEST(BRENT_AND_TOMS748)
{
  ...
  // Test 1
  itr = 100;
  x0 = findRootBrent(Func1(), 0.0, 1.0, tol, &itr);
  CHECK_CLOSE(0.5, x0, tol);
  printf(" 1 %11.5f %9i %9i\n", x0, itr, itr2[0]);

  // Test 2
  itr = 100;
  x0 = findRootBrent(Func2(), 1.57, 3.14, tol, &itr);
  printf(" 2 %11.5f %9i %9i\n", x0, itr, itr2[1]);

  // Test 3
  itr = 100;
  x0 = findRootBrent(Func3(), 0.0, 0.9, tol, &itr);
  printf(" 3 %11.5f %9i %9i\n", x0, itr, itr2[2]);

  // Test 4
  itr = 100;
  x0 = findRootBrent(Func4(), -1.0e+4, 1.57, tol, &itr);
  printf(" 4 %11.5f %9i %9i\n", x0, itr, itr2[3]);

  // Test 5
  itr = 100;
  x0 = findRootBrent(Func5(), 0.01, 0.8, tol, &itr);
  printf(" 5 %11.5f %9i %9i\n", x0, itr, itr2[4]);

  // Test 6
  itr = 100;
  x0 = findRootBrent(Func6(), 0.0, 5.0, tol, &itr);
