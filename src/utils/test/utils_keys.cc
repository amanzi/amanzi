...
SUITE(UTILS_KEYS)
{
  ...
  TEST(KEY_OPERATIONS)
  {
    ...
    CHECK_EQUAL("dsurface-water_content|dpressure",
                getDerivKey("surface-water_content", "pressure"));
  ...
  TEST(READS_DOMAINS)
  {
    ...
    }
    {
      // cleans names
      Teuchos::ParameterList plist;
      CHECK_EQUAL("domain-isgood",
                  cleanPListName(plist.sublist("my_sub").sublist("domain-isgood")));
    }
...

    {
      ...
      CHECK_EQUAL("my_surface",
                  plist.get<std::string>("surface domain name")); // default set into list
    }
  ...
  {
    {
      ...
      CHECK_EQUAL("surface-air_temperature",
                  readKey(plist, "surface", "air temperature", "air_temperature"));
    }

    {
      ...
      CHECK_EQUAL("surface-nottemperature",
                  readKey(plist, "surface", "temperature", "temperature"));
    }
