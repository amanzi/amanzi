...
class DenseMatrix {
 public:
  ...
  DenseMatrix(const DenseMatrix<MEMSPACE>& other)
    : m_(other.m_), n_(other.n_), mem_(other.mem_)
  {
    if (mem_ != 0) {
      Kokkos::resize(data_, mem_);
  ...
  KOKKOS_INLINE_FUNCTION int Multiply(const DenseMatrix<MEMSPACE>& A, const DenseMatrix<MEMSPACE>& B, bool transposeA)
  {
    if (!transposeA) assert(m_ == A.m_ && n_ == B.n_ && A.n_ == B.m_);
    else assert(m_ == A.n_ && n_ == B.n_ && A.m_ == B.m_);
  ...
  KOKKOS_INLINE_FUNCTION int OrthonormalizeColumns(int n1, int n2)
  {
    ...
    for (int i = n1; i < n2; ++i) {
      ...
      for (int j = n1; j < i; ++j) {
        ...
        for (int k = 0; k < m_; ++k)
          operator()(k,i) -= sum * operator()(k,j);
      }

      sum = 0.0;
      for (int k = 0; k < m_; ++k)
        sum += operator()(k,i) * operator()(k,i);
      if (sum == 0.0) return -1;
