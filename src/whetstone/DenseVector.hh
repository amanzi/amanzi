...
class DenseVector {
 public:
  ...
  DenseVector(int mrow, double* data)
    : m_(mrow), mem_(mrow), unmanaged_(false)
  {
  ...
  KOKKOS_INLINE_FUNCTION DenseVector<MEMSPACE>& update(double sa, const DenseVector<MEMSPACE>& A, double sthis)
  {
  ...
  update(double sa, const DenseVector<MEMSPACE>& A, double sb, const DenseVector<MEMSPACE>& B, double sthis)
  {
...
KOKKOS_INLINE_FUNCTION
double operator*(const DenseVector<MEMSPACE>& A, const DenseVector<MEMSPACE>& B)
{
  double res(0);
  A.dot(B, &res);
  return res;
...
template <class MEMSPACE>
KOKKOS_INLINE_FUNCTION
DenseVector<MEMSPACE> operator*(double val, const DenseVector<MEMSPACE>& v)
{
...
// output
inline std::ostream& operator<<(std::ostream& os, const DenseVector<Kokkos::HostSpace>& A)
{
