...
class Polynomial : public PolynomialBase<MEMSPACE> {
  ...
  void reshape(int d, int order, bool reset = false)
  {
    if (d_ != d) {
      ...
      if (reset) { 
        ...
        Kokkos::View<double*,MEMSPACE> data = coefs_.Values();
  ...
  ****************************************************************** */
  Polynomial ChangeOrigin(
      const Monomial<MEMSPACE>& mono, const AmanziGeometry::Point& origin)
  {
  ...
  ****************************************************************** */
  ...
  template<typename U> 
  friend Polynomial<U> operator*(const Polynomial<U>& poly1, const Polynomial<U>& poly2);
...
std::ostream& operator << (std::ostream& os, const Polynomial<MEMSPACE>& p)
{
  int d = p.dimension();
