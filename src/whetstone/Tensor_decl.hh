...
class Tensor {
 public:
  ...
  Tensor(const Tensor<MEMSPACE>& other) : Tensor(other.d_, other.rank_)
  {
    Kokkos::deep_copy(data_, other.data_);
  }


#if WHETSTONE_VIEW_SEMANTICS  // Default assigment implies view semantics
  KOKKOS_INLINE_FUNCTION
  Tensor(Kokkos::View<double*, MEMSPACE> data, int d, int rank, int size)
  {
    d_ = d;
    rank_ = rank;
    data_ = data;
  ...
  KOKKOS_INLINE_FUNCTION Tensor(Tensor<MEMSPACE>&& other)
  {
    d_ = other.d_;
    rank_ = other.rank_;
    data_ = other.data_;
  ...
  KOKKOS_INLINE_FUNCTION Tensor<MEMSPACE>& operator=(const Tensor<MEMSPACE>&& other)
  {
    d_ = other.d_;
    rank_ = other.rank_;
    data_ = other.data_;
...
#else
  Tensor<MEMSPACE>& operator=(const Tensor<MEMSPACE>& other) = delete;
#endif
   ...
   ****************************************************************** */
  KOKKOS_INLINE_FUNCTION Tensor<MEMSPACE>& operator*=(double c)
  {
   ...
   ****************************************************************** */
  KOKKOS_INLINE_FUNCTION Tensor<MEMSPACE>& operator-=(const Tensor<MEMSPACE>& T)
  {
   ...
   ****************************************************************** */
  KOKKOS_INLINE_FUNCTION int SetColumn(int column, const AmanziGeometry::Point& p)
  {
    if (rank_ == 2) {
  ...
  KOKKOS_INLINE_FUNCTION int SetRow(int row, const AmanziGeometry::Point& p)
  {
    if (rank_ == 2) {
...
operator*(const Tensor<MEMSPACE>& T, const AmanziGeometry::Point& p)
{
  ...
  if (rank == 1) {
    ...
    int idx = 0;
    for (int i = 0; i < d; i++) {
      for (int j = 0; j < d; j++) { p2[j] += data[idx++] * p[i]; }
    }
    return p2;
...
KOKKOS_INLINE_FUNCTION double DotTensor(const Tensor<MEMSPACE>& T1, const Tensor<MEMSPACE>& T2)
{
  Kokkos::View<double*, MEMSPACE> data1 = T1.data();
  Kokkos::View<double*, MEMSPACE> data2 = T2.data();
