...
def createFromXML(filename):
    ...
    #  Material Properties
    #
    strK = search.find_tag_path(xml, ["amanzi_input","materials","material,Left half","permeability",]).get('x')
    params["k1"] = float(strK)
    strK = search.find_tag_path(xml, ["amanzi_input","materials","material,Right half","permeability",]).get('x')
